package io.ruin.kotlin.api

import io.ruin.kotlin.content.event.GameEventContext
...
import io.ruin.model.inter.InterfaceEventMask.getMask
...
fun Player.openMainInterface(interId: Int) = openInterface(ToplevelComponent.MAINMODAL, interId)

fun Player.openPrimaryOverlay(interId: Int) = openInterface(ToplevelComponent.WILDERNESS_OVERLAY, interId)

fun Player.closePrimaryOverlay() = closeInterface(ToplevelComponent.WILDERNESS_OVERLAY)
...
suspend fun Player.chat(msg: String, npcId: Int, hideContinue: Boolean = false) {
	...
	dialogue.open(player)
	event.yield()
	closeInterface(ToplevelComponent.CHATBOX)
}
...
suspend fun NPC.chat(msg: String, hideContinue: Boolean = false) {
	...
	dialogue.open(player)
	event.yield()
	player.packetSender.removeSubInterface(ToplevelComponent.CHATBOX.interfaceId, Subcomponent.DIALOGUE.component)
...
suspend fun Player.options(vararg options: String): Int {
	...
	packetSender.sendToplevelSubInterface(Interface.OPTIONS_DIALOGUE, Subcomponent.DIALOGUE, ToplevelComponent.CHATBOX);
	packetSender.setAlignment(Interface.OPTIONS_DIALOGUE, 1, 0, -10)
	CS2Script.CHATBOX_MULTI_INIT.sendScript(this, *params.toTypedArray())
	packetSender.sendIfEvents(Interface.OPTIONS_DIALOGUE, 1, 1, options.size + 1, getMask(InterfaceEventMask.Continue))
...
suspend fun Player.messageBox(msg: String, hideContinue: Boolean = false, lineHeight: Int = 24) {
	val event = coroutineContext[GameEventContext.Key]!!.event
	player.packetSender.sendToplevelSubInterface(
		Interface.MESSAGE_DIALOGUE,
		Subcomponent.DIALOGUE,
		ToplevelComponent.CHATBOX
	);
