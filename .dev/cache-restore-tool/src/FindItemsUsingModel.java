...
// Read-only. Scans EVERY item in Zelus's live cache (using the same real modern opcode table as
// TraceItemOpcodes.java/DecodeMadAngelItemsV2.java) to find which OTHER, pre-existing items (if
// any) already reference a given model id -- checking whether "Jar of light"'s model (28441) has
// any already-proven-working precedent, or whether it's actually never been exercised by anything
// before despite being present in the cache.
public class FindItemsUsingModel {
    public static void main(String[] args) throws Exception {
        String cachePath = args[0];
        int targetModel = Integer.parseInt(args[1]);
        ...
        try (Store store = new Store(new File(cachePath))) {
            ...
            for (int i = 0; i < fileData.length; i++) {
                ...
                if (model != null && model == targetModel) {
                    System.out.println("item " + id + " uses model " + targetModel);
                    found++;
                    if (found >= 20) {
                        System.out.println("(stopping after 20 matches)");
                        break;
                    }
                }
            }
            System.out.println("total items found referencing model " + targetModel + ": " + found + (found >= 20 ? "+" : ""));
        }
    }

    /** Returns the item's inventoryModel (opcode 1 or 44), or null if decoding hits something
     * unrecognized (skips rather than crashing the whole scan). */
