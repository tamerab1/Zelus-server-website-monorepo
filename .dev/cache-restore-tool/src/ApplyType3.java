...
// Re-encodes the given model archives to Type3 in place (same model ids), using
// ModelType3Encoder. Verifies each model round-trips cleanly (decode -> encode -> decode, exact
// field match -- see RoundTripTestType3) BEFORE writing anything. Writes are all-or-nothing per
// run: if any model fails verification, nothing is written.
//
// Usage: <verify|apply> <cachePath> <modelId1,modelId2,...> [notexture]
public class ApplyType3 {
    public static void main(String[] args) throws Exception {
        ...
        if (args.length > 3 && args[3].equals("notexture")) {
            ModelType3Encoder.DISABLE_TEXTURING = true;
        ...
        try (Store store = new Store(new File(cachePath))) {
            ...
            for (int modelId : modelIds) {
                ...
                if (trailerA == -3 && trailerB == -1) {
                    System.out.println(modelId + ": already Type3 - skipping");
                    continue;
                }
                ...
                java.util.List<String> diffs = RoundTripTestType3.compare(orig, roundTrip);
                if (!diffs.isEmpty()) {
            ...
            for (var entry : newBytesByModelId.entrySet()) {
                ...
                // so there's no trailing revision suffix to exclude). See ApplyType1ToType2.java
                // for the full explanation of why this matters (silent JS5 download rejection).
            ...
            System.out.println("APPLY complete. " + newBytesByModelId.size() + " model archive(s) converted to Type3.");
        }
    }
}
