...
constructor(
	...
	public fun findAllEntries(zone: ZoneKey): List<Player>? {
		val entries = zones[zone] ?: return null
		return entries.entries
	}

	public fun findAllCoordEntries(coords: Int): List<Player>? =
		findAllEntries(ZoneKey.from(CoordGrid(coords)))//?.filter { it.coords == coords }

	fun zoneDel(player: Player) {
		zoneDel(player, ZoneKey.from(player.coords))
	}
