...
public class SeqType {
	...
	public static SeqType get(int id) {
		SeqType result = null;
	...
	public static SortedSet<Integer> findAnimationsWithSameRigging(int... ids) { // idk about this name LOL
		...
		for (int srcAnim : ids) {
			if (srcAnim != -1) {
				SeqType anim = SeqType.get(srcAnim);
				if (anim == null)
					continue;
		...
		anim_loop:
		for (SeqType anim : SeqType.LOADED) {
