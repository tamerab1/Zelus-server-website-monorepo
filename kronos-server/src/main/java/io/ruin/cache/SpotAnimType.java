...
public class SpotAnimType {
	...
	public static SpotAnimType[] LOADED;

	public static void load() {
		IndexFile index = Server.fileStore.get(2);
		LOADED = new SpotAnimType[index.getLastFileId(13) + 1];
		for (int id = 0; id < LOADED.length; id++) {
			byte[] data = index.getFile(13, id);
			SpotAnimType def = new SpotAnimType();
			def.id = id;
	...
	public static SpotAnimType get(int id) {
