...
public class ObjType {
	...
	public static final Int2ObjectMap<ObjType> cached = new Int2ObjectOpenHashMap<>();
	public static final Long2ObjectMap<List<ObjType>> nameToTypes = new Long2ObjectOpenHashMap<>();
	...
	public static void load() {
		...
		log.info("Loading {} objs", size);
		for (int id = 0; id < size; id++) {
			ObjType def = new ObjType();
			def.id = id;
			...
			if (data != null) {
				 ...
				 * List<ObjType> types = nameToTypes.computeIfAbsent(nameLong, k -> new
				 * ObjectArrayList<>());
				 * types.add(def);
				 */
			}
	...
	public static void forEach(Consumer<ObjType> consumer) {
		for (ObjType def : cached.values()) {
			if (def != null)
				consumer.accept(def);
		}
	}
	...
	public static int findId(String name) {
		...
		System.err
				.println("Failed to find item with name: " + name + " | " + new Throwable().getStackTrace()[1].toString());
		return -1;
	}
	...
	public boolean masterType;

	public int lowAlchValue, highAlchValue;

	public boolean achievementReqIsIronmanOnly;

	public boolean skipPathingBeforeInteracting;

	/**
	 * Custom data
	 */

	public int dropOption = 5;
	...
	public ItemNPCAction defaultItemOnNpcAction;

	public ItemPlayerAction defaultItemOnPlayerAction;

	public ItemObjectAction defaultItemOnObjectAction;
	...
	public boolean sunlightSpear;

	public boolean free;

	public boolean coxItem;

	public boolean completionistType;
	...
	public void setCustomFields() {
		...
		if (id == 5733) {
			skipPathingBeforeInteracting = true;
		}
		if (id == 30385) { // shadowrs: fix until done by cache tool.
			placeholderMainId = 30383; // was wrong id 30382
		}
	}
	...
	private void decode(InBuffer buffer, int opcode) {
		if (opcode == 1)
			...
			buffer.readStringCp1252NullTerminated(); // no idea
		} else if (opcode == 4)
			...
			unknown1 = buffer.readStringCp1252NullTerminated();
		} else if (opcode == 11)
			stackable = true;
		else if (opcode == 12)
			...
			wearPos3 = buffer.readByte(); // wearPos3
		else if (opcode >= 30 && opcode < 35) {
			groundOptions[opcode - 30] = buffer.readStringCp1252NullTerminated();
			if (groundOptions[opcode - 30].equalsIgnoreCase("Hidden"))
				groundOptions[opcode - 30] = null;
		} else if (opcode >= 35 && opcode < 40)
			inventoryOptions[opcode - 35] = buffer.readStringCp1252NullTerminated();
		else if (opcode == 40) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (stringType)
					attributes.put(key, buffer.readStringCp1252NullTerminated());
				else
	...
	public static int notedId(int id) {
	...
	public static ObjType unnoted(int id) {
		var def = ObjType.get(id);
		if (def == null) {
			return def;
		}
		return ObjType.get(def.unnotedId());
	}

	public static int unnotedId(int id) {
		var def = ObjType.get(id);
		if (def == null) {
			return id;
		}
		return def.unnotedId();
	}

	public ObjType fromNote() {
		return get(notedId);
	}

	public int unnotedId() {
		return isNote() ? fromNote().id : getId();
	}
	...
	public interface ItemDefPredicate {
		...
		boolean accept(ObjType def, String name);

		default boolean test(ObjType def) {
	...
	/**
	 * Adds a listener for when a player with this item equipped <b>deals
	 * damage</b>.
	 */
	...
	/**
	 * Adds a listener for when a player with this item equipped <b>takes a
	 * hite</b>.
	 */
	...
	/**
	 * Adds a listener for when a player with this item equipped <b>is defending
	 * against a hit</b>.
	 */
	...
	/**
	 * Adds a listener for when a player with this item equipped <b>is attacking but
	 * the hit hasn't yet been dealt.</b>.
	 */
