...
public class item_info extends DataFile {
	...
	public Object fromJson(String fileName, String json) {
		...
		temps.forEach(temp -> {
			ObjType def = ObjType.get(temp.id);
			if (def == null) {
		...
		ObjType.forEach(this::loadMisc);
		shield_types.unload();
//        weapon_types.unload();
		return temps;
	}

	private void loadMisc(ObjType def) {
		def.dropOption = def.getOption("drop");
		if (def.dropOption == -1) {
			// TODO(polish) either check if its alright in the real implementation
			def.dropOption = 5;
		}
		 ...
		 */
		if (def.isNote()) {
			ObjType reg = ObjType.get(def.notedId);
			def.name = reg.name;
		...
		if (def.isPlaceholder()) {
			ObjType reg = ObjType.get(def.placeholderMainId);
			def.name = reg.name;
			def.examine = reg.examine;
            /*ObjType.forEach(def2 -> {
	...
	public static final class Temp {
		...
		@Expose
		public Double weight_equipped;
		@Expose
		public int protect_value;
		@Expose
		 ...
		 */
		@Expose
		public Integer equip_slot;
		@Expose
		public Boolean two_handed;
		@Expose
		public Boolean hide_hair;
		@Expose
		public Boolean hide_beard;
		@Expose
		public Boolean hide_arms;
		//okay this isn't the prettiest design...
		@Expose
		public Integer attack_level, defence_level, strength_level, hitpoints_level, ranged_level, prayer_level, magic_level,
		...
		//neither is this...
		@Expose
		public Integer stab_attack_bonus, slash_attack_bonus, crush_attack_bonus, magic_attack_bonus, range_attack_bonus,
		...
		@Expose
		public String ranged_ammo;

		@Expose
		public String achievement;
		@Expose
		public Boolean achievement_req_is_ironman_only; // indicates the achievement req should only be applied to ironman characters
		@Expose
