...
public class teleports extends DataFile {
	...
	public static void open(Player player) {
		...
		player.getPacketSender().sendTeleports(
				World.type.getWorldName() + " Teleportsplateau",
		...
		player.openInterface(ToplevelComponent.MAINMODAL, 583);
	}
	...
	public static void previous(Player player) {
		if (player.previousTeleportX == -1)
			...
			teleportStart(player, player.previousTeleportX, player.previousTeleportY, player.previousTeleportZ,
					player.previousTeleportPrice);
	}
	...
	private static void teleportStart(Player player, int x, int y, int z, int price) {
		...
		player.unsafeDialogue(new YesNoDialogue("Are you sure you want to use this teleport?",
				"This teleport will cost " + price + " blood money.", BLOOD_MONEY, price, () -> {
