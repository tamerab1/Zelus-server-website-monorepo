...
import io.ruin.model.skills.construction.room.impl.AchievementGalleryRoom;
import io.ruin.model.skills.construction.room.impl.ChapelRoom;
import io.ruin.model.skills.construction.room.impl.DungeonGuardedRoom;
import io.ruin.model.skills.construction.room.impl.DungeonRoom;
import io.ruin.model.skills.construction.room.impl.MenagerieRoom;
import io.ruin.model.skills.construction.room.impl.OublietteRoom;
...
import io.ruin.model.skills.construction.room.impl.SuperiorGardenRoom;
import io.ruin.model.skills.construction.room.impl.TreasureRoom;
...
public enum RoomDefinition {
				...
				for (int localY = 0; localY < 8; localY++) {
					...
					if (tile != null && tile.gameObjects != null) {
						...
						tile.gameObjects.stream()
							...
							.map(o -> new VisualObject(o.getId(), finalLocalX, finalLocalY, o.getType(), o.getDirection()))
	...
	public static void openRoomSelection(Player player, Consumer<RoomDefinition> action) {
		player.openInterface(ToplevelComponent.MAINMODAL, Interface.CONSTRUCTION_ROOM_CREATION);
		player.set("ROOM_CREATION_CONSUMER", action);
	}

	public Room create() {
		try {
			return handler.getDeclaredConstructor().newInstance().init(this);
