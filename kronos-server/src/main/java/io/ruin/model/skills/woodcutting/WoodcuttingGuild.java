...
public class WoodcuttingGuild {
	...
	private enum Egg {
		...
		public static final Egg[] VALUES = values();
	...
	private static void openGate(Player player, GameObject northGate, GameObject southGate, int replacementX, int replacementY) {
		player.startEvent(event -> {
			...
			if (player.getAbsX() >= replacementX)
				player.stepAbs(replacementX - 1, player.getAbsY(), StepType.WALK);
			else
				player.stepAbs(replacementX, player.getAbsY(), StepType.WALK);
			event.delay(1);
