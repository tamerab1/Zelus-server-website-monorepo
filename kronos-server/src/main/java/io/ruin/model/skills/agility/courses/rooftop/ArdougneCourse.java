...
public class ArdougneCourse {
	...
	public static void register() {
		...
		ObjectAction.register(26635, "walk-on", (p, obj) -> p.startEvent(e -> {
			...
			p.stepAbs(2656, 3318, StepType.WALK);
			e.delay(1);
		...
		ObjectAction.register(28912, "balance-across", (p, obj) -> p.startEvent(e -> {
			...
			e.delay(1);
			p.stepAbs(2654, 3299, StepType.WALK);
			p.stepAbs(2656, 3297, StepType.WALK);
			e.waitForMovement(p);
			e.delay(1);
			p.getAppearance().removeCustomRenders();
			p.animate(759);
		...
		ObjectAction.register(15612, "jump", (p, obj) -> p.startEvent(e -> {
			...
			e.delay(2);
			p.stepAbs(2661, 3298, StepType.WALK);
			...
			e.delay(1);
			p.stepAbs(2666, 3297, StepType.WALK);
			...
			p.getMovement().restoreEnergy(Random.get(1, 2));
			AgilityPet.rollForPet(p, 450);
			PerkTaskHandler.handleCompleteActivity(p, 11);
			PlayerCounter.ARDOUGNE_ROOFTOP.increment(p, 1);
			PlayerCounter.TOTAL.increment(p, 1);
			RooftopLapCounter.notify(p, "Ardougne rooftop", PlayerCounter.ARDOUGNE_ROOFTOP.get(p));
			MarkOfGrace.rollMark(p, 90, MARK_SPAWNS);
			DailyTasks.handleTaskDecrement(p, "ardougneRooftop");
			e.delay(1);
			p.unlock();
		}));
	}
