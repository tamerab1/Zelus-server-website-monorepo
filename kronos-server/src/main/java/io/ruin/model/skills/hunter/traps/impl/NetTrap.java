...
public class NetTrap implements TrapType { // TODO net trap behavior is likely VERY buggy due to how the replacing works, needs thorough testing
	...
	public void onRemove(Player player, GameObject object) {
		...
		if (object.getDirection() < 2 && (object.getId() == getSuccessId()[0] || object.getId() == getFailedObjectId())) {
			if (!object.isRemoved())
	...
	public void collapse(Player player, Trap trap, boolean remove) {
		if (trap.isRemoved() || trap.getObject()
			.getId() == -1 || trap.getOwner() == null) {
			return;
		}
		GameObject object = trap.getObject();
		Position dropPos = new Position(object.x, object.y, object.z);
		Optional.ofNullable(getTreeObject(object)).ifPresent(tree -> tree.setId(getDefaultTreeId()));
		if (object.getDirection() < 2 && (object.getId() == getSuccessId()[0] || object.getId() == getFailedObjectId() || object.getId() == getFailingId() || object.getId() == getSucceedingId())) {
			int x = object.x;
			int y = object.y;
	...
	public static GameObject getNetObject(GameObject tree) {
		int x = tree.x;
		int y = tree.y;
		if (tree.getDirection() == 0)
			y++;
		else if (tree.getDirection() == 2)
			y--;
		else if (tree.getDirection() == 1)
			x++;
		else
			x--;
	...
	public static GameObject getTreeObject(GameObject net) {
		int x = net.x;
		int y = net.y;
		if (net.getDirection() == 0)
			y--;
		else if (net.getDirection() == 2)
			y++;
		else if (net.getDirection() == 1)
			x--;
		else
			x++;
	...
	public void layTrap(Player player, GameObject obj) {
		...
		player.startEvent(event -> {
			...
			Trap trap = new Trap(player, this, GameObject.spawn(getActiveObjectId(), player.getAbsX(), player.getAbsY(), player.getHeight(), 10, obj.getDirection()));
