...
public class ToolStorage {
	...
	enum Tool {//order should be the same as on the interface
		RAKE(5341, VarPlayerRepository.STORAGE_RAKE),
		SEED_DIBBER(5343, VarPlayerRepository.STORAGE_SEED_DIBBER),
		SPADE(952, VarPlayerRepository.STORAGE_SPADE),
		SECATEURS(5329, VarPlayerRepository.STORAGE_SECATEURS) {
			@Override
			...
			public void onDeposit(Player player, int item) {
				VarPlayerRepository.STORAGE_SECATEURS_TYPE.set(player, item == 7409 ? 1 : 0);
			}

			@Override
			public int addItem(Player player, int amount) {
				int added = player.getInventory().add(VarPlayerRepository.STORAGE_SECATEURS_TYPE.get(player) == 1 ? 7409 : 5329, amount);
				if (added > 0)
					VarPlayerRepository.STORAGE_SECATEURS_TYPE.set(player, 0);
				return added;
			}
		},
		WATERING_CAN(5331, VarPlayerRepository.STORAGE_WATERING_CAN) {
			@Override
			public boolean accept(int item) {
		...
		GARDENING_TROWEL(5325, VarPlayerRepository.STORAGE_TROWEL),
		PLANT_CURE(6036, VarPlayerRepository.STORAGE_PLANT_CURE),
		BOTTOMLESS_BUCKET(22997, VarPlayerRepository.STORAGE_BOTTOMLESS_COMPOST) {
			@Override
		...
		EMPTY_BUCKET(1925, null) {
			@Override
			public int get(Player player) {
				return (VarPlayerRepository.STORAGE_EMPTY_BUCKET_2.get(player) << 5) | VarPlayerRepository.STORAGE_EMPTY_BUCKET_1.get(player);
			}

			@Override
			public void update(Player player, int amount) {
				int newValue = get(player) + amount;
				VarPlayerRepository.STORAGE_EMPTY_BUCKET_1.set(player, newValue & 31);
				VarPlayerRepository.STORAGE_EMPTY_BUCKET_2.set(player, (newValue >> 5) & 7);
			}
		},
		COMPOST(6032, null) {
			@Override
			public int get(Player player) {
				return (VarPlayerRepository.STORAGE_COMPOST_2.get(player) << 8) | VarPlayerRepository.STORAGE_COMPOST_1.get(player);
			}

			@Override
			public void update(Player player, int amount) {
				int newValue = get(player) + amount;
				VarPlayerRepository.STORAGE_COMPOST_1.set(player, newValue & 255);
				VarPlayerRepository.STORAGE_COMPOST_2.set(player, (newValue >> 8) & 3);
			}
		},
		SUPERCOMPOST(6034, null) {
			@Override
			public int get(Player player) {
				return (VarPlayerRepository.STORAGE_SUPERCOMPOST_2.get(player) << 8) | VarPlayerRepository.STORAGE_SUPERCOMPOST_1.get(player);
			}

			@Override
			public void update(Player player, int amount) {
				int newValue = get(player) + amount;
				VarPlayerRepository.STORAGE_SUPERCOMPOST_1.set(player, newValue & 255);
				VarPlayerRepository.STORAGE_SUPERCOMPOST_2.set(player, (newValue >> 8) & 3);
			}
		},
		ULTRACOMPOST(21483, VarPlayerRepository.STORAGE_ULTRACOMPOST);

		private VarPlayerRepository config;

		private int itemId;
		private String name;

		Tool(int itemId, VarPlayerRepository config) {
			this.config = config;
			this.itemId = itemId;
	...
	private int wateringCanCharges;
	private boolean emptyBottomlessBucket;
	private transient Player player;

	public void open(Player player) {
		player.openInterface(ToplevelComponent.MAINMODAL, 125);
		player.openInterface(ToplevelComponent.SIDEMODAL, 126);
	}
	...
	public void deposit(Tool tool, int amount) {
		...
		if (maxAmount == 1) {
			for (Item item : player.getInventory().getItems()) {
				if (item == null)
					continue;
				if (tool.accept(item.getId()) || tool.accept(item.getId() + 1)) {
					item.remove();
	...
	private int getMaxAmount(Tool tool) {
		if (tool == Tool.EMPTY_BUCKET || tool == Tool.COMPOST || tool == Tool.SUPERCOMPOST || tool == Tool.ULTRACOMPOST || tool == Tool.PLANT_CURE)
			return 1000;
//        if (tool == Tool.RAKE || tool == Tool.SEED_DIBBER || tool == Tool.SPADE || tool == Tool.SECATEURS || tool == Tool.GARDENING_TROWEL) // TODO Find correct Varpbit
//            return 100;
		return 1;
	}
}
