...
public class TreasureRoom extends Room {
	...
	private int loot = 0;
	private transient int killerId = -1;
	...
	public void enableChallengeMode() {
		...
		if (getBuilt(Hotspot.TREASURE_BOSS) != null) {
			...
			boss = getHouse().addNPC(new NPC(DungeonGuardedRoom.GUARD_NPC_IDS.get(getBuilt(Hotspot.TREASURE_BOSS))).spawn(spawnPoint.x, spawnPoint.y, spawnPoint.z, Direction.getFromObjectDirection(spawnPoint.getDirection()), 0));
	...
	private void treasureChestInteract(Player player, GameObject obj) {
		...
		if (obj.getId() == chestType.getBuiltObjects()[0]) { // closed chest
