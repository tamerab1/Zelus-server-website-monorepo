...
public enum Buildable {
	...
	UNDEAD_COMBAT_DUMMY(53, 220, MID_BUILD, 20746, 9355, COMBAT_DUMMY, new Item(8921), new Item(4286, 4)),
	...
	DARK_ALTAR(80, 3888, MID_BUILD, 20619, 29149, LIMESTONE_BRICK.item(10), MAGIC_STONE.item(1), new Item(20615), Rune.BLOOD.toItem(5000), Rune.ASTRAL.toItem(10000), Rune.SOUL.toItem(5000)),
	...
	Buildable(int levelReq, double xp, int animation, int itemId, int[] builtObjects, Buildable upgradeFrom, Item... materials) {
		...
		this.requireTools = animation != -1 && SeqType.get(animation).rightHandItem == (Tool.HAMMER + 512);
		this.xp = xp;
		StringBuilder sb = new StringBuilder();
		sb.append(ObjType.get(itemId).name);
		sb.append("|");
		for (int i = 0; i < 4; i++) {
			if (i < this.materials.size()) {
				if (this.materials.get(i).getId() == 4819) {
					sb.append("Nails");
				} else {
					ObjType def = ObjType.get(this.materials.get(i).getId());
					if (def != null) {
						sb.append(def.name);
					}
				}
	...
	public void removeMaterials(Player player) {
		...
		if (player.getPlayerPerkHandler().getActivePerks(player).contains(Perks.BOB_THE_BUILDER)) {
			...
			if (Random.get(100) < c.getMaterialSaveChance())
				saveMaterials = true;
		}
		if (!saveMaterials) {
			materials.forEach(item -> {
				Material.removeMaterial(player, item);
			});
		}
	}
	...
	public boolean hasTools(Player player) {
		return !isRequireTools()
			|| (player.getInventory().contains(Tool.HAMMER, 1)
			|| player.getInventory().contains(Tool.IMCANDO_HAMMER, 1)
			|| player.getEquipment().hasId(Tool.IMCANDO_HAMMER)
			&& (player.getInventory().contains(Tool.SAW, 1) || player.getInventory().contains(Tool.CRYSTAL_SAW, 1)));
	}
