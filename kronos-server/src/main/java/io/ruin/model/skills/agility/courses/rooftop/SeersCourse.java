...
public class SeersCourse {
	...
	private static final List<Position> MARK_SPAWNS = Arrays.asList(new Position(2726, 3492, 3),
		new Position(2728, 3495, 3), new Position(2707, 3493, 2), new Position(2708, 3489, 2),
		new Position(2712, 3481, 2), new Position(2710, 3478, 2), new Position(2710, 3472, 3),
		new Position(2702, 3474, 3), new Position(2698, 3462, 2));
	...
	public static void register() {
		...
		ObjectAction.register(14932, "cross", (p, obj) -> p.startEvent(e -> {
			...
			p.stepAbs(2710, 3481, StepType.WALK);
			e.delay(1);
		...
		ObjectAction.register(14931, "jump", (p, obj) -> p.startEvent(e -> {
			...
			if (p.preventSkippingCourse) {
				...
				RooftopLapCounter.notify(p, "Seers rooftop", PlayerCounter.SEERS_ROOFTOP.get(p));
				AgilityPet.rollForPet(p, 500);
				PerkTaskHandler.handleCompleteActivity(p, 8);
				DailyTasks.handleTaskDecrement(p, "seersRooftop");
