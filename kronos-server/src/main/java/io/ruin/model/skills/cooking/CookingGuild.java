		...
		ObjectAction.register(24958, 3143, 3443, 0, "open", (player, obj) -> {
			...
			player.startEvent(event -> {
				...
				if (!player.isAt(obj.x, player.getAbsY())) {
					player.stepAbs(obj.x, player.getAbsY(), StepType.WALK);
					event.delay(1);
				}
				GameObject opened = GameObject.spawn(24959, 3143, 3444, 0, obj.getType(), 2);
				obj.skipClipping(true).remove();
				player.step(0, player.getAbsY() <= 3443 ? 1 : -1, StepType.WALK);
				event.delay(2);
