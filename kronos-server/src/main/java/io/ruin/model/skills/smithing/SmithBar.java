...
public enum SmithBar {
	...
	MITHRIL(
		...
		Arrays.asList(new Item(447, 1), new Item(453, 2)),
	...
	SmithBar(int itemId, int smithValue, int smeltLevel, double smeltXp, PlayerCounter counter, List<Item> smeltItems, SmithItem... smithItems) {
		...
		ObjType.get(itemId).smithBar = this;
	}

	public static final SmithBar[] VALUES = values();
	...
	private static void open(Player player) {
		if (!player.getInventory().hasId(Tool.HAMMER) && !player.getInventory().hasId(ItemID.DRAGON_WARHAMMER) && !player.getEquipment().hasId(ItemID.DRAGON_WARHAMMER)
			&& !player.getInventory().hasId(Tool.IMCANDO_HAMMER) && !player.getEquipment().hasId(Tool.IMCANDO_HAMMER)) {
	...
	private static void open(Player player, SmithBar bar) {
		player.smithBar = bar;
		VarPlayerRepository.SMITHING_TYPE.set(player, bar.smithValue);
		int amount = player.getInventory().getAmount(player.smithBar.itemId);
		VarPlayerRepository.SMITHING_QUANITY.set(player, 20);
		player.openInterface(ToplevelComponent.MAINMODAL, Interface.SMITHING);
		//player.getPacketSender().sendClientScript(2524, "i", -1 -1);
			...
			for (int i = 9; i <= 35; i++) {
				...
				h.actions[i] = (DefaultAction) (p, option, slot, itemId) -> {
					...
					if (amount.get() == 1) {
						item.make(p, 1);
						return;
					} else if (amount.get() == 2) {
						item.make(p, 5);
						return;
					} else if (amount.get() == 3) {
						item.make(p, 10);
						return;
					} else if (amount.get() == 4) {
						p.integerInput("Enter amount:", amt -> item.make(p, amt));
						return;
					} else if (amount.get() == 5) {
