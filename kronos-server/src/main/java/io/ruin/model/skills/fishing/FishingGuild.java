		...
		ObjectAction.register(20925, "open", (player, obj) -> {
			...
			player.startEvent(event -> {
				...
				if (!player.isAt(obj.x, player.getAbsY() <= 3393 ? obj.y - 1 : obj.y)) {
					player.stepAbs(obj.x, player.getAbsY() <= 3393 ? obj.y - 1 : obj.y, StepType.WALK);
					event.delay(1);
				}
				GameObject opened = GameObject.spawn(1542, 2611, 3393, 0, obj.getType(), 0);
				obj.skipClipping(true).remove();
				player.step(0, player.getAbsY() <= 3393 ? 1 : -1, StepType.WALK);
				event.delay(2);
