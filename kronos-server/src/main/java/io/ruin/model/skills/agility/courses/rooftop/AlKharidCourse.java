	...
	public static void register() {
		...
		ObjectAction.register(14398, "cross", (p, obj) -> p.startEvent(e -> {
			if (obj.x < 2070) {
				...
				p.lock(LockType.FULL_DELAY_DAMAGE);
				p.stepAbs(x, 3621, StepType.WALK);
				e.delay(1);
				...
				p.lock(LockType.FULL_DELAY_DAMAGE);
				p.stepAbs(3272, 3172, StepType.WALK);
				e.delay(1);
				p.getAppearance().setCustomRenders(Renders.AGILITY_BALANCE);
				e.delay(9);
				p.getAppearance().removeCustomRenders();
		...
		ObjectAction.register(14403, "teeth-grip", (p, obj) -> p.startEvent(e -> {
			...
			e.delay(1);
			p.resetAnimation();
			p.stepAbs(3315, 3163, StepType.WALK);
			p.getStats().addXp(StatType.Agility, 40.0, true);
			p.getMovement().restoreEnergy(Random.get(1, 2));
		...
		ObjectAction.register(14409, "cross", (p, obj) -> p.startEvent(e -> {
			...
			p.stepAbs(3302, 3186, StepType.WALK);
			e.delay(1);
			p.getAppearance().setCustomRenders(Renders.AGILITY_BALANCE);
			e.delay(11);
			p.stepAbs(3302, 3187, StepType.WALK);
			e.delay(1);
			p.getAppearance().removeCustomRenders();
		...
		ObjectAction.register(14399, "jump", (p, obj) -> p.startEvent(e -> {
			...
			RooftopLapCounter.notify(p, "Al-Kharid rooftop", PlayerCounter.ALKHARID_ROOFTOP.get(p));
			AgilityPet.rollForPet(p, 800);
			PerkTaskHandler.handleCompleteActivity(p, 3);
			DailyTasks.handleTaskDecrement(p, "alKharidRooftop");
