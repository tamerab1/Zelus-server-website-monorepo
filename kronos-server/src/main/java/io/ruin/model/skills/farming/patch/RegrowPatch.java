...
// for patches that "restore" produce after a certain delay, like fruit trees
public abstract class RegrowPatch extends Patch {
	...
	public boolean canRegrow() {
		return getPlantedCrop() != null &&
			getStage() == getPlantedCrop().getTotalStages() + 1 && getProduceCount() < getMaxProduce()
			&& System.currentTimeMillis() - lastRegrow >= getRegrowDelay();
	}
	...
	public void pick() {
		player.startEvent(event -> {
			while (true) {
				...
				player
					.sendFilteredMessage("You pick a " + ObjType.get(getPlantedCrop().getProduceId()).name.toLowerCase() + ".");
				removeProduce();
				update();
	...
	void regrow() {
		...
		lastRegrow += getRegrowDelay(); // increment by the delay so it works correctly when multiple regrows are
		// 'queued'
		update();
	}
