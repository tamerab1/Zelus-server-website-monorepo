...
public class MenagerieRoom extends Room {
		...
		InterfaceHandler.register(Interface.PET_HOUSE, h -> {
			h.actions[7] = (DefaultAction) (player, option, slot, itemId) -> {
				...
				if (room instanceof MenagerieRoom) {
					...
					menagerie.withdrawPet(player, slot - 50);
				}
			};
	...
	public void removeHotspotObject(GameObject obj) {
		if (getDefinition() == RoomDefinition.MENAGERIE_OUTDOORS && obj.getId() == 26865 && getBuilt(Hotspot.HABITAT) != null) { // arena floor, replace it with habitat floor instead of removing it
	...
	private void toggleRoam(Player player) {
		int newValue = VarPlayerRepository.PETS_ROAMING_DISABLED.get(player) == 0 ? 1 : 0;
		VarPlayerRepository.PETS_ROAMING_DISABLED.set(player, newValue);
	...
	private void withdrawPet(Player player, int slot) {
		...
		if (pet.move(pet.getId(), 1, player.getInventory()) > 0 && VarPlayerRepository.PETS_ROAMING_DISABLED.get(player) == 0) {
	...
	protected void onBuild() {
		loadSpawnPoints();
		if (getBuilt(Hotspot.PET_HOUSE) != null && VarPlayerRepository.PETS_ROAMING_DISABLED.get(getHouse().getOwner()) == 0) {
			for (Item item : getHouse().getPetContainer().getItems()) {
				if (item != null && item.getDef().pet != null) {
					spawnPet(item.getDef().pet);
				}
	...
	private void viewPets(Player player, GameObject gameObject) {
		...
		player.openInterface(ToplevelComponent.MAINMODAL, Interface.PET_HOUSE);
	...
	private void storePet(Player player, Item item, GameObject gameObject) {
		...
		if (item.getId() == 30016) {
			player.sendMessage("You can't store that pet in here, well because I said so.");
			return;
		}
		...
		if (VarPlayerRepository.PETS_ROAMING_DISABLED.get(player) == 0)
			spawnPet(item.getDef().pet);
		player.animate(833);
	}
	...
	private void spawnPet(Pet pet) {
		...
		NPCAction.register(npc, "pick-up", (player, n) -> {
			if (Pet.pickup(player, n, pet, false)) {
	...
	public static int getMaxPets(Buildable house) {
		return switch (house) {
			case OAK_HOUSE -> 3;
			case TEAK_HOUSE -> 5;
			case MAHOGANY_HOUSE -> 7;
			case CONSECRATED_HOUSE -> 9;
			case DESECRATED_HOUSE -> 12;
			case NATURE_HOUSE -> 96;
			default -> 0;
		};
	}
}
