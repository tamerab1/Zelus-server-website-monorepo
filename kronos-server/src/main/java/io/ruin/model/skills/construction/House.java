...
public class House {
		...
		LoginListener.register(player -> {
			if (player.house != null) {
				...
				if (player.house.servantSave.getHiredServant() != null) {
					VarPlayerRepository.HIRED_SERVANT.set(player, player.house.servantSave.getHiredServant().getVarpbitValue());
				}
	...
	@Setter
	@Getter
	private HouseStyle style = HouseStyle.BASIC_WOOD;
	private final Room[][][] rooms = new Room[3][8][8];
	@Setter
	@Getter
	private HouseLocation location = HouseLocation.EDGEVILLE;
	@Setter
	@Getter
	private boolean locked;
	@Setter
	@Getter
	private int tips; // gp left in tip jar by other players for the house owner
	@Setter
	@Getter
	private boolean tipJarNotifications = true;
	@Getter
	private final ItemContainer petContainer = new ItemContainer(); // where pets are stored
	@Setter
	@Getter
	private int moneyInMoneybag = 0;
	@Getter
	private final ServantSave servantSave = new ServantSave();
	@Getter
	private final Map<Costume, int[]> fancyDressStorage = new HashMap<>();
	@Getter
	private final Map<Costume, int[]> armourCaseStorage = new HashMap<>();
	@Getter
	private final Map<Costume, int[]> magicWardrobeStorage = new HashMap<>();
	@Getter
	private final Map<Costume, int[]> capeRackStorage = new HashMap<>();
	@Getter
	private final Map<Costume, int[]> beginnerTreasureTrailsStorage = new HashMap<>();
	@Getter
	private final Map<Costume, int[]> easyTreasureTrailsStorage = new HashMap<>();
	@Getter
	private final Map<Costume, int[]> mediumTreasureTrailsStorage = new HashMap<>();
	@Getter
	private final Map<Costume, int[]> hardTreasureTrailsStorage = new HashMap<>();
	@Getter
	private final Map<Costume, int[]> eliteTreasureTrailsStorage = new HashMap<>();
	@Getter
	private final Map<Costume, int[]> masterTreasureTrailsStorage = new HashMap<>();
	@Getter
	private final Map<Costume, int[]> toyBoxStorage = new HashMap<>();

	private transient Servant servant;
	private transient boolean hasServantsMoneybag = false;
	@Getter
	private transient boolean hasBellPull;
	@Getter
	private transient boolean buildingMode = true;
	@Getter
	private transient DynamicMap map;
	@Getter
	private transient Player owner;
	private final transient Map<RoomDefinition, Integer> roomTypeCounts = new HashMap<>();
	@Getter
	private transient Position entryPosition = null;
	@Getter
	private transient Position fairyRingPosition = null;
	@Getter
	private transient Position spiritTreePosition = null;
	@Getter
	private transient int portalCount;
	@Getter
	private transient int roomCount;
	@Getter
	private transient ChallengeMode challengeMode = ChallengeMode.OFF;
	...
	public void guestEnter(Player player, Position localToEnter) {
		...
		player.startEvent(event -> {
			...
			player.openInterface(ToplevelComponent.OVERLAY, Interface.CONSTRUCTION_LOADING);
	...
	public void buildAndEnter(Player player, Position localToEnter, boolean buildingMode, Runnable onEnter) {
		...
		VarPlayerRepository.BUILDING_MODE.set(player, buildingMode ? 1 : 0);
		player.lock();
		player.startEvent(event -> {
			player.openInterface(ToplevelComponent.POH_LOADING, Interface.CONSTRUCTION_LOADING);
			player.getPacketSender().sendMapState(2);
			try {
				build(); // may be worth investigating doing this off the main thread in the future
			} catch (DynamicMap.DynamicMapBuildException e) {
				player.getPacketSender().sendMapState(0);
				player.closeInterface(ToplevelComponent.POH_LOADING);
				player.closeInterfaces();
				player.unlock();
	...
	public void teleportInside(Player player, Position localToEnter) {
		if (localToEnter != null && (localToEnter.getZ() == 1
				|| getRoom(localToEnter.getX() / 8, localToEnter.getY() / 8, localToEnter.getZ()) != null)) {
			player.getMovement().teleport(map.swRegion.baseX + localToEnter.getX(), map.swRegion.baseY + localToEnter.getY(),
					localToEnter.getZ());
		} else if (entryPosition == null) {
			player.getMovement().teleport(map.swRegion.baseX + 31, map.swRegion.baseY + 31, 1);
		} else {
			player.getMovement().teleport(map.swRegion.baseX + entryPosition.getX(),
					map.swRegion.baseY + entryPosition.getY(), entryPosition.getZ());
		}
	}
	...
	public void build() throws DynamicMap.DynamicMapBuildException {
		build(new DynamicMap());
	}

	public void build(DynamicMap map) throws DynamicMap.DynamicMapBuildException {
		List<DynamicChunk> chunks = new ArrayList<>();
		for (int z = 0; z < 4; z++) { // up to height 3 here because even though the player can't build at 3, they can
			// build at 2 and so there might be roofs at 3
	...
	public void calculate() {
		...
		// reference to the menagerie
		MenagerieRoom menagerie = null;
					...
					if (room != null) {
						...
						if (room.getDefinition() == RoomDefinition.SUPERIOR_GARDEN) {
							if (room.hasBuildable(Buildable.FAIRY_RING) || room.hasBuildable(Buildable.SPIRIT_TREE_AND_FAIRY_RING)) {
								fairyRingPosition = room.getLocalPosition(3,
										room.hasBuildable(Buildable.SPIRIT_TREE_AND_FAIRY_RING) ? 2 : 3, x, y, z);
							}
		...
		roomCount = roomTypeCounts.values().stream().mapToInt(i -> i).sum();
	}
	...
	public boolean canAddRoom(Player player, RoomDefinition def, int x, int y, int z) {
		...
		if ((def == RoomDefinition.MENAGERIE_INDOORS || def == RoomDefinition.MENAGERIE_OUTDOORS)
				&& roomTypeCounts.getOrDefault(RoomDefinition.MENAGERIE_INDOORS, 0)
						+ roomTypeCounts.getOrDefault(RoomDefinition.MENAGERIE_OUTDOORS, 0) > 0) {
			player.dialogue(new MessageDialogue(
					"You may only have one menagerie in your house.<br><br>Use the house viewer if you'd like to move it here."));
			return false;
		}
		if (def == RoomDefinition.COSTUME_ROOM
				&& roomTypeCounts.getOrDefault(RoomDefinition.COSTUME_ROOM, 0) > 0) {
			player.dialogue(new MessageDialogue(
					"You may only have one costume room in your house.<br><br>Use the house viewer if you'd like to move it here."));
			return false;
		}
		if (roomCount >= Construction.getMaxRooms(player.getStats().get(StatType.Construction).fixedLevel)
				&& !player.isAdmin()) { // admins can bypass room limit but it probably breaks the house viewer. need to
			// test
			player.dialogue(
					new MessageDialogue("You already have the maximum number of rooms allowed for your Construction level."));
			return false;
		}
		if (z > 1) {
	...
	public boolean canRemoveRoom(Player player, Room room) {
		...
		if (room.getDefinition() == RoomDefinition.MENAGERIE_OUTDOORS
				|| room.getDefinition() == RoomDefinition.MENAGERIE_INDOORS) {
			if (!getPetContainer().isEmpty()) {
				player.dialogue(
						new MessageDialogue("You must remove the pets from the pet house before you can remove the menagerie."));
				return false;
			}
		}
		if (room.getDefinition() == RoomDefinition.COSTUME_ROOM) {
			for (CostumeStorage storage : CostumeStorage.VALUES) {
				if (!storage.getSets(player).isEmpty()) {
					player.dialogue(
							new MessageDialogue("You must empty out all the containers in the room before you can remove the room."));
					return false;
				}
			}
		}
	...
	public Room getCurrentRoom(Player player) {
		...
		// this only works because i locked houses to a single region... it would be
	...
	public void confirmRemoveRoom(Player player, Room adjacent) {
		player.dialogue(new OptionsDialogue("Remove the %s?".formatted(adjacent.getDefinition().getName()),
				new Option("Yes.", () -> {
	...
	public void setChallengeMode(ChallengeMode mode) {
		...
		switch (challengeMode) {
			case OFF:
				getMap().swRegion.players
						.forEach(p -> p.sendMessage(Color.RED.wrap("%s has disabled Challenge Mode.".formatted(owner.getName()))));
				getMap().swRegion.players.forEach(p -> p.setAction(1, null));
				break;
			case ON:
				getMap().swRegion.players
						.forEach(p -> p.sendMessage(Color.RED.wrap("%s has enabled Challenge Mode!".formatted(owner.getName()))));
				getMap().swRegion.players.forEach(p -> p.setAction(1, null));
				break;
			case PVP:
				getMap().swRegion.players
						.forEach(p -> p.sendMessage(Color.RED.wrap("%s has enabled PvP Challenge Mode!".formatted(owner.getName()))));
	...
	private void spawnServant() {
		if (servantSave.getHiredServant() != null) {
			if (!canHaveServant()) {
				owner.sendMessage(Color.RED.wrap(
						"You currently employ a servant, but do not have 2 beds in your house. They will not appear until you do."));
				return;
			}
	...
	private DynamicChunk getEmptyBasement(int x, int y) {
		...
		// east = true;
		// }
		// return adjacents > 0 ? RoomDefinition.BASEMENT_FLOORS.getChunk(style) :
		// RoomDefinition.BLANK.getChunk(style);
	}
	...
	private void entered(Player player) {
		...
		player.attackPlayerListener = (p, pTarget, message) -> {
			if (p.getCurrentRoom() != null && p.getCurrentRoom().getDefinition() == RoomDefinition.COMBAT_ROOM
					&& pTarget.getCurrentRoom() != null
					&& pTarget.getCurrentRoom().getDefinition() == RoomDefinition.COMBAT_ROOM) {
	...
	private void exited(Player p, boolean logout) {
		...
		p.getCombat().resetKillers(); // important
		p.deathEndListener = null;
		p.attackPlayerListener = null;
