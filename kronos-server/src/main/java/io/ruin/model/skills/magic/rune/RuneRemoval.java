...
import io.ruin.model.item.actions.impl.storage.DivineRunePouch;
...
public class RuneRemoval {
	...
	public static RuneRemoval get(Player player, Item... runeItems) {
		...
		if (player.getInventory().hasId(RunePouch.RUNE_POUCH)) {
			...
			if (player.getInventory().hasId(DivineRunePouch.RUNE_POUCH)) {
				for (Item item : player.getRunePouch().getItems()) {
					if (item != null)
						pItems.add(item);
				}
...
//        if(player.getInventory().hasId(GrandRunePouch.GRAND_RUNE_POUCH)) {
//            for(Item item : player.getRunePouch().getItems()) {
//                if(item != null)
//                    pItems.add(item);
//            }
