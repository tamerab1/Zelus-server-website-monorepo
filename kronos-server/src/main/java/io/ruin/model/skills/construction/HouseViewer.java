...
public class HouseViewer { // mostly just converted jordan's code to work with my house system... could use a lot of improvements...
	...
	public static int open(Player player) {
		...
		player.openInterface(ToplevelComponent.MAINMODAL, Interface.CONSTRUCTION_HOUSE_VIEWER);
		...
		player.getPacketSender().sendIfEvents(Interface.CONSTRUCTION_HOUSE_VIEWER, 5, 0, 242, 66);
		if (player.houseViewerRoom == null) {
			VarPlayerRepository.HOUSE_VIEWER_SELECTED.set(player, 0);
			VarPlayerRepository.HOUSE_VIEWER_HIGHLIGHTED.set(player, 0);
			VarPlayerRepository.HOUSE_VIEWER_ALLOW_ROTATION.set(player, 0);
			VarPlayerRepository.HOUSE_VIEWER_ROOM_ID.set(player, 0);
		}
	...
	public static void addRoom(Player player, RoomDefinition type) {
		...
		open(player);
		VarPlayerRepository.HOUSE_VIEWER_SELECTED.set(player, 0);
		VarPlayerRepository.HOUSE_VIEWER_HIGHLIGHTED.set(player, gridPos(player));
		VarPlayerRepository.HOUSE_VIEWER_ROOM_ROTATION.set(player, 0);
		VarPlayerRepository.HOUSE_VIEWER_ALLOW_ROTATION.set(player, 1);
		VarPlayerRepository.HOUSE_VIEWER_ROOM_ID.set(player, type.getId());
	}
	...
	private static void viewRoom(Player player, int index) {
		...
		VarPlayerRepository.HOUSE_VIEWER_SELECTED.set(player, index + 1);
		VarPlayerRepository.HOUSE_VIEWER_HIGHLIGHTED.set(player, 0);
		VarPlayerRepository.HOUSE_VIEWER_ROOM_ROTATION.set(player, room.getRotation());
		VarPlayerRepository.HOUSE_VIEWER_ALLOW_ROTATION.set(player, 0);
		VarPlayerRepository.HOUSE_VIEWER_ROOM_ID.set(player, room.getDefinition().getId());
	}
	...
	private static void moveRoom(Player player, int pointX, int pointY, int pointZ) {
		...
		VarPlayerRepository.HOUSE_VIEWER_HIGHLIGHTED.set(player, gridPos(pointX, pointY, pointZ));
		VarPlayerRepository.HOUSE_VIEWER_ALLOW_ROTATION.set(player, 1);
	}
	...
	private static void rotate(Player player, boolean clockwise) {
		...
		VarPlayerRepository.HOUSE_VIEWER_ROOM_ROTATION.set(player, player.houseViewerRoom.viewerRotation);
	}
	...
	private static void cancel(Player player) {
		...
		if (player.houseViewerRoom.viewerRotation != player.houseViewerRoom.getRotation()) {
			...
			VarPlayerRepository.HOUSE_VIEWER_ROOM_ROTATION.set(player, player.houseViewerRoom.getRotation());
		}
	...
	private static void done(Player player) {
		...
		if (room.viewerCost != 0) {
			...
			var tooPoor = (coins == null || coins.getAmount() < room.viewerCost) && !(World.isDev() && player.isAdmin());
			if (tooPoor) {
