...
public class FaladorCourse {
	...
	public static void register() {
		...
		ObjectAction.register(14899, "cross", (p, obj) -> p.startEvent(e -> {
			...
			p.stepAbs(3047, 3343, StepType.WALK);
			e.delay(1);
		...
		ObjectAction.register(14901, "cross", (p, obj) -> p.startEvent(e -> {
			...
			for (int i = 0; i < 3; i++) {
				p.privateSound(2459, 0, 35);
				p.animate(1118);
				p.step(0, 1, StepType.WALK);
				p.getMovement().force(0, 1, 0, 0, 34, 52, Direction.WEST);
				e.delay(2);
			}
			p.privateSound(2459, 0, 35);
			p.animate(1120);
			p.stepAbs(3050, 3357, StepType.RUN);
		...
		ObjectAction.register(14905, "cross", (p, obj) -> p.startEvent(e -> {
			...
			p.stepAbs(3033, 3361, StepType.WALK);
			e.waitForMovement(p);
			p.stepAbs(3028, 3356, StepType.WALK);
			e.waitForMovement(p);
			p.stepAbs(3028, 3354, StepType.WALK);
			e.waitForMovement(p);
			p.getAppearance().removeCustomRenders();
			e.delay(2);
		...
		ObjectAction.register(14911, "cross", (p, obj) -> p.startEvent(e -> {
			p.lock(LockType.FULL_DELAY_DAMAGE);
			p.stepAbs(3026, 3353, StepType.WALK);
			e.delay(2);
			...
			p.stepAbs(3020, 3353, StepType.WALK);
			p.resetAnimation();
		...
		ObjectAction.register(14925, "jump", (p, obj) -> p.startEvent(e -> {
			...
			p.getMovement().restoreEnergy(Random.get(1, 2));
			AgilityPet.rollForPet(p, 600);
			PerkTaskHandler.handleCompleteActivity(p, 6);
			PlayerCounter.FALADOR_ROOFTOP.increment(p, 1);
			PlayerCounter.TOTAL.increment(p, 1);
			RooftopLapCounter.notify(p, "Falador rooftop", PlayerCounter.FALADOR_ROOFTOP.get(p));
