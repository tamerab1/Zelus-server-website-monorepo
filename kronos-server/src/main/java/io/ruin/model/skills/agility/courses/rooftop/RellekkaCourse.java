...
public class RellekkaCourse {
	...
	public static void register() {
		...
		ObjectAction.register(14987, "cross", (p, obj) -> p.startEvent(e -> {
			...
			p.stepAbs(2626, 3654, StepType.WALK);
			p.stepAbs(2627, 3654, StepType.WALK);
			e.waitForMovement(p);
		...
		ObjectAction.register(14990, "leap", (p, obj) -> p.startEvent(e -> {
			...
			e.delay(1);
			p.stepAbs(2635, 3658, StepType.WALK);
			e.delay(6);
			p.getAppearance().setCustomRenders(Renders.AGILITY_BALANCE);
			p.stepAbs(2639, 3654, StepType.WALK);
			p.stepAbs(2639, 3653, StepType.WALK);
			e.delay(6);
			p.getAppearance().removeCustomRenders();
		...
		ObjectAction.register(14992, "cross", (p, obj) -> p.startEvent(e -> {
			...
			p.getAppearance().setCustomRenders(Renders.AGILITY_BALANCE);
			p.stepAbs(2647, 3663, StepType.WALK);
			e.delay(1);
			p.stepAbs(2654, 3670, StepType.WALK);
			e.delay(1);
			p.stepAbs(2655, 3670, StepType.WALK);
			e.waitForMovement(p);
			p.getAppearance().removeCustomRenders();
			e.delay(1);
		...
		ObjectAction.register(14994, "jump-in", (p, obj) -> p.startEvent(e -> {
			...
			p.getMovement().restoreEnergy(Random.get(1, 2));
			p.stepAbs(2652, 3676, StepType.WALK);
			PlayerCounter.RELLEKKA_ROOFTOP.increment(p, 1);
			PlayerCounter.TOTAL.increment(p, 1);
			RooftopLapCounter.notify(p, "Relleka rooftop", PlayerCounter.RELLEKKA_ROOFTOP.get(p));
			MarkOfGrace.rollMark(p, 80, MARK_SPAWNS);
			AgilityPet.rollForPet(p, 470);
			DailyTasks.handleTaskDecrement(p, "rellekaRooftop");
			PerkTaskHandler.handleCompleteActivity(p, 10);
			e.delay(1);
			p.unlock();
		}));
	}
