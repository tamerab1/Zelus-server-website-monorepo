...
public class VarrockCourse {
	...
	public static void register() {
		...
		ObjectAction.register(14832, "balance", (p, obj) -> p.startEvent(e -> {
			p.lock(LockType.FULL_DELAY_DAMAGE);
			p.stepAbs(3194, 3416, StepType.WALK);
			...
			p.animate(753);
			e.delay(1);
			p.stepAbs(3190, 3407, StepType.WALK);
		...
		ObjectAction.register(14841, "jump-off", (p, obj) -> p.startEvent(e -> {
			...
			PerkTaskHandler.handleCompleteActivity(p, 4);
			PlayerCounter.VARROCK_ROOFTOP.increment(p, 1);
			PlayerCounter.TOTAL.increment(p, 1);
			RooftopLapCounter.notify(p, "Varrock rooftop", PlayerCounter.VARROCK_ROOFTOP.get(p));
			DailyTasks.handleTaskDecrement(p, "varrockRooftop");
