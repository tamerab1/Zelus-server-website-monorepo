...
public enum PestleAndMortar {
	...
	CORRUPTED_SHARDS(23824, 23830, "You grind the corrupted shards to dust."),
	CRYSTAL_SHARDS(23866, 23867, "You grind the crystal shards to dust.");;
			...
			ItemItemAction.register(item.before, Tool.PESTLE_AND_MORTAR, (player, before, pestleAndMortar) -> player.startEvent(event -> {
				for (Item resource : player.getInventory().getItems()) {
					if (resource == null || resource.getId() != item.before)
						continue;
					if (item == CRYSTAL_SHARDS || item == CORRUPTED_SHARDS) {
						int shards = player.getInventory().count(item.before);
						player.integerInput("How many shards would you like to crush? (0 - " + shards + ")", amount -> {
							if (amount < 1)
								return;
							if (!player.getInventory().contains(item.before, amount)) {
								player.sendMessage("You do not have that many of this item!");
								player.closeDialogue();
								return;
							}
							player.getInventory().remove(item.before, amount);
							player.getInventory().add(item.after, amount * 10);
							player.animate(364);
						});
					} else if (item == LAVA_SCALE) {
