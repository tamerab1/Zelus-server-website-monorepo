...
public enum Mould {
	...
	//Bracelets
	GOLD_BRACELET(11065, 11068, -1, 7, 25.0, 50),
	...
	private static void craft(Player player, Mould mould, int amount) {
		...
		if (!player.getStats().check(StatType.Crafting, mould.levelReq, "make this"))
			return;
		if (mould == Mould.SLAYER_RING && VarPlayerRepository.RING_BLING.get(player) == 0) {
		...
		player.startEvent(event -> {
			...
			while (amt-- > 0) {
				...
				DailyTasks.handleItemObtained(player, mould.jewellery, StatType.Crafting);
				double xpToGain = mould.exp;
				float multiplier = 1;
				...
				if (mould == Mould.ZENYTE_AMULETS)
					player.tortureSmithed = true;
				event.delay(1);
			}
		});
	}
	...
	public static void register() {
		...
		ItemObjectAction.register(GOLD_BAR, "clay forge", (player, item, object) -> player.sendMessage("You need to use a mould on the forge in order to smelt jewelry."));
		/*
		 * Moulds
		 */
		Integer[] moulds = {1592, 1595, 1597, 11065};
		for (int id : moulds) {
			ItemObjectAction.register(id, "furnace", (player, item, obj) -> player.openInterface(ToplevelComponent.MAINMODAL, Interface.MOULD));
			ItemObjectAction.register(id, "clay forge", (player, item, obj) -> player.openInterface(ToplevelComponent.MAINMODAL, Interface.MOULD));
		}
		...
		ItemObjectAction.register(BUCKET_OF_SAND, "clay forge", (player, item, obj) -> {
			SkillDialogue.make(player, moltenGlass);
		});
		ItemObjectAction.register(SODA_ASH, "clay forge", (player, item, obj) -> {
			SkillDialogue.make(player, moltenGlass);
		});
