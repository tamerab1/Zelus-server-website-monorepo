...
public enum HouseLocation {
	EDGEVILLE(15477, 1, 10000, new Position(3079, 3496, 0)), // same as Taverly
	...
	PRIFDDINAS(34947, 70, 100000, new Position(3237, 6077, 0));

	public static interface Hook {
		record BeforeEnterFriendsHouse(Player player, Player friend) implements Hook {};
	}
	...
	public static void home(Player player, boolean buildingMode) {
		if (player.house == null) {
			player.dialogue(new MessageDialogue("You don't own a house. Talk to the estate agent to purchase one."));
			return;
		}
		player.house.buildAndEnter(player, buildingMode);
	}

	public static void friendsHouse(Player player, String friend) {
		Player other = World.getPlayer(friend);
		if (other == null) {
			player.dialogue(new MessageDialogue(
					"Player not found.<br>Make sure you typed their name correctly and that they are online."));
			return;
		}

		if (hooks.handle(new Hook.BeforeEnterFriendsHouse(player, other))) {
			return;
		}
	...
	public static void register() {
		ObjectAction.register(15478, "Home", (player, obj) -> {
			home(player, false);
		});
		ObjectAction.register(15478, "build mode", (player, obj) -> {
			home(player, true);
		});
		ObjectAction.register(15478, "friend's house", (player, obj) -> {
			player.nameInput("Enter friend's name:", name -> friendsHouse(player, name));
		});
		ObjectAction.register(15478, "Enter", (player, obj) -> {
			player.dialogue(new OptionsDialogue("Choose an option",
					new Option("Enter your house", () -> home(player, false)),
					new Option("Enter your house in building mode", () -> home(player, true)),
					new Option("Enter a friend's house", () -> {
						player.nameInput("Enter friend's name:", name -> friendsHouse(player, name));
					})));
		});
		for (HouseLocation loc : values()) {
			ObjectAction.register(loc.getPortalId(), "Home", (player, obj) -> {
				loc.home(player, false);
			});
			ObjectAction.register(loc.getPortalId(), "build mode", (player, obj) -> {
				loc.home(player, true);
			});
			ObjectAction.register(loc.getPortalId(), "friend's house", (player, obj) -> {
				player.nameInput("Enter friend's name:", name -> loc.friendsHouse(player, name));
			});
			ObjectAction.register(loc.getPortalId(), "Enter", (player, obj) -> {
				player.dialogue(new OptionsDialogue("Choose an option",
