...
public enum Teleother {
	...
	private boolean teleport(Player player, Entity target) {
		...
		if (VarPlayerRepository.ACCEPT_AID.get(target.player) == 0) {
		...
		target.player.openInterface(ToplevelComponent.MAINMODAL, Interface.TELEOTHER);
	...
	public static void register() {
		InterfaceHandler.register(Interface.TELEOTHER, h -> {
			...
			h.actions[99] = (SimpleAction) player -> player.closeInterface(ToplevelComponent.MAINMODAL);
		});
	}
