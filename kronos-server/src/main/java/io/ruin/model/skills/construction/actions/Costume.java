...
public enum Costume {
	...
	SPOTTED_CAPE(10069),
	SPOTTIER_CAPE(10071),
	MAX_CAPE(MaxCape.MaxCapes.values()),
	INFERNAL_CAPE(21295),
	...
	Costume(MaxCape.MaxCapes[] maxCapes) { // max cape
		pieces = new int[2][maxCapes.length + 1];
		pieces[0][0] = 13281;
		pieces[1][0] = 13280;
		for (int i = 0; i < maxCapes.length; i++) {
			var cape = maxCapes[i];
			pieces[0][i + 1] = cape.newHoodId;
			pieces[1][i + 1] = cape.newCapeId;
		}
	}
	...
	public void sendRequiredItems(Player player) {
		...
		for (int[] piece : pieces) {
			...
			for (int i = 0; i < piece.length; i++) {
				sb.append(Color.RED.wrap(ObjType.get(piece[i]).name));
				if (i < piece.length - 1)
					sb.append(" or ");
			}
