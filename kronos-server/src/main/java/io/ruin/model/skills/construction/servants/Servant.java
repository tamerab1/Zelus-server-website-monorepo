...
public class Servant extends NPC {
	...
	private enum State {
		IDLE, GREETING, FOLLOWING, FETCHING
	}
	...
	private void itemUsed(Player player, Item item) {
		...
		if (canUnnote(item)) {
			options
					.add(new Option("Un-note it", () -> player.integerInput("Enter amount:", amt -> unnote(player, item, amt))));
		}
		Plank plankType = Plank.getFromLog(item.getId());
		...
		options.add(new Option("Deposit it in bank",
				() -> player.integerInput("Enter amount to deposit:", amount -> deposit(player, item.getId(), amount))));
		if (options.size() == 1)
	...
	private void inventoryFullDialogue(Player player) {
		...
		player.dialogue(new NPCDialogue(getId(), getDefinition().getInventoryFullLine(amount)),
				new OptionsDialogue("Choose an option",
						new Option("Hold on to them for now",
								() -> player.dialogue(new NPCDialogue(getId(), getDefinition().getAgreeLine()))),
						new Option("Take them to the bank",
								() -> player.dialogue(new NPCDialogue(getId(), getDefinition().getAgreeLine()),
										new ActionDialogue(this::depositInventory)))));
	}
	...
	private void interact(Player player) {
		...
		if (save.getLastAction() != null) { // give option to repeat last task with only 1 click
			player.dialogue(new OptionsDialogue("Choose an option",
					new Option(
							"Repeat last task: "
									+ save.getLastAction().toString(save.getLastActionItemId(), save.getLastActionItemAmount()),
							this::repeatLast),
	...
	private void manualPayment(Player player) {
		if (player.getInventory().contains(COINS_995, getDefinition().getPayment())) {
			...
			player.dialogue(new MessageDialogue("You don't have enough coins. Your servant requires "
					+ NumberUtils.formatNumber(getDefinition().getPayment()) + " coins as payment."));
		}
	}
	...
	private boolean takePaymentFromMoneybag() {
		if (house.hasServantsMoneybag() && house.getMoneyInMoneybag() >= getDefinition().getPayment()) {
			...
			if (house.getMoneyInMoneybag() > getDefinition().getPayment())
				house.getOwner().sendMessage("Your servant has taken their payment from the moneybag. The moneybag has "
						+ NumberUtils.formatNumber(house.getMoneyInMoneybag()) + " coins left in it.");
			else if (house.getMoneyInMoneybag() > 0)
				house.getOwner()
						.sendMessage("<col=ff0000>Your servant has taken their payment from the moneybag. The moneybag has "
								+ NumberUtils.formatNumber(house.getMoneyInMoneybag())
								+ " coins left in it, which will not be enough for the servant's next payment.");
			else
				house.getOwner().sendMessage(
						"<col=ff0000>Your servant has taken their payment from the moneybag. The moneybag is now empty.");
			save.resetActions();
			return true;
		}
		return false;
	...
	private void makePlanks(Player player, int logs, int amount) {
		Plank plank = Plank.getFromLog(logs);
		if (plank == null)
			return;
		...
		int finalAmount = Math.min(Math.min(player.getInventory().getAmount(plank.woodId),
				player.getInventory().getAmount(COINS_995) / plank.cost), amount);
		if (finalAmount <= 0)
			return;
	...
	private static final List<Material> BANK_ITEMS = Arrays.asList(Material.REGULAR_PLANK, Material.OAK_PLANK,
			Material.TEAK_PLANK, Material.MAHOGANY_PLANK, Material.SOFT_CLAY, Material.LIMESTONE_BRICK, Material.STEEL_BAR,
			Material.BOLT_OF_CLOTH, Material.GOLD_LEAF, Material.MARBLE_BLOCK, Material.MAGIC_STONE);

	private void configWithdraw(Player player) {
		startFollowing();
		OptionScroll.open(player, "Choose a material", true,
				BANK_ITEMS.stream().map(mat -> new Option(ObjType.get(mat.getItemId()).name, () -> {
	...
	private void unnote(Player player, Item item, int amount) {
		if (amount <= 0 || !canUnnote(item))
			return;
		int finalAmount =
				Math.min(getDefinition().getItemCapacity(), Math.min(player.getInventory().getAmount(item.getId()), amount));
