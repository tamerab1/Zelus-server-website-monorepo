...
public class Hall {
	...
	private static void upAction(Player player, Room room, GameObject obj) {
		...
		if (destination == null) {
			...
			GameObject above = Tile.getObject(-1, obj.x, obj.y, obj.z + 1, 10, obj.getDirection());
			if (room.getRoomAbove() == null || room.getRoomAbove().getRotation() != room.getRotation()
				...
				|| (room.getRoomAbove().getDefinition() != RoomDefinition.GARDEN && (above == null || above.isRemoved() || (above.getId() != obj.getId() + 1 && above.getId() != obj.getId())))) {
			...
			if (obj.getId() == 13505 || obj.getId() == 13503)
	...
	private static void downAction(Player player, Room room, GameObject obj) {
		...
		if (destination == null) {
			...
			GameObject below = Tile.getObject(-1, obj.x, obj.y, obj.z - 1, 10, obj.getDirection());
			if (room.getRoomBelow() == null || room.getRoomBelow().getRotation() != room.getRotation()
				...
				|| (room.getRoomBelow().getDefinition() != RoomDefinition.GARDEN && (below == null || below.isRemoved() || (below.getId() != obj.getId() - 1 && below.getId() != obj.getId())))) {
