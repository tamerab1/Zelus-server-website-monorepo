...
public enum DiningBench implements Seat {
	...
	private void sit(Player player, GameObject object) {
		...
		player.startEvent(event -> {
			player.lock();
			Direction step = object.getDirection() == 1 || object.getDirection() == 3 ? Direction.NORTH : Direction.WEST;
			player.step(step.deltaX, step.deltaY, StepType.WALK);
			event.delay(1);
			if (object.getId() != benchId) { // someone else sat on it maybe
				player.unlock();
				return;
			}
			player.seat = this;
			GameObject temp = GameObject.spawn(tempId, pullTo.getX(), pullTo.getY(), pullTo.getZ(), 10, object.getDirection());
			...
			player.getMovement().force(0, 0, dir.deltaX, dir.deltaY, 25, 5, Direction.getFromObjectDirection(object.getDirection()));
	...
	public void stand(Player player) {
		...
		else if (obj.getDirection() == 1)
			step = Direction.NORTH_EAST;
		else if (obj.getDirection() == 2)
			step = Direction.SOUTH_WEST;
		else
			step = Direction.NORTH_WEST;
		player.seat = null;
		player.animate(Construction.STAND_UP);
		player.getMovement().force(0, 0, step.deltaX, step.deltaY, 25, 5, Direction.getFromObjectDirection(obj.getDirection()));
		obj.setId(benchId);
	}
