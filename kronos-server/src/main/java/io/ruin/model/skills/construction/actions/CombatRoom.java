...
public class CombatRoom {
	...
	public static void register() { // mark room template as multi
		...
		for (Dummy dummy : CombatRoom.Dummy.values()) {
			ObjectAction.register(dummy.objId, "attach", (player, obj) -> {
				House house = player.getCurrentHouse();
				if (house == null || obj.dummyRemoved()) {
					return;
				}
				obj.dummyRemoved(true);
				NPC npc = new NPC(dummy.npcId);
				house.addNPC(npc.spawn(obj.x, obj.y, obj.z, Direction.getFromObjectDirection(obj.getDirection()), 0));
				World.startEvent(event -> { // since the npc will only become visible to the player in the next tick, wait 1 tick to remove the object too
					event.delay(1);
					obj.remove();
					obj.remove(RsAttribute.dummyRemoved);
				});
	...
	public static boolean allowEquip(Player player, ObjType selectedDef) {
	...
	enum Dummy {
		...
		UNDEAD(Buildable.UNDEAD_COMBAT_DUMMY, 10507);
