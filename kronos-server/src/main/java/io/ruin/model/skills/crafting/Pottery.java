...
public enum Pottery {
	...
	Pottery(int unfired, int fired, int levelReq, double unfiredExp, double firedExp) {
		...
		this.name = ObjType.get(fired).name.toLowerCase();
	}
	...
	private static final int[] POTTERY_OVENS = {14888, 11601, 21302};
	...
	public static void register() {
		...
		ItemObjectAction.register(SOFT_CLAY, POTTERS_WHEEL, (player, item, obj) -> SkillDialogue.make(player,
			new SkillItem(POT.unfired).name(ObjType.get(POT.fired).name).addAction((p, amount, event) -> spinClay(p, POT, amount)),
			new SkillItem(PIE_DISH.unfired).name(ObjType.get(PIE_DISH.fired).name).addAction((p, amount, event) -> spinClay(p, PIE_DISH, amount)),
			new SkillItem(BOWL.unfired).name(ObjType.get(BOWL.fired).name).addAction((p, amount, event) -> spinClay(p, BOWL, amount)),
			new SkillItem(PLANT_POT.unfired).name(ObjType.get(PLANT_POT.fired).name).addAction((p, amount, event) -> spinClay(p, PLANT_POT, amount))));
		...
		for (int oven : POTTERY_OVENS) {
			ObjectAction.register(oven, "fire", (player, obj) -> SkillDialogue.make(player,
				new SkillItem(POT.unfired).name(ObjType.get(POT.fired).name).addAction((p, amount, event) -> firePottery(p, POT, amount)),
				new SkillItem(PIE_DISH.unfired).name(ObjType.get(PIE_DISH.fired).name).addAction((p, amount, event) -> firePottery(p, PIE_DISH, amount)),
				new SkillItem(BOWL.unfired).name(ObjType.get(BOWL.fired).name).addAction((p, amount, event) -> firePottery(p, BOWL, amount)),
				new SkillItem(PLANT_POT.unfired).name(ObjType.get(PLANT_POT.fired).name).addAction((p, amount, event) -> firePottery(p, PLANT_POT, amount))));
