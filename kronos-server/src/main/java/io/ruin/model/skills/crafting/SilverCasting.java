...
public enum SilverCasting {
		...
		if (rightSide) {
			...
			player.startEvent(event -> {
				...
				while (amt-- > 0) {
					...
					DailyTasks.handleItemObtained(player, silverCasting.result, StatType.Crafting);
					player.getStats().addXp(StatType.Crafting, silverCasting.exp, true);
					event.delay(1);
				}
			});
	...
	public static void register() {
		ItemObjectAction.register(SILVER_BAR, "furnace", (player, item, object) -> {
			player.openInterface(ToplevelComponent.MAINMODAL, Interface.SILVER_CASTING);
			player.getPacketSender().sendIfEvents(Interface.SILVER_CASTING, 4, 0, 11, 62);
			player.getPacketSender().sendIfEvents(Interface.SILVER_CASTING, 6, 0, 7, 62);
		});

		ItemObjectAction.register(SILVER_BAR, "clay forge", (player, item, object) -> {
			player.openInterface(ToplevelComponent.MAINMODAL, Interface.SILVER_CASTING);
			player.getPacketSender().sendIfEvents(Interface.SILVER_CASTING, 4, 0, 11, 62);
			player.getPacketSender().sendIfEvents(Interface.SILVER_CASTING, 6, 0, 7, 62);
		});
