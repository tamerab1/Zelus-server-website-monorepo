...
public class PortalChamberRoom extends Room {
	...
	public enum PortalDestination {
		...
		public static final PortalDestination[] VALUES = values();
		int levelReq;
		Item[] runes;
	...
	public static void find() {
		for (PortalDestination pd : PortalDestination.VALUES) {
			...
			List<LocType> types = LocType.getByName(nameSearch);
			if (types != null) {
				for (LocType def : types) {
					if (def.modelIds == null) continue;
	...
	private void scry(Player player, int portalIndex) {
		...
		player.addEvent(event -> {
			...
			player.openInterface(ToplevelComponent.OVERLAY, Interface.SCRYING_POOL);
			player.lock();
			event.delay(1);
