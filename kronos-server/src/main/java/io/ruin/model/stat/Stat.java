...
public class Stat {
	...
	public static final double MAX_XP = Integer.MAX_VALUE;
	...
	protected int boostedFor, depletedFor;
	...
	public int alter(int newLevel) {
		if (newLevel < 0)
			Server.logError("",
				new Throwable("Not a real error, just want to see where newLevel (" + newLevel + ") is set to less than 0!"));
	...
	public void process(boolean hitpoints, boolean rapidRestore, boolean rapidHeal, boolean preserve,
		...
		if (currentLevel > fixedLevel) {
			...
			if (hitpoints ? rapidHeal : rapidRestore)
				depleteTime /= 2;
			if (hitpoints && eternalRegenLevel > 0) {
				depleteTime /= eternalRegenLevel;
			}
