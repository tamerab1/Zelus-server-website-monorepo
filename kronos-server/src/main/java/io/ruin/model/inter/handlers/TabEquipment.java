	...
	public static void register() {
		InterfaceHandler.register(Interface.EQUIPMENT, h -> {
			...
			h.actions[26] = (SimpleAction) p -> {
				p.openInterface(ToplevelComponent.MAINMODAL, 719);
			};
			h.actions[28] = (OptionAction) (player, option) -> {
				if (option == 1)
					itemAction(player, option, Equipment.SLOT_QUIVER);
//				if (option == 2)
//					DizanaQuiver.fillQuiver(player);
			};
	...
	public static void itemAction(Player player, int option, int slot) {
		if (player.isLocked())
			return;
		if (slot == Equipment.SLOT_QUIVER && VarPlayerRepository.DIZANA_QUIVER_AMMO_ID.get(player) > 0) {
			var cape = player.getEquipment().get(Equipment.SLOT_CAPE);
			DizanaQuiver.emptyQuiver(player, cape);
		}
