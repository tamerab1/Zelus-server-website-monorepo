...
//        }, (SimpleAction) player -> player.openUrl("https://KROTAps.uk/")), //need to hookup tfa to the website somehow
...
//        COMPONENT_21(21, player -> "Discord", (SimpleAction) player -> player.openUrl("https://discord.gg/sVrcatBdyG")),
       ...
       /* InterfaceHandler.register(Interface.QUEST_TAB, h ->
        {
            ...
            h.actions[3] = (SimpleAction) p -> {
                p.getPacketSender().removeSubInterface(629, 33);
                p.getPacketSender().sendToplevelSubInterface(712, 33, ToplevelComponent.QUEST_TAB_AREA);
                VarPlayerRepository.QUEST_ACTIVE_TAB.set(p, 0);
            };
            h.actions[8] = (SimpleAction) p -> {
                p.getPacketSender().removeSubInterface(629, 33);
                p.getPacketSender().sendToplevelSubInterface(399, 33, ToplevelComponent.QUEST_TAB_AREA);
                VarPlayerRepository.QUEST_ACTIVE_TAB.set(p, 1);
            };
            h.actions[13] = (SimpleAction) p -> {
                p.getPacketSender().removeSubInterface(629, 33);
                p.getPacketSender().sendToplevelSubInterface(259, 33, ToplevelComponent.QUEST_TAB_AREA);
                VarPlayerRepository.QUEST_ACTIVE_TAB.set(p, 2);
            };

            h.actions[18] = (SimpleAction) p -> {
                p.getPacketSender().removeSubInterface(629, 33);
                p.getPacketSender().sendToplevelSubInterface(245, 33, ToplevelComponent.QUEST_TAB_AREA);
                VarPlayerRepository.QUEST_ACTIVE_TAB.set(p, 3);
            };
