...
public class Preset extends QuestTabEntry {
	...
	public static void register() {
		io.ruin.model.entity.shared.listeners.LoginListener.register(player -> PresetCustom.load(player));

		// Mark preset items as free so they cannot be sold to shops or alchemized
		int[] presetItems = new int[]{
			...
			RUNE_FULL_HELM, MYSTIC_HAT_DARK, DRAGON_DAGGER, BERSERKER_HELM, MYSTIC_BOOTS_DARK, RUNE_PLATESKIRT
		};
		for (int presetItem : presetItems) {
			ObjType.get(presetItem).free = true;
			//ItemDef.get(presetItem).neverProtect = true;
			int notedId = ObjType.get(presetItem).notedId;
			if (ObjType.get(notedId) != null) {
				ObjType.get(notedId).free = true;
	...
	protected transient boolean free;
	protected String name;
	protected int[] attack, strength, defence, ranged, prayer, magic, hitpoints;
	protected SpellBook spellBook;
	protected Item[] invItems = new Item[28], equipItems = new Item[14];
	protected Item[] runePouchItems = new Item[3];
	...
	public final boolean allowSelect(Player player) {
        ...
        /*if (TournamentManager.Lobby.contains(player) || player.tournament != null) {
            ...
            return false;//TODO OLEM UNCOMMENT
        }*/
		if (player.isLocked()) {
...
/*        if (player.pvpInstancePosition != null && !player.pvpAttackZone) {
	...
	@Override
	public final void select(Player player) {
	...
	public void selectFinish(Player player) {
		...
		boolean equipBanked = bankItems(player, false);
		if (!equipBanked) {
			if (!free) {
			...
			// Free preset: force-clear equipment that couldn't be banked
		...
		Nurse.heal(player, null);//TODO OLEM UNCOMMENT
		//player.lastPresetUsed = this;//TODO OLEM UNCOMMENT
	...
	private void withdrawItems(Player player, boolean inventory) {
		...
		for (int i = 0; i < items.length; i++) {
			if (container.get(i) != null) {
				if (free) {
					container.set(i, null); // force-clear for free presets
				} else {
					continue; // item failed to bank, skip
				}
			}
			...
			ObjType presetDef = presetItem.getDef();
...
//            if (presetItem.getId() == GrandRunePouch.GRAND_RUNE_POUCH) {
//                if(GrandrunePouchItems != null) {
//                    for (Item rune : GrandrunePouchItems) {
//                        if (rune != null) {
//                            BankItem runeItem = player.getBank().findItem(rune.getId());
//                            if (runeItem == null)
//                                continue;
//                            player.getGrandRunePouch().deposit(runeItem, rune.getAmount());
//                        }
//                    }
//                }
//            }
