...
public class RiskProtection extends JournalToggle {
	...
	public static interface Hook {
		int getKeepCount(boolean skulled, boolean ultimateIronMan, boolean highRiskSkull);

		ArrayList<Item> itemsSortedByProtectValue(Player player);
	}
			...
			new Option("Toggle Risk Protection", () -> {
				...
				if (player.riskProtectionExpirationDelay.isDelayed()) {
					if (delay >= 100) {
						...
						player.dialogue(new ItemDialogue().one(8792,
							"You can toggle your risk protection in " + minutes + " minute" + (minutes == 1 ? "" : "s") + "."));
					} else {
						int seconds = delay / 10 * 6;
						player.dialogue(new ItemDialogue().one(8792,
							"You can toggle your risk protection in " + seconds + " second" + (seconds == 1 ? "" : "s") + "."));
					}
					return;
				}
	...
	private static void toggleRiskProtection(Player player, int tier) {
		...
		player.sendMessage(Color.DARK_RED.wrap(
			"You now have a risk protection value up to " + NumberUtils.formatNumber(protectionValue(player)) + " Coins."));
	...
	private String protectionState(Player player) {
		...
		if (tier == 0)
			...
			return NumberUtils.formatNumber(protectionValue(player)) + "+ Coins";
	}
