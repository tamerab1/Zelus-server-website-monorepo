...
public class UpgradeMachine {
	...
	public void sendInterface() {
		player.openInterface(ToplevelComponent.MAINMODAL, Interface.UPGRADE_MACHINE);
		player.openInterface(ToplevelComponent.SKILLS_TAB_AREA, Interface.UPGRADE_MACHINE_INVENTORY);
		player.getPacketSender().sendIfEvents(Interface.UPGRADE_MACHINE_INVENTORY, 3, 0, 27, 1181694);
		player.getPacketSender().sendIfEvents(Interface.UPGRADE_MACHINE_INVENTORY, 10, 0, 27, 1054);
	...
	private static boolean canSelect(ObjType def) {
