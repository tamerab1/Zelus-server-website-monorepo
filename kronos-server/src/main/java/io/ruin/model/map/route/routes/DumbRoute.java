...
public class DumbRoute {
	...
	public static boolean step(Entity entity, int destX, int destY) {
		if (entity.isMovementBlocked(true, false)) {
			return false;
		}

		Direction stepDir = getStepDirection(entity.getRouteFinder().getClipUtils(), entity.getAbsX(), entity.getAbsY(),
				entity.getHeight(), entity.getSize(), destX, destY);
		// varvordis
		if (entity.isNpc() && entity.npc.getId() == 12223) {
			entity.getRouteFinder().routeAbsolute(destX, destY);
			return true;
		} else if (stepDir != null) {
			var dumbNext = entity.getPosition().translated(stepDir.deltaX, stepDir.deltaY);
			var stepAdded = entity.addStep(dumbNext.getX(), dumbNext.getY());
			return stepAdded;
		}
		return false;
	}
	...
	public static void step(Entity entity, Entity target, int distance) {
		/**
		 * Use the route finder to find the DESTINATION x/y to walk to.. I know.. bad..
		 */
		RouteFinder rf = entity.getRouteFinder();
		rf.customClipUtils = ClipUtils.NONE; // none (no clipping!) is the EQUIVILENT of dumb path finding. it'll paint a
																					// straight line, and obviously when attempting to take each step will get
																					// stuck on any obstacles in the way
		rf.routeEntity(target);
		rf.customClipUtils = null;
		entity.getMovement().reset(); // clear the pathing, we're not going to use A*, we're just using the results of
																	// the calculation
		// todo - Figure out a correct way to calc the destX/destY because this doesn't
	...
	public static boolean withinDistance(Entity entity, Entity target, int distance) {
		...
		if (entity.getSize() > 1) {
			source = Misc.getClosestPosition(entity, target); // if this is a "big" entity, we need to check from the closest
		...
		if (distance == 1) {
			var routeFinished = entity.getRouteFinder().routeEntity == null
					|| entity.getRouteFinder().routeEntity.finished(entity.getPosition());
			return routeFinished
					&& getDirection(source.getX(), source.getY(), entity.getHeight(), 1, target.getAbsX(),
							target.getAbsY()) != null;
		} else {
	...
	private static boolean allowEntrance(ClipUtils clipUtils, int x, int y, int z, int size, Direction dir) {
		...
		if (size == 1) {
			...
			if (dx == -1 && dy == -1 && (clipUtils.getAbs(x - 1, y - 1, z) & SOUTH_WEST_MASK) == 0
					&& (clipUtils.getAbs(x - 1, y, z) & WEST_MASK) == 0 && (clipUtils.getAbs(x, y - 1, z) & SOUTH_MASK) == 0)
				return true;
			if (dx == 1 && dy == -1 && (clipUtils.getAbs(x + 1, y - 1, z) & SOUTH_EAST_MASK) == 0
					&& (clipUtils.getAbs(x + 1, y, z) & EAST_MASK) == 0 && (clipUtils.getAbs(x, y - 1, z) & SOUTH_MASK) == 0)
				return true;
			if (dx == -1 && dy == 1 && (clipUtils.getAbs(x - 1, y + 1, z) & NORTH_WEST_MASK) == 0
					&& (clipUtils.getAbs(x - 1, y, z) & WEST_MASK) == 0 && (clipUtils.getAbs(x, y + 1, z) & NORTH_MASK) == 0)
				return true;
			if (dx == 1 && dy == 1 && (clipUtils.getAbs(x + 1, y + 1, z) & NORTH_EAST_MASK) == 0
					&& (clipUtils.getAbs(x + 1, y, z) & EAST_MASK) == 0 && (clipUtils.getAbs(x, y + 1, z) & NORTH_MASK) == 0)
				return true;
		} else if (size == 2) {
			if (dx == -1 && dy == 0 && (clipUtils.getAbs(x - 1, y, z) & SOUTH_WEST_MASK) == 0
					&& (clipUtils.getAbs(x - 1, y + 1, z) & NORTH_WEST_MASK) == 0)
				return true;
			if (dx == 1 && dy == 0 && (clipUtils.getAbs(x + 2, y, z) & SOUTH_EAST_MASK) == 0
					&& (clipUtils.getAbs(x + 2, y + 1, z) & NORTH_EAST_MASK) == 0)
				return true;
			if (dx == 0 && dy == -1 && (clipUtils.getAbs(x, y - 1, z) & SOUTH_WEST_MASK) == 0
					&& (clipUtils.getAbs(x + 1, y - 1, z) & SOUTH_EAST_MASK) == 0)
				return true;
			if (dx == 0 && dy == 1 && (clipUtils.getAbs(x, y + 2, z) & NORTH_WEST_MASK) == 0
					&& (clipUtils.getAbs(x + 1, y + 2, z) & NORTH_EAST_MASK) == 0)
				return true;
			if (dx == -1 && dy == -1 && (clipUtils.getAbs(x - 1, y, z) & 0x124013e) == 0
					&& (clipUtils.getAbs(x - 1, y - 1, z) & SOUTH_WEST_MASK) == 0
					&& (clipUtils.getAbs(x, y - 1, z) & 0x124018f) == 0)
				return true;
			if (dx == 1 && dy == -1 && (clipUtils.getAbs(x + 1, y - 1, z) & 0x124018f) == 0
					&& (clipUtils.getAbs(x + 2, y - 1, z) & SOUTH_EAST_MASK) == 0
					&& (clipUtils.getAbs(x + 2, y, z) & 0x12401e3) == 0)
				return true;
			if (dx == -1 && dy == 1 && (clipUtils.getAbs(x - 1, y + 1, z) & 0x124013e) == 0
					&& (clipUtils.getAbs(x - 1, y + 2, z) & NORTH_WEST_MASK) == 0
					&& (clipUtils.getAbs(x, y + 2, z) & 0x12401f8) == 0)
				return true;
			if (dx == 1 && dy == 1 && (clipUtils.getAbs(x + 1, y + 2, z) & 0x12401f8) == 0
					&& (clipUtils.getAbs(x + 2, y + 2, z) & NORTH_EAST_MASK) == 0
					&& (clipUtils.getAbs(x + 2, y + 1, z) & 0x12401e3) == 0)
				return true;
		} else {
			if (dx == -1 && dy == 0 && (clipUtils.getAbs(x - 1, y, z) & SOUTH_WEST_MASK) == 0
			...
			if (dx == 1 && dy == 0 && (clipUtils.getAbs(x + size, y, z) & SOUTH_EAST_MASK) == 0
			...
			if (dx == 0 && dy == 1 && (clipUtils.getAbs(x, y + size, z) & NORTH_WEST_MASK) == 0
			...
			if (dx == 1 && dy == -1 && (clipUtils.getAbs(x + size, y - 1, z) & SOUTH_EAST_MASK) == 0) {
				for (int i = 1; i < size - 1; i++) {
					if ((clipUtils.getAbs(size + x, y - 1 + i, z) & 0x12401e3) != 0
							|| (clipUtils.getAbs(i + x, y - 1, z) & 0x124018f) != 0)
						return false;
				}
				return true;
			}
