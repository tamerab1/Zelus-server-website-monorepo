...
//        ObjectAction.register(4469, 1357, 10206, 1, 1, Karuulm::handleBarrier);//TODO "Error loading object 4469, at (1357, 10206, 1)"
//        ObjectAction.register(4470, 1357, 10207, 1, 1, Karuulm::handleBarrier);//TODO "Error loading object 4470, at (1357, 10207, 1)"
	...
	private static void onEnter(Player player) {
		player.attackNpcListener = (player1, npc, message) -> {
			if(npc.getId() == 17022 || npc.getId() == 17021 && npc.ownerId == player.getUserId())
				return true;
	...
	private static boolean isProtectedFromBurn(Player player) {
		return player.getEquipment().hasId(23037) // boots of stone
			...
			|| player.getEquipment().hasId(21643)
			|| player.getTotalDonated() >= 100; // granite boots
