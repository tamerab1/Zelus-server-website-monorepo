...
//                    SnowballPoints.INSTANCE.send(player);
	...
	public static void register() {
		...
		ObjType.get(SNOWBALL).addOnTickEvent((player, item) -> {
		...
		if (World.christmas) {
			...
			for (Position snowPosition : SNOW_POSITIONS)
				MapListener.registerPosition(snowPosition)
					.onEnter(player -> player.openInterface(ToplevelComponent.OVERLAY, 204))
					.onExit((player, logout) -> player.closeInterface(ToplevelComponent.OVERLAY));
