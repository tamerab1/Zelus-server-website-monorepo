...
public enum CanoeStation {
	...
	LUMBRIDGE(12163, VarPlayerRepository.LUMBRIDGE_CANOE,
			new int[] { 32, 11 },
			...
			12, 50),
	CHAMPION_GUILD(12164, VarPlayerRepository.CHAMPION_GUILD_CANOE,
			new int[] { 13, 33 },
			...
			12, 47),
	BARBARIAN_VILLAGE(12165, VarPlayerRepository.BARBARIAN_VILLAGE_CANOE,
			new int[] { 34, 15 },
			...
			12, 44),
	EDGEVILLE(12166, VarPlayerRepository.EDGEVILLE_CANOE,
			new int[] { 38, 39 },
			...
			12, 36),
	WILDERNESS_CHINS(-1, VarPlayerRepository.WILDERNESS_CHINS_CANOE,
			new int[] { 35 },
	...
	CanoeStation(int objectId, VarPlayerRepository config, int[] travelButtonIds, Position endPosition, String endMessage,
			Position chopPosition, Position shapePosition,
	...
	private static final int TREE_FALLING_ANIMATION = 3304,
	...
	private static void shapeBoat(Player player, int boatType) {
		...
		player.startEvent(event -> {
			...
			player.face(player.getAbsX() + canoeStation.faceBoatPosition.getX(),
					player.getAbsY() + canoeStation.faceBoatPosition.getY());
			int loops = 10;
			while (loops-- > 0) {
				event.delay(1);
	...
	private static void travelToDestination(Player player, CanoeStation canoeStation) {
		...
		player.startEvent(event -> {
			...
			try {
				dynamicMap = new DynamicMap().buildSw(7238, 1);
			...
			player.currentDynamicMap = dynamicMap;
			...
			player.getPacketSender().moveCameraToLocation(dynamicMap.convertX(1814), dynamicMap.convertY(4518), 255, 100,
					100);
			player.getPacketSender().turnCameraToLocation(dynamicMap.convertX(1817), dynamicMap.convertY(4518), 255, 100,
					100);
			...
			player.sendFilteredMessage(
					"You arrive at the Barbarian Village.<br>Your canoe sinks into the water after the hard journey.");
			World.startEvent(e -> {
				GameObject canoe = GameObject.spawn(sinkId, canoeStation.sinkPosition.getX(), canoeStation.sinkPosition.getY(),
						0, 10, 1);
				event.delay(2);
				canoe.remove();
			});
	...
	private static void station(Player player, GameObject obj, CanoeStation canoeStation) {
		if (canoeStation.config.get(player) == BEGINNING_STATE) {
			player.startEvent(event -> {
				Position chopPosition = new Position(obj.x + canoeStation.chopPosition.getX(),
						obj.y + canoeStation.chopPosition.getY(), 0);
				player.lock();
				if (!player.isAt(chopPosition)) {
					player.stepAbs(chopPosition.getX(), chopPosition.getY(), StepType.WALK);
				...
				if (player.getStats().get(StatType.Woodcutting).currentLevel < canoeStation.levelReq) {
					player.dialogue(new MessageDialogue(
							"You must have at least level " + canoeStation.levelReq + " Woodcutting to start making canoes."));
					player.unlock();
					return;
				}
				Hatchet hatchet = Hatchet.find(player);
			...
			player.startEvent(event -> {
				...
				Position shapePosition = new Position(obj.x + canoeStation.shapePosition.getX(),
						obj.y + canoeStation.shapePosition.getY(), 0);
				if (!player.isAt(shapePosition)) {
					player.stepAbs(shapePosition.getX(), shapePosition.getY(), StepType.WALK);
					event.waitForMovement(player);
				}
				player.face(obj.x + canoeStation.faceBoatPosition.getX(), obj.y + canoeStation.faceBoatPosition.getY());
				event.delay(1);
				...
				player.openInterface(ToplevelComponent.MAINMODAL, Interface.CANOE_LOCATION);
	...
	public static void register() {
		...
		// TODO - changed in rev182 probably new ID - dunno what it is
		/*
