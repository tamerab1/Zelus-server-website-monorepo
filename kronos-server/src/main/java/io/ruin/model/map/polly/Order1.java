...
public final class Order1 extends Curve {
    ...
    public int compareTo(Curve other, double[] yrange) {
        if (!(other instanceof Order1 c1)) return super.compareTo(other, yrange);
        ...
        if (xmax <= c1.xmin) return (xmin == c1.xmax) ? 0 : -1;
		...
		if (denom != 0) {
            double num = ((x0 - c1.x0) * dya * dyb
                          ...
                          + c1.y0 * dxb * dya);
            y = num / denom;
			if (y <= yrange[0]) y = Math.min(y1, c1.y1);
			else {
				if (y < yrange[1]) yrange[1] = y;
                y = Math.max(y0, c1.y0);
            }
        }
		else y = Math.max(y0, c1.y0);
