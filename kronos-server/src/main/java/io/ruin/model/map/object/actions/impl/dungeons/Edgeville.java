			...
			if (player.getInventory().hasId(983)) {
				player.startEvent(event -> {
					...
					if (player.getAbsX() != obj.x || player.getAbsY() != obj.y + (obj.y > player.getAbsY() ? -1 : 0)) {
						player.stepAbs(obj.x, obj.y + (obj.y > player.getAbsY() ? -1 : 0), StepType.WALK);
						event.delay(1);
					}
					GameObject opened = GameObject.spawn(11728, 3115, 3449, 0, obj.getType(), 0);
					obj.skipClipping(true).remove();
					player.stepAbs(player.getAbsX(), obj.y + (obj.y > player.getAbsY() ? 0 : -1), StepType.WALK);
