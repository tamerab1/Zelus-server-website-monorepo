...
public class Bounds {
	...
	public int getDiagonalSize() {
		return (int) Math.sqrt(Math.pow(neX - swX, 2) + Math.pow(neY - swY, 2));
	}
	...
	public boolean inBounds(Position position, int range) {
		return inBounds(position.getX(), position.getY(), position.getZ(), range);
	}
	...
	public boolean inBoundsWithoutPlane(int x, int y, int range) {
		return x >= swX - range && x <= neX + range && y >= swY - range && y <= neY + range;
	}
	...
	public Position randomPositionWithClipping() {
		Position pos = null;
		int MAX_ATTEMPTS = 50;
		int attempts = 0;
		while (pos == null) {
			Position position = randomPosition();
			if (Tile.get(position, true).clipping == 0) {
				pos = position;
			}
			attempts++;
	...
	public Position getMiddlePosition() {
		return new Position((swX + neX) / 2, (swY + neY) / 2, z);
	}

	public static Position clamp(Position position, Bounds bounds) {
		int x = Math.max(bounds.swX, Math.min(bounds.neX, position.getX()));
		int y = Math.max(bounds.swY, Math.min(bounds.neY, position.getY()));
		int z = position.getZ(); // Keep the same z-coordinate
