...
public class ClipUtils {
	...
	public static final ClipUtils SWIM = new ClipUtils() {
		...
		public int getAbs(int absX, int absY, int z) {
			...
			if (region.empty)
				return UNMOVABLE_MASK;
			Tile tile = region.getTile(absX, absY, z, false);
			return (tile == null || tile.clipping == 0 || (tile.clipping & UNMOVABLE_MASK) == 0) ? UNMOVABLE_MASK : (tile.clipping & ~UNMOVABLE_MASK);
		}
	};
	...
	public boolean method3065(int startX, int startY, int size, int checkX, int checkY, int type, int direction) {
		...
		if (size == 1) {
			if (type == 0) {
				if (direction == 0) {
					if (checkX - 1 == startX && startY == checkY)
						return true;
					if (startX == checkX && 1 + checkY == startY && 0 == (get(startX, startY) & NORTH_MASK))
						return true;
					if (startX == checkX && checkY - 1 == startY && 0 == (get(startX, startY) & SOUTH_MASK))
						return true;
				} else if (direction == 1) {
					if (checkX == startX && 1 + checkY == startY)
						return true;
					if (checkX - 1 == startX && checkY == startY && (get(startX, startY) & WEST_MASK) == 0)
						return true;
					if (checkX + 1 == startX && startY == checkY && 0 == (get(startX, startY) & EAST_MASK))
						return true;
				} else if (direction == 2) {
					if (startX == checkX + 1 && checkY == startY)
						return true;
					if (checkX == startX && 1 + checkY == startY && 0 == (get(startX, startY) & NORTH_MASK))
						return true;
					if (checkX == startX && checkY - 1 == startY && (get(startX, startY) & SOUTH_MASK) == 0)
						return true;
				} else if (3 == direction) {
					if (startX == checkX && startY == checkY - 1)
						return true;
					if (startX == checkX - 1 && checkY == startY && (get(startX, startY) & WEST_MASK) == 0)
						return true;
					if (startX == 1 + checkX && startY == checkY && (get(startX, startY) & EAST_MASK) == 0)
						return true;
				}
			}
			if (2 == type) {
				if (0 == direction) {
					...
					if (checkX == startX && startY == 1 + checkY)
						return true;
					if (checkX + 1 == startX && checkY == startY && (get(startX, startY) & EAST_MASK) == 0)
						return true;
					if (startX == checkX && startY == checkY - 1 && 0 == (get(startX, startY) & SOUTH_MASK))
						return true;
				} else if (1 == direction) {
					if (checkX - 1 == startX && startY == checkY && 0 == (get(startX, startY) & WEST_MASK))
						return true;
					if (startX == checkX && 1 + checkY == startY)
						return true;
					if (startX == checkX + 1 && checkY == startY)
						return true;
					if (checkX == startX && startY == checkY - 1 && (get(startX, startY) & SOUTH_MASK) == 0)
						return true;
				} else if (direction == 2) {
					if (checkX - 1 == startX && checkY == startY && (get(startX, startY) & WEST_MASK) == 0)
						return true;
					if (startX == checkX && startY == 1 + checkY && 0 == (get(startX, startY) & NORTH_MASK))
						return true;
					...
					if (startX == checkX - 1 && checkY == startY)
						return true;
					if (startX == checkX && 1 + checkY == startY && (get(startX, startY) & NORTH_MASK) == 0)
						return true;
					if (1 + checkX == startX && checkY == startY && (get(startX, startY) & EAST_MASK) == 0)
						return true;
					if (startX == checkX && startY == checkY - 1)
						return true;
				}
			...
			if (type == 0) {
				if (0 == direction) {
					if (startX == checkX - size && checkY >= startY && checkY <= i_45_)
						return true;
					if (checkX >= startX && checkX <= i_44_ && startY == 1 + checkY && ((get(checkX, startY) & NORTH_MASK) == 0))
						return true;
					if (checkX >= startX && checkX <= i_44_ && startY == checkY - size && ((get(checkX, i_45_) & SOUTH_MASK) == 0))
						return true;
				} else if (1 == direction) {
					if (checkX >= startX && checkX <= i_44_ && startY == 1 + checkY)
						return true;
					if (startX == checkX - size && checkY >= startY && checkY <= i_45_ && ((get(i_44_, checkY) & WEST_MASK) == 0))
						return true;
					if (startX == 1 + checkX && checkY >= startY && checkY <= i_45_ && (get(startX, checkY) & EAST_MASK) == 0)
						return true;
				} else if (2 == direction) {
					if (1 + checkX == startX && checkY >= startY && checkY <= i_45_)
						return true;
					if (checkX >= startX && checkX <= i_44_ && startY == checkY + 1 && 0 == (get(checkX, startY) & NORTH_MASK))
						return true;
					if (checkX >= startX && checkX <= i_44_ && checkY - size == startY && 0 == (get(checkX, i_45_) & SOUTH_MASK))
						return true;
				} else if (direction == 3) {
					if (checkX >= startX && checkX <= i_44_ && startY == checkY - size)
						return true;
					if (startX == checkX - size && checkY >= startY && checkY <= i_45_ && 0 == (get(i_44_, checkY) & WEST_MASK))
						return true;
					if (startX == checkX + 1 && checkY >= startY && checkY <= i_45_ && 0 == (get(startX, checkY) & EAST_MASK))
						return true;
				}
			}
			if (2 == type) {
				if (0 == direction) {
					...
					if (checkX >= startX && checkX <= i_44_ && 1 + checkY == startY)
						return true;
					if (1 + checkX == startX && checkY >= startY && checkY <= i_45_ && 0 == (get(startX, checkY) & EAST_MASK))
						return true;
					if (checkX >= startX && checkX <= i_44_ && checkY - size == startY && ((get(checkX, i_45_) & SOUTH_MASK) == 0))
						return true;
				} else if (1 == direction) {
					if (startX == checkX - size && checkY >= startY && checkY <= i_45_ && 0 == (get(i_44_, checkY) & WEST_MASK))
						return true;
					...
					if (checkX - size == startX && checkY >= startY && checkY <= i_45_)
						return true;
					if (checkX >= startX && checkX <= i_44_ && checkY + 1 == startY && 0 == (get(checkX, startY) & NORTH_MASK))
						return true;
					if (startX == checkX + 1 && checkY >= startY && checkY <= i_45_ && 0 == (get(startX, checkY) & EAST_MASK))
						return true;
					if (checkX >= startX && checkX <= i_44_ && startY == checkY - size)
			...
			if (9 == type) {
				if (checkX >= startX && checkX <= i_44_ && startY == checkY + 1 && (get(checkX, startY) & NORTH_MASK) == 0)
					return true;
				if (checkX >= startX && checkX <= i_44_ && checkY - size == startY && 0 == (get(checkX, i_45_) & SOUTH_MASK))
					return true;
				if (checkX - size == startX && checkY >= startY && checkY <= i_45_ && (get(i_44_, checkY) & WEST_MASK) == 0)
					return true;
				if (1 + checkX == startX && checkY >= startY && checkY <= i_45_ && (get(startX, checkY) & EAST_MASK) == 0)
					return true;
			}
		}
		return false;
