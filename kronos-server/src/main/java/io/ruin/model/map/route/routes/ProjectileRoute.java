...
public class ProjectileRoute {
	...
	public static boolean allow(Entity entity, Entity target) {
		return allow(entity.getAbsX(), entity.getAbsY(), entity.getHeight(), entity.getSize(), target.getAbsX(),
				target.getAbsY(), target.getSize());
	}
	...
	public static boolean allow(Position src, int size, Entity target) {
		return allow(src.x(), src.y(), src.z(), size, target.getPosition().x(), target.getPosition().y(), target.getSize());
	}

	public static boolean allow(Position src, int size, Position target, int targetSize) {
		return allow(src.x(), src.y(), src.z(), size, target.x(), target.y(), targetSize);
	}
	...
	private static boolean allowEntrance(int x, int y, int z, int dx, int dy) {
		...
		if (dx == 0 && dy == 1 && (getClipping(x, y + 1, z) & NORTH_MASK) == 0)
			return true;
		if (dx == -1 && dy == -1 && (getClipping(x - 1, y - 1, z) & SOUTH_WEST_MASK) == 0
				&& (getClipping(x - 1, y, z) & WEST_MASK) == 0 && (getClipping(x, y - 1, z) & SOUTH_MASK) == 0)
			return true;
		if (dx == 1 && dy == -1 && (getClipping(x + 1, y - 1, z) & SOUTH_EAST_MASK) == 0
				&& (getClipping(x + 1, y, z) & EAST_MASK) == 0 && (getClipping(x, y - 1, z) & SOUTH_MASK) == 0)
			return true;
		if (dx == -1 && dy == 1 && (getClipping(x - 1, y + 1, z) & NORTH_WEST_MASK) == 0
				&& (getClipping(x - 1, y, z) & WEST_MASK) == 0 && (getClipping(x, y + 1, z) & NORTH_MASK) == 0)
			return true;
		if (dx == 1 && dy == 1 && (getClipping(x + 1, y + 1, z) & NORTH_EAST_MASK) == 0
				&& (getClipping(x + 1, y, z) & EAST_MASK) == 0 && (getClipping(x, y + 1, z) & NORTH_MASK) == 0)
			return true;
		return false;
	}
