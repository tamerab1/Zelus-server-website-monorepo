...
public class KourendCatacombs {
	...
	public static void register() { // Object actions
		...
		ObjectAction.register(28895, "climb-up", (player, obj) -> { // NW
			...
			VarPlayerRepository.CATACOMBS_ENTRANCE_NW.set(player, 1);
		});
		ObjectAction.register(28921, 1, (player, obj) -> {
			if (VarPlayerRepository.CATACOMBS_ENTRANCE_NW.get(player) == 1) {
		...
		ObjectAction.register(28896, "climb-up", (player, obj) -> { // SW
			...
			VarPlayerRepository.CATACOMBS_ENTRANCE_SW.set(player, 1);
		});
		ObjectAction.register(28919, 1, (player, obj) -> {
			if (VarPlayerRepository.CATACOMBS_ENTRANCE_SW.get(player) == 1) {
		...
		ObjectAction.register(28897, "climb-up", (player, obj) -> { // SE
			...
			VarPlayerRepository.CATACOMBS_ENTRANCE_SE.set(player, 1);
		});
		ObjectAction.register(28918, 1, (player, obj) -> {
			if (VarPlayerRepository.CATACOMBS_ENTRANCE_SE.get(player) == 1) {
		...
		ObjectAction.register(28898, "climb-up", (player, obj) -> { // NE
			...
			VarPlayerRepository.CATACOMBS_ENTRANCE_NE.set(player, 1);
		});
		ObjectAction.register(28920, 1, (player, obj) -> {
			if (VarPlayerRepository.CATACOMBS_ENTRANCE_NE.get(player) == 1) {
	...
	public static void drop(Player pKiller, NPC npc, Position dropPosition) {
		...
		if (Random.rollDie(50, 1)) {
			...
			pKiller.sendMessage(Color.RED.wrap("Drop: Ancient Shard"));
		}

	}

	private static boolean superiorSlayerCreature(NPC npc) {
		List<Integer> superiorIds = List.of(
			7388, 7389, 7390, 7391, 7392, 7393, 7394, 9465, 7395, 9287, 9288, 7396, 7397, 7398, 7399, 7400, 10397, 7401,
			7402, 7403, 9258, 10398, 10399, 7404, 7405, 7407, 7408, 7411, 10400, 10401, 7410, 7409, 7406, 10402
		);
		if (superiorIds.contains(npc.getId()))
			return true;
		return false;
	}

	private static boolean rollTotemDrop(Player pKiller, NPC npc) {
		if (superiorSlayerCreature(npc))
			return true;
