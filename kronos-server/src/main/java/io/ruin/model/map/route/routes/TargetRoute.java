...
public class TargetRoute {
	...
	public static void set(Entity entity, Entity target, int distance, Runnable finishAction) {
		...
		if (target instanceof NPC && target.getCombat() != null) {
	...
	private void beforeMovement0(Entity entity) {
		...
		if (distance == 1) {
			...
			if (!inTarget(absX, absY, size, targetX, targetY, targetSize)
				...
				&& ((target.npc != null && target.npc.getDef() != null && target.npc.getId() == 7706) || ((target.npc != null && target.npc.getDef() != null && target.npc.getDef().name.toLowerCase().contains("leviathan"))) || ((target.npc != null && target.npc.getDef() != null && target.npc.getDef().name.toLowerCase().contains("nylocas"))) || (target.npc != null && target.npc.getDef() != null && target.npc.getId() == 7903) || (target.npc != null && target.npc.getDef() != null && target.npc.getId() == 11903) || ((target.npc != null && target.npc.getDef() != null && target.npc.getId() == 8370) || ProjectileRoute.allow(absX, absY, entity.getHeight(), size, targetX, targetY, targetSize)))) { // TODO: look into projectile clipping so the zuk(inferno boss) exception is not required?
				withinDistance = true;
	...
	public static boolean inTarget(int absX, int absY, int size, int targetX, int targetY, int targetSize) {
	...
	public static void sets(Entity entity, Entity target, Runnable finishAction, int interfaceId) {
		int distance = (interfaceId >> 16) == 218 ? Math.min(entity.getPosition().distance(target.getPosition()), 10) : 1;
		set(entity, target, distance, finishAction);
	}
