...
public class HamHideout {
	...
	public static void register() {
		...
		ObjectAction.register(5492, 1, (player, obj) -> {
			if (VarPlayerRepository.HAM_HIDEOUT_ENTRANCE.get(player) == 1) {
		...
		ObjectAction.register(5492, 5, (player, obj) -> player.startEvent(event -> {
			...
			if (Random.rollPercent(70)) {
				...
				World.startEvent(e -> {
					VarPlayerRepository.HAM_HIDEOUT_ENTRANCE.set(player, 1);
					e.delay(100);
					VarPlayerRepository.HAM_HIDEOUT_ENTRANCE.set(player, 0);
				});
			} else {
