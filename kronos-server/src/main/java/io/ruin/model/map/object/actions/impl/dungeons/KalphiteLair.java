...
public class KalphiteLair {
	...
	private static final int DESERT_ECHO_ORB = 30394;

	private static void handleEnteringTheHive(Player player, GameObject entrance) {
		if (VarPlayerRepository.KALPHITE_LAIR_ROPE_INTERIOR.get(player) == 0) return;
		movePlayerToNormalHive(player);
	}

	private static void movePlayerToNormalHive(Player player) {
	...
	public static void register() {
		...
		ObjectAction.register(23609, 1, KalphiteLair::handleEnteringTheHive);
		...
		ObjectAction.register(3827, 1, (player, obj) -> {
			if (VarPlayerRepository.KALPHITE_LAIR_ROPE_EXTERIOR.get(player) == 0)
		...
		ItemObjectAction.register(3827, (player, item, obj) -> {
			...
			if (VarPlayerRepository.KALPHITE_LAIR_ROPE_EXTERIOR.get(player) == 1) {
				player.sendMessage("You already have a rope set up here.");
				return;
			}
			item.remove();
			VarPlayerRepository.KALPHITE_LAIR_ROPE_EXTERIOR.set(player, 1);
		...
		ItemObjectAction.register(23609, (player, item, obj) -> {
			...
			if (VarPlayerRepository.KALPHITE_LAIR_ROPE_INTERIOR.get(player) == 1) {
				player.sendMessage("You already have a rope set up here.");
				return;
			}
			item.remove();
			VarPlayerRepository.KALPHITE_LAIR_ROPE_INTERIOR.set(player, 1);
			player.sendMessage("You set up the rope.");
		});
	}
