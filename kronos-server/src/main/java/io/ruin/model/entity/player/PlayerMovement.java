...
import io.ruin.model.entity.shared.MovementState;
...
public class PlayerMovement extends Movement {
	 ...
	 * Misc
	 */
	public static int getWalkDirection(int diffX, int diffY) {
		if (diffX < 0) {
			if (diffY < 0)
				return 0;
			else if (diffY > 0)
	...
	public static int getRunDirection(int diffX, int diffY) {
		if (diffX == -2) {
			if (diffY == -2)
				return 0;
			else if (diffY == -1)
	...
	public transient int followX = -1, followY = -1;
	public transient int lastFollowX = -1, lastFollowY = -1;
	public transient Player following;
	private transient boolean ctrlRun;
	...
	public void toggleRunning() {
		if (VarPlayerRepository.RUNNING.toggle(player) == 1 && player.energyUnits <= 0) {
			VarPlayerRepository.RUNNING.set(player, 0);
	...
	public void process() {
		...
		if (finishTeleport(player.getPosition())) {
			if (!player.forceMovementUpdate.updated)
				player.teleportModeUpdate.set(MovementState.TELEPORT.getId());
			else
				player.movementModeUpdate.set(MovementState.WALK.getId());
			followX = followY = -1;
			...
			boolean ran = (forceRun || (isRunning() && stepType != StepType.WALK)) && step(player);
			...
			if (ran) {
				...
				player.movementModeUpdate.set(MovementState.RUN.getId());
			} else {
				restoreEnergy();
				walkDirection = getWalkDirection(diffX, diffY);
				player.movementModeUpdate.set(MovementState.WALK.getId());
		...
		player.getUpdater().updateRegion = player.updateRegion();
		player.checkMulti();
		Tile.occupy(player);
	}
	...
	public boolean startTeleport(int maxWildernessLevel, EventConsumer eventConsumer) {
		if (player.isLocked()) {
			player.sendMessage("Something prevents you from teleport.");
			return false;
		}
		if (player.teleportListener != null && !player.teleportListener.allow(player)) {
			player.sendMessage("Some force prevents you from teleporting.");
			return false;
		}
		...
		player.resetStun();
		player.resetFreeze();
		// closes things properly like trade
	...
	private boolean isRunning() {
		return player.energyUnits > 0 && (ctrlRun || VarPlayerRepository.RUNNING.get(player) == 1);
	}
