...
public enum PlayerCounter {
	 ...
	 * Runecrafting
	 */
	CRAFTED_AIR((p, amt) -> p.craftedAir = amt, p -> p.craftedAir),
	CRAFTED_MIND((p, amt) -> p.craftedMind = amt, p -> p.craftedMind),
	CRAFTED_WATER((p, amt) -> p.craftedWater = amt, p -> p.craftedWater),
	CRAFTED_EARTH((p, amt) -> p.craftedEarth = amt, p -> p.craftedEarth),
	CRAFTED_FIRE((p, amt) -> p.craftedFire = amt, p -> p.craftedFire),
	CRAFTED_BODY((p, amt) -> p.craftedBody = amt, p -> p.craftedBody),
	CRAFTED_COSMIC((p, amt) -> p.craftedCosmic = amt, p -> p.craftedCosmic),
	CRAFTED_LAW((p, amt) -> p.craftedLaw = amt, p -> p.craftedLaw),
	CRAFTED_NATURE((p, amt) -> p.craftedNature = amt, p -> p.craftedNature),
	CRAFTED_CHAOS((p, amt) -> p.craftedChaos = amt, p -> p.craftedChaos),
	CRAFTED_DEATH((p, amt) -> p.craftedDeath = amt, p -> p.craftedDeath),
	CRAFTED_ASTRAL((p, amt) -> p.craftedAstral = amt, p -> p.craftedAstral),
	CRAFTED_BLOOD((p, amt) -> p.craftedBlood = amt, p -> p.craftedBlood),
	CRAFTED_SOUL((p, amt) -> p.craftedSoul = amt, p -> p.craftedSoul),
	...
	POLLNIVNEACH_ROOFTOP((p, amt) -> p.pollnivneachRooftopLaps = amt, p -> p.pollnivneachRooftopLaps),
	TOTAL((p, amt) -> p.totalLaps = amt, p -> p.totalLaps),
	...
	MINED_BASALT((p, amt) -> p.minedBasalt = amt, p -> p.minedBasalt),
	MINED_URT_SALT((p, amt) -> p.minedUrtSalt = amt, p -> p.minedUrtSalt),
	MINED_TE_SALT((p, amt) -> p.minedTeSalt = amt, p -> p.minedTeSalt),
	MINED_EFH_SALT((p, amt) -> p.minedEfhSalt = amt, p -> p.minedEfhSalt),
	MINED_LIMESTONE((p, amt) -> p.minedLimestone = amt, p -> p.minedLimestone),
	MINED_ASHPILE((p, amt) -> p.minedAshPile = amt, p -> p.minedAshPile),
	/**
	 * Smithing
	 */
	...
	WALL_SAFES_CRACKED((p, amt) -> p.wallSafesCracked = amt, p -> p.wallSafesCracked),
	...
	COOKED_FOOD((p, amt) -> p.cookedFood = amt, p -> p.cookedFood),
	...
	RAURG_BONES_BURIED((p, amt) -> p.raurgBonesBuried = amt, p -> p.raurgBonesBuried),
	...
	RAURG_BONES_ALTAR((p, amt) -> p.raurgBonesAltar = amt, p -> p.raurgBonesAltar),
	...
	HARVESTED_SNAPEGRASS((p, amt) -> p.harvestedSnapegrass = amt, p -> p.harvestedSnapegrass),
	...
	HARVESTED_GUAM((p, amt) -> p.harvestedGuam = amt, p -> p.harvestedGuam),
	HARVESTED_MARRENTILL((p, amt) -> p.harvestedMarrentill = amt, p -> p.harvestedMarrentill),
	HARVESTED_TARROMIN((p, amt) -> p.harvestedTarromin = amt, p -> p.harvestedTarromin),
	HARVESTED_HARRALANDER((p, amt) -> p.harvestedHarralander = amt, p -> p.harvestedHarralander),
	HARVESTED_RANARR((p, amt) -> p.harvestedRanarr = amt, p -> p.harvestedRanarr),
	HARVESTED_TOADFLAX((p, amt) -> p.harvestedToadflax = amt, p -> p.harvestedToadflax),
	HARVESTED_IRIT((p, amt) -> p.harvestedIrit = amt, p -> p.harvestedIrit),
	HARVESTED_AVANTOE((p, amt) -> p.harvestedAvantoe = amt, p -> p.harvestedAvantoe),
	HARVESTED_KWUARM((p, amt) -> p.harvestedKwuarm = amt, p -> p.harvestedKwuarm),
	HARVESTED_SNAPDRAGON((p, amt) -> p.harvestedSnapdragon = amt, p -> p.harvestedSnapdragon),
	HARVESTED_CADANTINE((p, amt) -> p.harvestedCadantine = amt, p -> p.harvestedCadantine),
	HARVESTED_LANTADYME((p, amt) -> p.harvestedLantadyme = amt, p -> p.harvestedLantadyme),
	HARVESTED_DWARF_WEED((p, amt) -> p.harvestedDwarfWeed = amt, p -> p.harvestedDwarfWeed),
	HARVESTED_TORSTOL((p, amt) -> p.harvestedTorstol = amt, p -> p.harvestedTorstol),
	...
	CLEANED_PAYDIRT((p, amt) -> p.cleanedPaydirt = amt, p -> p.cleanedPaydirt),
	 ...
	 * PVM
	 */
	ABYSSAL_CREATURES_KC((p, amt) -> p.abyssCreaturesKilled = amt, p -> p.abyssCreaturesKilled),
	...
	DEMON_KILLS((p, amt) -> p.demonKills = amt, p -> p.demonKills),

	IMPLINGS_CAUGHT((p, amt) -> p.implingCaught = amt, p -> p.implingCaught),
	FIENDISH_ASHES_SCATTERED((p, amt) -> p.fiendishAshesScattered = amt, p -> p.fiendishAshesScattered),
	VILE_ASHES_SCATTERED((p, amt) -> p.vileAshesScattered = amt, p -> p.vileAshesScattered),
	NALICIOUS_ASHES_SCATTERED((p, amt) -> p.maliciousAshesScattered = amt, p -> p.maliciousAshesScattered),
	ABYSSAL_ASHES_SCATTERED((p, amt) -> p.abyssalAshesScattered = amt, p -> p.abyssalAshesScattered),
	INFERNAL_ASHES_SCATTERED((p, amt) -> p.infernalAshesScattered = amt, p -> p.infernalAshesScattered);
	...
	PlayerCounter(BiConsumer<Player, Integer> setAction, Function<Player, Integer> getAction) {
		this(setAction, getAction, new String[0]);
	}

	PlayerCounter(BiConsumer<Player, Integer> setAction, Function<Player, Integer> getAction, String... achievements) {
