...
public enum Tanner {
	...
	Tanner(String displayName, int raw, int product, int cost) {
		...
		this.rawName = ObjType.get(raw).name.toLowerCase();
		this.productName = ObjType.get(product).name.toLowerCase();
	}
	...
	private static void tan(Player player, Tanner tanner, int amount) {
		...
		Item noted = null;
		if (rawItem == null && (ObjType.get(tanner.raw).notedId < 1 || (noted = player.getInventory().findItem(ObjType.get(tanner.raw).notedId)) == null)) {
		...
		player.startEvent(event -> {
			int made = 0;
			if (ObjType.get(tanner.raw).notedId > 0) {
				if (finalNoted != null) {
					...
					player.getInventory().add(ObjType.get(tanner.product).notedId, tan);
					made += tan;
				}
			}
			while (made++ < amount) {
	...
	private static void leatherTanning(Player player) {
		player.openInterface(ToplevelComponent.MAINMODAL, Interface.LEATHER_TANNING);
		/* Interface settings */
	...
	private static String color(Player player, int rawMaterial, int cost) {
		if ((player.getInventory().findItem(rawMaterial) == null && (ObjType.get(rawMaterial).notedId < 1 || player.getInventory().findItem(ObjType.get(rawMaterial).notedId) == null)) || !player.getInventory().contains(COINS_995, cost))
			return "<col=f80000>";
		return "<col=00c8f8>";
	}
	...
	public static void register() {
		...
		for (int tanner : TANNERS) {
			 ...
			 * Trade
			 */
			NPCAction.register(tanner, "tan", (player, obj) -> leatherTanning(player));
		}
