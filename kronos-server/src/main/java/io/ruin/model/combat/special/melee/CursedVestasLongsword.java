...
public class CursedVestasLongsword implements Special {

	@Override
	public boolean accept(ObjType def, String name) {
		return name.equalsIgnoreCase("cursed vesta's longsword");
	}
	...
	public boolean handle(Player player, Entity target, AttackStyle attackStyle, AttackType attackType, int maxDamage) {
		...
		int chance = (int) (sides * 0.85);
		if (Random.rollDie(sides, chance)) {
			hit.defenceIgnored = true;
			hit.randDamage((int) (maxDamage * 0.25), (int) (maxDamage * 1.25));
		} else {
			hit.block();
		}
