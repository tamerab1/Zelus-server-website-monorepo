...
public class BankPin {
	 ...
	 * Pin settings
	 */
	// TODO(polish) - static var for logic?
	public static int pin = -1;
	private int recoveryDelay = 3;
	private long recoverAt;
	private long lockAt = -1;
	private String lastVerificationIp;

	private transient Player player;
	private transient int confirmationType;
	private transient int enteredPin = -1;
	...
	public void openSettings() {
		...
		player.openInterface(ToplevelComponent.MAINMODAL, 14);
		...
		player.getPacketSender().sendString(14, 14,
			"Players are reminded that they should NEVER tell anyone their account PINs or passwords, nor should they ever enter their PINs on any website form.<br><br>PINs are a good form of secondary account protection, and are highly recommended.");
	}
	...
	private boolean recover() {
		...
		player.sendMessage(
			"Your bank PIN will be reset in: " + "<col=800000>" + TimeUtils.fromMs(recoverAt - currentMs, true));
		return false;
	}
	...
	private void setup() {
		player.getPacketSender().sendClientScript(917, "ii", -1, -1);
		player.openInterface(ToplevelComponent.MAINMODAL, 213);
		...
		player.consumerInt = value -> {
			...
			if (enteredPin != -1) {
				if (value == enteredPin) {
					...
					player.sendMessage(
						"Those numbers did not match, please be sure the PIN you setup is something you can remember!");
				}
				openSettings();
				return;
			}
