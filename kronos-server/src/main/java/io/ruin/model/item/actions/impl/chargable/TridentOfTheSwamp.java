...
public class TridentOfTheSwamp {
	...
	public static void register() {
		...
		ObjType.get(CHARGED).addPreTargetDefendListener(TridentOfTheSwamp::consumeCharge);
	...
	private static void consumeCharge(Player player, Item staff, Hit hit, Entity entity) {
		if (AttributeExtensions.getCharges(staff) > 0) {
			...
			if (AttributeExtensions.hasAttribute(staff, FULLY_LOADED)) {
				int level = AttributeExtensions.getCharges(FULLY_LOADED, staff);
				int chance = 5 - level;
		...
		player.dialogue(new OptionsDialogue("You will NOT get the " + lostItem + " back.",
			new Option("Okay. uncharge it.", () -> {
				if (player.getInventory().getFreeSlots() < 4) {
					player.sendMessage("You'll need at least 4 free inventory spaces to do this.");
					return;
				}
