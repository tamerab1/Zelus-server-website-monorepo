...
public enum EssencePouch {
	...
	public static void fill(Player player, EssencePouch pouch) {
		if (!player.getStats().check(StatType.Runecrafting, pouch.levelReq, "use this pouch"))
			return;

		int inventoryAmount = player.getInventory().count(Essence.PURE.id);
		...
		int pouchAmount = pouchMap.getOrDefault(pouch, 0);

		int spaceInPouch = pouch.capacity - pouchAmount;
		if (spaceInPouch <= 0) {
			player.sendFilteredMessage("You cannot add any more essence to the pouch.");
			return;
		}

		int amountToAdd = Math.min(spaceInPouch, inventoryAmount);
		int totalAmount = pouchAmount + amountToAdd;

		player.getInventory().remove(Essence.PURE.id, amountToAdd);
		pouchMap.put(pouch, totalAmount);
	}

	public static void fillFromBank(Player player, EssencePouch pouch) {
		if (!player.getStats().check(StatType.Runecrafting, pouch.levelReq, "use this pouch"))
			return;

		int inventoryAmount = player.getBank().count(Essence.PURE.id);
		if (inventoryAmount == 0) {
			player.sendFilteredMessage("You do not have any essence to fill your pouch with.");
			return;
		}
		...
		player.getBank().remove(Essence.PURE.id, amountToAdd);
		pouchMap.put(pouch, totalAmount);
	}

	public static void empty(Player player, EssencePouch pouch) {
		if (!player.getStats().check(StatType.Runecrafting, pouch.levelReq, "use this pouch"))
			return;
	...
	private static boolean pouchFull(Player player, EssencePouch pouch, int amount) {
		Map<EssencePouch, Integer> pouchMap = player.runeEssencePouches;
		int totalAmount = pouchMap.getOrDefault(pouch, 0) + amount;

		if (totalAmount >= pouch.capacity) {
			player.sendFilteredMessage("You cannot add any more essence to the pouch.");
			return true;
		}
	...
	public static void register() {
		...
		ObjType.get(Essence.PURE.id).bankWithdrawListener = EssencePouch::withdrawToPouch;
	}
