...
public enum DonatorLamp {
	...
	DonatorLamp(int childId, int hiddenChildId, StatType statType, boolean combat, boolean disabled) {
	...
	public static final DonatorLamp[] VALUES = values();
	...
	public static void register() {
		ItemAction.registerInventory(SKILL_LAMP, "rub", (player, item) -> {
			...
			player.openInterface(ToplevelComponent.MAINMODAL, 1102);
			player.getPacketSender().sendSkillinterface("");
		});

		InterfaceHandler.register(1102, h -> {

			for (DonatorLamp skill : DonatorLamp.VALUES) {
				h.actions[skill.childId] = (SimpleAction) player -> {
					...
					for (DonatorLamp skills : DonatorLamp.VALUES) {
						player.getPacketSender().setHidden(1102, skills.hiddenChildId, true);
					}
					float experience;
					...
					player.getPacketSender().setHidden(1102, skill.hiddenChildId, false);
					player.getPacketSender().sendString(1102, 75, "You will receive " + NumberUtils.formatNumber((long) experience) + "  experience in " + skillName + ".");
					player.getPacketSender().setHidden(1102, 75, false);
					player.openInterface(ToplevelComponent.MAINMODAL, 1102);
