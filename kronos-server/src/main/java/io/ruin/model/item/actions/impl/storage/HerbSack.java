...
public class HerbSack {
	...
	private static void addToSack(Player player, int herbId, int amount) {
		...
		if (herbSize >= getMaxStorage(player)) {
			player.sendFilteredMessage("Your herb sack cannot carry anymore " + ObjType.get(herbId).name.toLowerCase() + "s.");
			return;
		}
	...
	public static void emptyToBank(Player player) {
		for (int herbId : GRIMY_HERBS) {
			int herbSize = amountStored(player, herbId);
			if (herbSize > 0) {
				int banked = player.getBank().add(herbId, herbSize);
				decrementSackStored(player, herbId, banked);
				if (herbSize != banked) {
					return;
				}
			}
		}
		player.sendMessage("Your herbs have been added to your bank.");
	}
	...
	public static void register() {
		...
		ItemAction.registerInventory(13226, "check", (player, item) -> {
			...
			for (int herbs : GRIMY_HERBS) {
				...
				player.sendFilteredMessage(herbSize + " " + ObjType.get(herbs).name.toLowerCase() + "s");
			}
		});
		/**
		 * Empty
