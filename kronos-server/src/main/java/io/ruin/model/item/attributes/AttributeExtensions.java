...
public final class AttributeExtensions {
	...
	public static int deincrementCharges(Item item, int amount) {
		if (AttributeExtensions.hasAttribute(item, FULLY_LOADED)) {
			int level = getCharges(FULLY_LOADED, item);
			int chance = 5 - level;
	...
	public static int deincrementCharges(AttributeTypes type, Item item, int amount) {
		int charges = getCharges(type, item);
		setCharges(type, item, charges - amount);
		return getCharges(type, item);
	}

	public static boolean clearCharges(AttributeTypes type, Item item) {
		item.clearAttribute(type.name());
		item.update();
		return true;
	}

	public static void setCharges(AttributeTypes type, Item item, int chargesTotal) {
		if (chargesTotal > 0)
			item.putAttribute(type.name(), chargesTotal);
		else
			clearCharges(type, item);
		item.update();
	}
	...
	public static int getCharges(AttributeTypes type, Item item) {
		return item.getAttributeInt(type.name(), 0);
	}
