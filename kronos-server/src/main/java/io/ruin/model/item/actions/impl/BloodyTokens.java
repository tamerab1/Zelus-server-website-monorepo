...
public class BloodyTokens {
	...
	public static final int BLOODY_TOKENS = 24719;
		...
		player.dialogue(
			new OptionsDialogue("Exchange your blood money for bloody tokens?",
				new Option("Yes", () -> {
					...
					if (tokens != null) {
						int removeAmount = (Integer.MAX_VALUE - tokens.getAmount()) * 1000;
						if (removeAmount == 0) {
							player.dialogue(new MessageDialogue("Your Platinum tokens stack is too large!"));
							return;
						}
						removeAmount = Math.min(bloodMoney.getAmount(), removeAmount);
