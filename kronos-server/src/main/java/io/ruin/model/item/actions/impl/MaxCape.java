...
public class MaxCape {
	...
	public enum MaxCapes {
		...
		public static final MaxCapes[] VALUES = values();
	}
	...
	public static void register() {
			...
			ItemAction.registerInventory(cape.newCapeId, "revert", (player, item) -> {
				...
				player.dialogue(new OptionsDialogue("Are you use you want to revert your max cape?",
					new Option("Yes", () -> player.startEvent(event -> {
						...
						player.dialogue(new ItemDialogue().two(cape.secondaryId, 13280, "You rip the " + ObjType.get(cape.secondaryId).name + " from your max cape."));
						player.unlock();
					})),
		...
		ObjType.forEach(def -> def.maxType = def.name.toLowerCase().contains("max cape") || def.name.toLowerCase().contains("max hood"));
		ObjType.forEach(def -> def.masterType = def.name.toLowerCase().contains("master cape") || def.name.toLowerCase().contains("master hood"));
	}
