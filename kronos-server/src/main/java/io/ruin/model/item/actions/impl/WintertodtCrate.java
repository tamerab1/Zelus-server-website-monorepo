	...
	public static void register() {
		ItemAction.registerInventory(20703, "open", (player, item) -> {
			if (Random.rollDie(50, 1)) {
				Item rare = new Item(RARE_ITEMS[Random.get(RARE_ITEMS.length - 1)], 1);
				player.getInventory().addOrDrop(rare);
				player.addToCollectionLog(rare);
			}
			item.remove();
			Item firstReward = table.rollItem();
			Item secondReward = table.rollItem();
			player.addToCollectionLog(firstReward);
			player.addToCollectionLog(secondReward);

			if (Random.get(100) == 0)
				Pet.PHOENIX.unlock(player, -1);
			...
			player.dialogue(
				new MessageDialogue("You have earned: " + firstReward.getDef().name + " x " + firstReward.getAmount() + ", "
					+ secondReward.getDef().name + " x " + secondReward.getAmount() + "."));

			player.addToCollectionLog(firstReward, secondReward);
		});
	}
}
