...
public enum DragonfireShield {
	...
	public static void register() {
		for (DragonfireShield shield : DragonfireShield.values()) {
			 ...
			 * Dragonfire charging
			 */
			ObjType.get(shield.chargedId).addPreDefendListener(DragonfireShield::chargeCheck);
			ObjType.get(shield.unchargedId).addPostDamageListener(DragonfireShield::chargeCheck);
	...
	private static boolean wearingDragonfireShield(Player player) {
		...
		return bootId == 30505 || shieldId == 11283 || shieldId == 11284 || shieldId == 21634 || shieldId == 21633 || shieldId == 22003 || shieldId == 22002;
	}
...
//            if(!player.getInventory().hasId(shield.getId())) { // This stopped the player from being able to uncharge it, Don't even see the point of the check anyways.
...
//            System.err.println("4");
	...
	private static void dragonfireShieldSpecial(Player player, Entity target) {
		player.startEvent(event -> {
			...
			player.getCombat().updateLastAttack(4);
			event.delay(3);
			PROJECTILE.send(player, target);
			final int distance = getChebyshevDistance(player, target);
			final int delayTicks = PlayerCombat.getShieldSpecialHitDelay(distance);
			target.hit(new Hit(player, AttackStyle.MAGIC, AttackType.ACCURATE).randDamage(Random.get(25)).delay(delayTicks));
			player.unlock();
		});
	}
