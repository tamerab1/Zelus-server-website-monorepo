...
public enum ItemBreaking {
	...
	ItemBreaking(int fixedId, int brokenId, int bmRepairCost, int coinRepairCost, boolean freeFromShops) {
		...
		ObjType.get(brokenId).brokenFrom = this;
		ObjType.get(fixedId).breakTo = this;
	}

	public static final ItemBreaking[] VALUES = values();

	public static ItemBreaking get(int itemId) {
		return Arrays.stream(VALUES)
			.filter(itemBreaking -> itemBreaking.brokenId == itemId || itemBreaking.fixedId == itemId)
			.findFirst()
			.orElseThrow(() -> new IllegalArgumentException("No ItemBreaking for id: %d".formatted(itemId)));
	}

	public static boolean isBroken(int itemId) {
		return Arrays.stream(VALUES)
			.anyMatch(itemBreaking -> itemBreaking.brokenId == itemId);
	}
}