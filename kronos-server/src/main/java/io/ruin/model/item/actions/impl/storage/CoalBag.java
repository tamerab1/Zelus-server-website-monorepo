...
public class CoalBag {
	...
	private static int getBagAddition(Player player) {
		int amountToAdd = 0;
		switch (player.getSecondaryGroup()) {
			case DONATOR -> amountToAdd = 8;
			case SUPER_DONATOR -> amountToAdd = 23;
			case ELITE_DONATOR -> amountToAdd = 73;
			case NOBLE_DONATOR -> amountToAdd = 123;
			case GOLD_DONATOR -> amountToAdd = 223;
			case PLATINUM_DONATOR -> amountToAdd = 323;
			case LEGENDARY_DONATOR -> amountToAdd = 423;
			case SUPREME_DONATOR -> amountToAdd = 523;
		}
		return amountToAdd;
	}
	...
	private static int withdrawToBag(Player player, Item item, int amount) {
		...
		// Adjust banked amount if withdrawing from the bank
		if (player.getBank().contains(item.getId())) {
			int bankedAmount = player.getBank().count(item.getId());
			int newBankedAmount = Math.max(0, bankedAmount - intercept);
	...
	public static void register() {
		...
		ObjType.get(COAL).bankWithdrawListener = CoalBag::withdrawToBag;
	}
}
