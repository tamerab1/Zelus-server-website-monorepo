 ...
 * @project Kronos
 */
public class HolySanguinestiStaff {

	public static final int CHARGED = 25731;
	public static final int UNCHARGED = 25733;
	...
	public static void check(Player player, Item staff) {
		player.sendMessage("Your Holy sanguinesti staff has " + NumberUtils.formatNumber(AttributeExtensions.getCharges(staff)) + " charges remaining.");
	}
	...
	private static void wield(Player player, Item item) {
		player.sendMessage("Your Holy sanguinesti staff has no charges! You can use blood runes to power the staff.");
	}

	public static void register() {

		ItemItemAction.register(UNCHARGED, ItemID.BLOOD_RUNE, HolySanguinestiStaff::charge);
		ItemAction.registerInventory(UNCHARGED, "charge", HolySanguinestiStaff::charge);
		ItemAction.registerInventory(UNCHARGED, "wield", HolySanguinestiStaff::wield);

		ItemItemAction.register(CHARGED, ItemID.BLOOD_RUNE, HolySanguinestiStaff::charge);
		ItemAction.registerInventory(CHARGED, "charge", HolySanguinestiStaff::charge);
		ItemAction.registerEquipment(CHARGED, "check", HolySanguinestiStaff::check);
		ItemAction.registerInventory(CHARGED, "check", HolySanguinestiStaff::check);
		ItemAction.registerInventory(CHARGED, "uncharge", HolySanguinestiStaff::uncharge);
		ObjType.get(CHARGED).addPreTargetDefendListener(HolySanguinestiStaff::consumeCharge);
