...
import static io.ruin.cache.ItemID.REVENANT_ETHER;
...
public enum RingOfWealth {
	...
	public static final RingOfWealth[] VALUES = values();
	...
	private static void displayFeatures(Player player, Item item) {
		player.dialogue(new OptionsDialogue("What features would you like to toggle?",
				...
				new Option("Auto Collect Blood money: " + (player.ROWAutoCollectBM ? Color.DARK_GREEN.wrap("Enabled") : Color.DARK_RED.wrap("Disabled")), () -> toggleBMCollect(player)),
				new Option("Auto Collect Ether: " + (player.ROWAutoCollectEther ? Color.DARK_GREEN.wrap("Enabled") : Color.DARK_RED.wrap("Disabled")), () -> toggleEtherCollect(player))
			)
		);
	}

	private static void toggleBMCollect(Player player) {
		player.ROWAutoCollectBM = !player.ROWAutoCollectBM;
		if (player.ROWAutoCollectBM)
	...
	public static boolean check(Player pKiller, Item item) {
		if (!RingOfWealth.wearingRingOfWealth(pKiller))
			return false;
		if (item.getId() == 13307 && pKiller.ROWAutoCollectBM)
			return true;
		if (item.getId() == REVENANT_ETHER && pKiller.ROWAutoCollectEther)
			return true;
