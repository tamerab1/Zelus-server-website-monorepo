...
public class AccursedSceptre {

	private static final int CHARGED = 8876;
	private static final int CHARGED_A = 8878;
	private static final int UNCHARGED = 30565;
	private static final int UNCHARGED_A = 30560;
	...
	public static void register() {
		ItemAction.registerInventory(CHARGED, "check", AccursedSceptre::check);
		ItemAction.registerInventory(CHARGED, "swap", AccursedSceptre::swap);
		ItemAction.registerEquipment(CHARGED, "check", AccursedSceptre::check);
		ItemAction.registerEquipment(CHARGED, "swap", AccursedSceptre::swap);
		ItemAction.registerInventory(CHARGED, "uncharge", AccursedSceptre::uncharge);
		ItemItemAction.register(CHARGED, REVENANT_ETHER, AccursedSceptre::charge);
		ItemItemAction.register(UNCHARGED, REVENANT_ETHER, AccursedSceptre::charge);

		ItemAction.registerInventory(CHARGED_A, "check", AccursedSceptre::check);
		ItemAction.registerInventory(CHARGED_A, "swap", AccursedSceptre::swap);
		ItemAction.registerEquipment(CHARGED_A, "check", AccursedSceptre::check);
		ItemAction.registerEquipment(CHARGED_A, "swap", AccursedSceptre::swap);
		ItemAction.registerInventory(CHARGED_A, "uncharge", AccursedSceptre::uncharge);
		ItemItemAction.register(CHARGED_A, REVENANT_ETHER, AccursedSceptre::charge);
		ItemItemAction.register(UNCHARGED_A, REVENANT_ETHER, AccursedSceptre::charge);
		ObjType.get(CHARGED_A).addPreTargetDefendListener((player, item, hit, target) -> {
			if (hit.attackStyle != null && hit.attackStyle.isMagic() && target.npc != null && player.wildernessLevel > 0) {
				hit.boostAttack(1.5); //100% accuracy increase
				hit.boostDamage(1.5);
	...
	private static void swap(Player player, Item sceptre) {
		if (sceptre.getId() == CHARGED) {
			sceptre.setId(CHARGED_A);
		} else if (sceptre.getId() == CHARGED_A) {
			sceptre.setId(CHARGED);
		} else if (sceptre.getId() == UNCHARGED) {
			sceptre.setId(UNCHARGED_A);
		} else if (sceptre.getId() == UNCHARGED_A) {
			sceptre.setId(UNCHARGED);
		}
		player.sendMessage("You swap your sceptre to a " + sceptre.getDef().name + ".");
	}
