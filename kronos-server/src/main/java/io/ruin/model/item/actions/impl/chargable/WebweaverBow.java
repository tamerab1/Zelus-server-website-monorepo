...
public class WebweaverBow {

	private static final int CHARGED = 8874;
	private static final int UNCHARGED = 30568;
	...
	public static void register() {
		ItemAction.registerInventory(CHARGED, "check", WebweaverBow::check);
		ItemAction.registerEquipment(CHARGED, "check", WebweaverBow::check);
		ItemAction.registerInventory(CHARGED, "uncharge", WebweaverBow::uncharge);
		ItemItemAction.register(CHARGED, REVENANT_ETHER, WebweaverBow::charge);
		ItemItemAction.register(UNCHARGED, REVENANT_ETHER, WebweaverBow::charge);
		ObjType.get(CHARGED).addPreTargetDefendListener((player, item, hit, target) -> {
			if (hit.attackStyle != null && hit.attackStyle.isRanged() && target.npc != null && player.wildernessLevel > 0) {
				if (consumeCharge(player, item)) {
					hit.boostAttack(1.9); //50% accuracy increase
					hit.boostDamage(1.6); //50% damage increase
				}
			}
		});
	}
