...
public enum RingOfSuffering {
	...
	public static void check(Player player, Hit hit) {
		...
		if (hit.attacker instanceof NPC && hit.attacker.npc.getDef().name.equalsIgnoreCase("Snakeling")) {
			Objects.requireNonNull(player.combatAchievementsList.get(player.getCombatAchievementIndexByOrdinal(CombatAchievements.TAKE_THAT.ordinal())).
