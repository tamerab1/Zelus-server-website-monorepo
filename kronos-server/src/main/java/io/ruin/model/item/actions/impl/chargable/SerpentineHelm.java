...
public enum SerpentineHelm {
	...
	public static void register() {
		...
		for (SerpentineHelm helm : SerpentineHelm.values()) {
			...
			ObjType.get(helm.charged).addOnTickEvent(SerpentineHelm::consumeCharge);
			ObjType.get(helm.charged).addPreTargetDefendListener((player, item, hit, target) -> {
				if (hit.attackStyle != null && hit.attackStyle.isMelee() && target.npc != null && !target.isPoisonImmune() && Random.rollDie(6, 1))
					target.envenom(6);
			});
			ObjType.get(helm.charged).breakId = SERPENTINE.uncharged;
		}
		ObjType.get(29796).addPostTargetDefendListener((player, item, hit, target) -> {
				if(Random.get(2) == 0) {
					target.envenom(6);
				}
		});
