...
public class SkeletalMystic extends NPCCombat {
	...
	private void fireAttack() {
		...
		if (target.player != null && target.player.getPrayer().isActive(Prayer.PROTECT_FROM_MAGIC))
			maxDamage /= 2;
		target.hit(new Hit(npc, AttackStyle.MAGIC).randDamage(maxDamage).clientDelay(delay).ignoreDefence().ignorePrayer());
	...
	private void meleeAttack() {
		...
		if (target.player.getPrayer().isActive(Prayer.PROTECT_FROM_MELEE))
			maxDamage /= 2;
		target.hit(new Hit(npc, AttackStyle.CRUSH).randDamage(maxDamage).ignoreDefence().ignorePrayer());
	}
}
