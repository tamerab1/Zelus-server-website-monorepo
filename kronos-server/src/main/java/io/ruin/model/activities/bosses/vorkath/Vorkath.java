...
public class Vorkath extends NPCCombat {
	...
	private static final Projectile VENOM_PROJECTILE = new Projectile(1470, BREATH_START_HEIGHT, BREATH_END_HEIGHT,
			BREATH_DELAY, BREATH_DURATION_START, BREATH_DURATION_INCREMENT, BREATH_CURVE, BREATH_OFFSET)
			.tileOffset(TILE_OFFSET);
	private static final Projectile PURPLE_PROJECTILE = new Projectile(1471, BREATH_START_HEIGHT, BREATH_END_HEIGHT,
			BREATH_DELAY, BREATH_DURATION_START, BREATH_DURATION_INCREMENT, BREATH_CURVE, BREATH_OFFSET)
			.tileOffset(TILE_OFFSET);
	private static final Projectile ICE_PROJECTILE = new Projectile(395, BREATH_START_HEIGHT, BREATH_END_HEIGHT,
			BREATH_DELAY, BREATH_DURATION_START, BREATH_DURATION_INCREMENT, BREATH_CURVE, BREATH_OFFSET)
			.tileOffset(TILE_OFFSET);
	private static final Projectile FIRE_PROJECTILE = new Projectile(393, BREATH_START_HEIGHT, BREATH_END_HEIGHT,
			BREATH_DELAY, BREATH_DURATION_START, BREATH_DURATION_INCREMENT, BREATH_CURVE, BREATH_OFFSET)
			.tileOffset(TILE_OFFSET);

	private static final Projectile RANGED_PROJECTILE = new Projectile(1477, BREATH_START_HEIGHT, BREATH_END_HEIGHT,
			BREATH_DELAY, BREATH_DURATION_START, BREATH_DURATION_INCREMENT, BREATH_CURVE, BREATH_OFFSET)
			.tileOffset(TILE_OFFSET);
	private static final Projectile MAGIC_PROJECTILE = new Projectile(1479, BREATH_START_HEIGHT, BREATH_END_HEIGHT,
			BREATH_DELAY, BREATH_DURATION_START, BREATH_DURATION_INCREMENT, BREATH_CURVE, BREATH_OFFSET)
			.tileOffset(TILE_OFFSET);

	private static final Projectile UP_FIREBALL_PROJECTILE = new Projectile(1481, BREATH_START_HEIGHT + 10, 0,
			BREATH_DELAY + 20, 180 - BREATH_DELAY, 0, 55, BREATH_OFFSET);
	private static final Projectile ZOMBIE_SPAWN_PROJECTILE = new Projectile(1484, BREATH_START_HEIGHT + 10, 0,
			BREATH_DELAY + 20, 180 - BREATH_DELAY, 0, 40, 0);
	private static final Projectile POISON_PROJECTILE = new Projectile(1483, BREATH_START_HEIGHT + 15, 0,
			BREATH_DELAY - 10, 105 - BREATH_DELAY, 0, 45, 0);

	private static final Projectile FIREBALL_PROJECTILE = new Projectile(1482, BREATH_START_HEIGHT, 0, 0, 30, 0,
			BREATH_CURVE, BREATH_OFFSET);
	...
	public boolean damagedWithSpecial = false;
	boolean damagedWithCrumbleUndead = false;
	public int attacksInAcidSpecial = 0;
	...
	public static void createAndEnter(Player player) {
		DynamicMap map;
		try {
			map = new DynamicMap().build(9023, 1);
		} catch (DynamicMap.DynamicMapBuildException e) {
			player.sendMessage("Unable to create dynamic map.");
			return;
		}

		NPC vorkath = new NPC(8059).spawn(map.convertX(SPAWN_POSITION.getX()), map.convertY(SPAWN_POSITION.getY()), 0,
				Direction.SOUTH, 0);
		map.addNpc(vorkath);
	 ...
	 * 7957 - Upwards breath attack, followed by a channeled breath forward (acid
	 * pool attack)
	 * <p>
	 * GFX:
	...
	public void init() {
		...
		npc.hitListener = new HitListener().postDefend(this::postDefend).postDamage(this::postDamage);
	}
	...
	private void postDamage(Hit hit) {
		...
		ObjType wepDef = hit.attacker.player.getEquipment().getDef(Equipment.SLOT_WEAPON);
		WeaponType wepType = (wepDef == null || wepDef.weaponType == null) ? WeaponType.UNARMED : wepDef.weaponType;
		if (wepType != WeaponType.UNARMED)
			usedNonFist = true;
		if (resistance == Resistance.PARTIAL)
			attacksInAcidSpecial++;
		if (attacksInAcidSpecial >= 20 && hit.attacker != null && hit.attacker.isPlayer()) {
			Objects.requireNonNull(hit.attacker.player.combatAchievementsList
					.get(hit.attacker.player.getCombatAchievementIndexByOrdinal(CombatAchievements.THE_WALK.ordinal()))
					.getCombatAchievement()).check(hit.attacker.player);
		}
	}

	public boolean usedNonMelee = false;
	public boolean usedNonFist = false;
	...
	public void resetVariables() {
		...
		resistance = null;
		usedNonMelee = false;
		damagedWithSpecial = false;
		damagedWithCrumbleUndead = false;
		attacksInAcidSpecial = 0;
		timerStarted = false;
		usedNonFist = false;
	}
	...
	public boolean attack() {
		...
		if (!timerStarted) {
			timerStarted = true;
			target.player.vorkathTimer = new ActivityTimer();
		}
	...
	private void fireballAttack() {
		...
		npc.addEvent(event -> {
			...
			if (target != null && target.getPosition().isWithinDistance(targetPos, 1)) {
				target.hit(new Hit(npc).randDamage(50, 90).delay(0));
				damagedWithSpecial = true;
			}
		});
	}
	...
	private void venomBreath() {
		...
		target
				.hit(new Hit(npc, AttackStyle.DRAGONFIRE).randDamage(60).penetrateDragonfireResistance(0.2).clientDelay(delay));
	...
	private void fireBreath() {
		...
		target
				.hit(new Hit(npc, AttackStyle.DRAGONFIRE).randDamage(80).penetrateDragonfireResistance(0.3).clientDelay(delay));
	...
	private void purpleBreath() {
		...
		target.hit(new Hit(npc, AttackStyle.DRAGONFIRE).randDamage(70).penetrateDragonfireResistance(0.2).clientDelay(delay)
				.postDamage(entity -> {
	...
	private void zombieSpawn() {
		...
		Position pos = Random.get(npc.getPosition().area(7, position -> position.getTile().clipping == 0
		...
		npc.addEvent(event -> {
			...
			spawn.skipReachCheck = p -> p.distance(npc.getPosition()) < 100;
	...
	private void poisonAttack() {
		...
		// may seem odd that this event is on the target but it's because players are
		// processed before npcs, if the event is on the npc then it will not look right
		target.addEvent(event -> {
			...
			var positions = npc.getSpawnPosition().relative(3, 3).area(11, p -> p.getTile().clipping == 0);
			var pools = new LinkedList<GameObject>();
