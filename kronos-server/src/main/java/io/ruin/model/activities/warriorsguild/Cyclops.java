...
public class Cyclops extends BasicCombat {
	...
	private static final Bounds WARRIORSGUILD = new Bounds(2837, 3530, 2877, 3557, -1);
	private static final Bounds WARRIORSGUILD2 = new Bounds(2905, 9956, 2940, 9974, -1);
	...
	public void init() {
		npc.deathEndListener = (DeathListener.SimpleKiller) killer -> {
			...
			if (!npc.getPosition().inBounds(WARRIORSGUILD) && defenderId != DRAGON_DEFENDER)
				return;
			if (!npc.getPosition().inBounds(WARRIORSGUILD2) && defenderId == DRAGON_DEFENDER)
				return;
			if (defenderId == -1)
				return;
			if (!Random.rollDie(defenderId == DRAGON_DEFENDER ? 20 : 10)) //20 :10
				return;
			killer.player.addToCollectionLog(new Item(defenderId, 1));
			new GroundItem(defenderId, 1)
				.owner(killer.player)
				.position(npc.getPosition())
				.spawn();
			killer.player.sendMessage("<col=804080>The cyclops has dropped a " + ObjType.get(defenderId).name + ".");
		};
	}
