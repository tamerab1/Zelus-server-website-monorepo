...
public class GreatOlm extends NPCCombat {
	...
	public static void register() {
		LocType.get(32297).clipType = 1; // force flame wall fire to clip tiles
		LocType.get(32297).gateType = true;
		ObjectAction.register(32297, "douse", (player, obj) -> { // fire wall douse
			...
			if (rune == null || !SpellBook.LUNAR.isActive(player) || !player.getStats().check(StatType.Magic, 68)) {
				player.sendMessage(
						"You do not have the required runes to cast a water spell from your current spellbook at your Magic level.");
				return;
			}
			rune.remove();
			player.animate(6294);
	...
	private static final Projectile MAGIC_PROJECTILE = new Projectile(1339, 90, 43, 35, 40, 17, 16, 192);
	private static final Projectile RANGED_PROJECTILE = new Projectile(1340, 90, 43, 35, 40, 18, 16, 192);
	...
	enum Facing {
		RIGHT(7337, 7376, 7346, 7373,
				7339, 7343, 7381, 7379,
				28, 44,
				37, 52), CENTER(7336, 7374, 7345, 7371,
						7340, 7342, 7382, 7378,
						28, 39,
						37, 49), LEFT(7338, 7375, 7347, 7372,
								7341, 7344, 7377, 7380,
								28, 34,
								37, 45),
								;

		Facing(int idleAnim, int empoweredIdleAnim, int attackAnim, int empoweredAttackAnim, int closeTransitionAnim,
				int farTransitionAnim, int empoweredCloseTransitionAnim, int empoweredFarTransitionAnim, int swX, int swY,
				int neX, int neY) {
	...
	enum PhasePower {
		ACID(Color.DARK_GREEN.wrap("acid")), FLAME(Color.RED.wrap("flame")), CRYSTAL(Color.RAID_PURPLE.wrap("crystal")),
		;
	...
	private static final StatType[] SCALED_STATS =
			{StatType.Hitpoints, StatType.Defence, StatType.Attack, StatType.Strength, StatType.Magic, StatType.Ranged};
	...
	@Override
	public void init() {
		basePosition = new Position(npc.getPosition().getRegion().baseX, npc.getPosition().getRegion().baseY, 0);
		northTargetBounds = new Bounds(getPosition(RIGHT.swX, RIGHT.swY), getPosition(RIGHT.neX, RIGHT.neY), 0);
		centerTargetBounds = new Bounds(getPosition(CENTER.swX, CENTER.swY), getPosition(CENTER.neX, CENTER.neY), 0);
		southTargetBounds = new Bounds(getPosition(LEFT.swX, LEFT.swY), getPosition(LEFT.neX, LEFT.neY), 0);
		arenaBounds = new Bounds(getPosition(28, 35), getPosition(37, 51), 0);
		arenaBounds =
				new Bounds(new Position(npc.getPosition().getRegion().baseX + 28, npc.getPosition().getRegion().baseY + 35),
						new Position(npc.getPosition().getRegion().baseX + 37, npc.getPosition().getRegion().baseY + 51), 0);
		lastPhase = 2; // 0,1,2 = 3 phases default
		lastPhase += scaledFor / 8; // 1 extra phase for every 8 party members
		rightClaw = spawnNPC(7553, 38, 47, Direction.WEST, 0);
		leftClaw = spawnNPC(7555, 38, 37, Direction.WEST, 0);
		leftClaw.getCombat().setAllowRetaliate(false);
		rightClaw.getCombat().setAllowRetaliate(false);
		scaleNPC(rightClaw, scaledFor);
		scaleNPC(leftClaw, scaledFor);
		scaleNPC(npc, scaledFor);
		rightClaw.getCombat().restore();
		leftClaw.getCombat().restore();
		rightClaw.deathEndListener = leftClaw.deathEndListener = (entity, killer, killHit) -> clawDeathEnd(entity.npc);
		rightClaw.deathStartListener =
				leftClaw.deathStartListener = (entity, killer, killHit) -> clawDeathStart(entity.npc);
	...
	private void preLeftClawDefend(Hit hit) {
		...
		else if (hit.attackStyle != null && !hit.attackStyle.isMelee()) {
	...
	private void olmDeathEnd() {
		NPC guide =
				new NPC(7601).spawn(npc.getPosition().getRegion().baseX + 35, npc.getPosition().getRegion().baseY + 53, 0);
		getObject(npc).setId(29882);
		GameObject crystal =
				Tile.getObject(30018, npc.getPosition().getRegion().baseX + 32, npc.getPosition().getRegion().baseY + 53, 0);
		Tile.getObject(30027, npc.getPosition().getRegion().baseX + 33, npc.getPosition().getRegion().baseY + 55, 0)
				.setId(30028); // reward chest
		crystal.animate(7506);
		Player player = npc.getPosition().getRegion().players.getFirst();
		if (player != null) {
			player.getCustomXericRaid().completeRaid();
		}
	...
	private void startClawReviveTimer(NPC claw) {
		...
		claw.addEvent(event -> {
			...
			if (!otherClaw.getCombat().isDead() && progress >= 25) { // failed, revive claw
				...
				restore(claw);
				var clawObj = getObject(claw);
				if (clawObj != null) {
					clawObj.setId(claw == rightClaw ? 29886 : 29883);
				}
				event.delay(1);
				damagedPlayer = true;
				animate(claw, claw == rightClaw ? 7350 : 7354);
				event.delay(5);
				clawObj = getObject(claw);
				if (clawObj != null) {
					clawObj.setId(claw == rightClaw ? 29887 : 29884);
				}
				claw.hitsUpdate.hpbarId = 1;
			} else {
	...
	private void nextPhase() {
		...
		World.startEvent(event -> {
			...
			/*
			 * if (isOnEastSide()) { // to west npc.getMovement().teleport(getPosition(23, 42));
			 * leftClaw.getMovement().teleport(getPosition(23, 47)); rightClaw.getMovement().teleport(getPosition(23, 37)); }
			 * else { npc.getMovement().teleport(npc.getSpawnPosition());
			...
			System.out.println("rise starting");
			rise(event);
			restore(leftClaw);
			restore(rightClaw);
			currentPhase++;
	...
	private void rise(Event event) {
		...
		setId(rightClaw, 29886);
		setId(npc, 29880);
		setId(leftClaw, 29883);
		event.delay(1);
		...
		setId(rightClaw, 29887);
		setId(npc, 29881);
		setId(leftClaw, 29884);
		npc.unlock();
		leftClaw.unlock();
		rightClaw.unlock();
		leftClaw.getCombat().setAllowRetaliate(false);
	...
	private void basicAttack(List<Player> targets) {
		...
		lastBasicAttackStyle = Random.rollPercent(75) ? lastBasicAttackStyle
				: (lastBasicAttackStyle == AttackStyle.RANGED ? AttackStyle.MAGIC : AttackStyle.RANGED);
		targets.forEach(p -> {
			World.startEvent(e -> {
				...
				if (p.getPrayer().isActive(
						lastBasicAttackStyle == AttackStyle.RANGED ? Prayer.PROTECT_FROM_MISSILES : Prayer.PROTECT_FROM_MAGIC))
					maxDamage /= 4;
				p.hit(new Hit(npc, lastBasicAttackStyle).randDamage(maxDamage).ignorePrayer().ignoreDefence());
			});
		});
	}

	TickDelay sphereDelay = new TickDelay();

	private void sphereAttack(List<Player> targets) {
		if (sphereDelay.remaining() > 0)
			return;
		sphereDelay.delay(35);
		animate(npc, facing.getAttackAnim(isEmpowered()));
		delayedAnimation(npc, facing.getIdleAnim(isEmpowered()), 1);
		for (int i = 0; i < 3 && targets.size() > 0; i++) {
			Player target = targets.remove(Random.get(targets.size() - 1));
			AttackStyle style =
					Random.get() < 1d / 3 ? AttackStyle.MAGIC : (Random.get() < 1d / 2 ? AttackStyle.RANGED : AttackStyle.STAB);
			String message;
			...
			if (target.getPrayer().isActive(Prayer.PROTECT_FROM_MISSILES)
					|| target.getPrayer().isActive(Prayer.PROTECT_FROM_MAGIC)
					|| target.getPrayer().isActive(Prayer.PROTECT_FROM_MELEE)) {
		...
		getAllTargets().forEach(p -> {
			...
			p.addEvent(event -> {
				...
				if (p.getPosition().distance(pos) < 1) {
					p.hit(new Hit(npc).randDamage(30, 50).ignoreDefence().ignorePrayer());
					damagedPlayer = true;
				}
				event.delay(1);
				crystal.remove();
	...
	private void lightningAttack() {
		...
		for (int x = 30; x <= 36; x++) {
			...
			Position lightningPos = getPosition(x, yStep == -1 ? 52 : 35);
			npc.addEvent(event -> {
				event.setCancelCondition(this::isDead);
				for (int i = 0; i < 17; i++) {
	...
	private void swapAttack() {
		...
		for (int i = 0; i < 4; i++) {
			...
			if (!targets.isEmpty())
				other = targets.pop();
			if (other != null) {
				player.sendMessage("The Great Olm has paired you with " + Color.RED.wrap(other.getName())
						+ "! The magical power will enact soon...");
				other.sendMessage("The Great Olm has paired you with " + Color.RED.wrap(player.getName())
						+ "! The magical power will enact soon...");
			} else {
	...
	private void crystalBomb() {
		...
		delayedAnimation(npc, facing.getIdleAnim(isEmpowered()), 1);
		int bombCount;
		if (npc.getPosition().getRegion().players.size() >= 5)
			bombCount = 3;
		else
			bombCount = 2;
	...
	private void crystalMark() {
		...
		for (int i = potentialTargets.size() - 1; i >= 0; i--) {
			if (!potentialTargets.get(i).olmSpecialDelay.finished())
		...
		target.graphics(246);
		target.olmSpecialDelay.delay(30);
		World.startEvent(event -> {
			...
			while (crystals++ < 10) {
				...
				getAllTargets().forEach(p -> {
					if (p.getPosition().equals(pos)) {
						p.hit(new Hit(npc).randDamage(10, 15).ignorePrayer().ignoreDefence());
						damagedPlayer = true;
					}
				});
				...
				getAllTargets().forEach(p -> {
					if (p.getPosition().equals(pos)) {
						p.hit(new Hit(npc).randDamage(5, 10).ignorePrayer().ignoreDefence());
						damagedPlayer = true;
					}
				});
			}
	...
	private static final StatType[] BURN_STAT_DRAIN =
			{StatType.Attack, StatType.Strength, StatType.Defence, StatType.Ranged, StatType.Magic};
	...
	private void spawnAcidPool(Position position, int duration) {
		if (Tile.getObject(30032, position.getX(), position.getY(), 0, 10, -1) == null)
			return;
	...
	private void turn() {
		...
		if (dest == null || dest == facing)
			return;
		if ((facing == LEFT && dest == RIGHT) || (facing == RIGHT && dest == LEFT) || (facing == LEFT && dest == CENTER)) { // 'far'
																																																												// transition
	...
	private void setId(NPC npc, int id) {
		if (npc == null) {
			return;
		}
		var obj = getObject(npc);
		if (obj != null) {
			obj.setId(id);
		}
	}
	...
	public NPC spawnNPC(int id, int localX, int localY, Direction face, int walkRange) {
		...
		Position spawnPosition =
				basePosition.relative(rotatedX(getTileSize(), localX, localY, npc.getSize(), npc.getSize(), 0), // direction is
																																																				// irrelevant
																																																				// here since
																																																				// npcs are
																																																				// always square
	...
	private void setOlmNoTargetFace() {
		List<Bounds> bounds = new ArrayList<>();
		bounds.add(centerTargetBounds);
		bounds.add(northTargetBounds);
