...
public class Guardian_Drake extends NPCCombat {
	...
	@Override
	public void init() {
		npc.deathEndListener = (entity, killer, killHit) -> npc.transform(10401);
	}
	...
	private void specialAttack() {
		...
		npc.addEvent(event -> {
			...
			if (target != null && target.getPosition().equals(targetPos)) {
				target.hit(new Hit(npc, AttackStyle.DRAGONFIRE).randDamage(12, 39));
			}
		});
	}
