...
public class KalphiteQueen extends NPCCombat {
	...
	List<Integer> flails = Arrays.asList(
	...
	private void preDefend(Hit hit) {
		if (hit.attackStyle == null)
			return;
		if (hit.attackStyle.isMelee() && hit.attacker != null && hit.attacker.isPlayer() &&
			hit.attacker.player.getEquipment().get(Equipment.SLOT_WEAPON) != null && flails.contains(hit.attacker.player.getEquipment().get(Equipment.SLOT_WEAPON).getId())) {

		} else usedNonFlail = true;
	...
	private void aggroMinions() {
		npc.localNpcs().forEach(n -> {
			...
			if (combat == null)
				return;
			if (combat.getTarget() == null && Random.rollDie(6)) {
				final Player p = Random.get(npc.localPlayers());
				if (p != null) {
					combat.setTarget(p);
					n.face(p);
				}
