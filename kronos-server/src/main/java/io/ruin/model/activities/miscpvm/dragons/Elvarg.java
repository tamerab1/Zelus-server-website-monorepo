...
public class Elvarg extends ChromaticDragon {
	...
	private Position closestLavaSource() {
		...
		return lavaSources.stream()
				.reduce((position,
						position2) -> Misc.getDistance(target.getPosition(), position2) < Misc.getDistance(target.getPosition(),
	...
	@Override
	protected void meleeDragonfire() {
		npc.animate(81);
		npc.graphics(1, 100, 0);
		target.hit(new Hit(npc, AttackStyle.DRAGONFIRE).randDamage(20, 50).penetrateDragonfireResistance(0.2));
	}

	@Override
	public Hit basicAttack() {
		Hit mainHit = super.basicAttack();
		target.hit(new Hit(npc, AttackStyle.DRAGONFIRE).randDamage((int) (info.max_damage * 1.5))
				.penetrateDragonfireResistance(0.2));
		return mainHit;
	}
	...
	private void burnGround(Entity entity) {
		...
		entity.getPosition().area(1).forEach(pos -> {
			...
			if (Tile.allowObjectPlacement(pos.getX(), pos.getY(), npc.getHeight())
					|| (obj != null && (obj.getId() == 25065 || (obj.getId() >= 25087 && obj.getId() <= 25089)))) {
				// fires.add(GameObject.spawn(732, pos.getX(), pos.getY(), npc.getHeight(), 10,
				// 0));
	...
	public static void loot(Player player, NPC npc, GameObject corpse) {
		List<Item> items = NPCType.get(6118).lootTable.rollItems(true,
				DropRateBonusManager.getInstance().getTotalBonusDropRate(player));
		player.startEvent(event -> {
			...
			if (player.getInventory().isFull()) {
				player.sendMessage(
						Color.ORANGE_RED.wrap("WARNING: Items left on the ground will be lost when you leave the instance."));
			}
			if (!corpse.isRemoved())
				corpse.remove();
	...
	public static void createAndEnter(Player player) {
		DynamicMap map;
		try {
			map = new DynamicMap().build(11414, 1);
		} catch (DynamicMap.DynamicMapBuildException e) {
			player.sendMessage("Unable to create dynamic map.");
			return;
		}

		NPC elvarg = new NPC(6118).spawn(map.convertX(SPAWN_POSITION.getX()), map.convertY(SPAWN_POSITION.getY()), 0,
				Direction.SOUTH, 10);
		map.addNpc(elvarg);
		player.startEvent(event -> {
		...
		// ObjectAction.register(25161, map.convertX(2846), map.convertY(9636), 0,
		// "climb-over", exitAction);
		// ObjectAction.register(25161, map.convertX(2846), map.convertY(9635), 0,
		// "climb-over", exitAction);
	}
