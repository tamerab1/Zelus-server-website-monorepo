...
public class IceDemonChamber extends Chamber {
	...
	private void addKindling(Player player, GameObject gameObject) {
		...
		if (gameObject.getId() == UNLIT_BRAZIER && !player.getInventory().contains(Tool.TINDER_BOX, 1)) {
		...
		gameObject.kindling(gameObject.kindling() + amount);
		if (gameObject.getId() == UNLIT_BRAZIER)
			gameObject.setId(LIT_BRAZIER);
		int points = amount * 25;
		float multiplier = 2;
		multiplier -= player.getDifficulty().GetDropRate();
		points *= multiplier;
		ChambersOfXeric.addPoints(player, points);
	}
}
