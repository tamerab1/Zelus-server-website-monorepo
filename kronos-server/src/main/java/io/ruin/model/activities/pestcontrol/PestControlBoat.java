...
public final class PestControlBoat {
	...
	/**
	 * Constructs a new {@link PestControlBoat} instance with the respective
	 * properties.
	 *
	 * @param settings
	 */
	...
	public void join(Player player, GameObject gangplank) {
		...
		if (player.getCombat().getLevel() < settings.combatLvReq()) {
			player.dialogue(new MessageDialogue(
					"You require a combat level of at least " + settings.combatLvReq() + " or higher to join this lander."));
			return;
		}

		int offset = gangplank.getDirection() == 0 ? 4 : -4;
		player.lock();
		players.add(player);
		...
		player.getPacketSender().sendString(OVERLAY, 5, "Points: " + player.pestPoints);
		updateOverlay();
	...
	/**
	 * Starts a new {@link PestControlGame} session and inherits the specified
	 * difficulty.
	 */
