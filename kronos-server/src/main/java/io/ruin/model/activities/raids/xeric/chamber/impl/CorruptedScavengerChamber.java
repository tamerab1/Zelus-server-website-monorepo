...
public class CorruptedScavengerChamber extends Chamber {
	...
	public static void register() {
		ObjectAction.register(CLOSED_CHEST, 1, (player, obj) -> player.startEvent(event -> {
			while (true) {
				if (obj.getId() != CLOSED_CHEST)
					return;

				player.animate(832);
				if (Random.get() > (player.getStats().get(StatType.Thieving).currentLevel / 2.0 / 100.0)) {
					event.delay(1);
					player.sendMessage("we are thieving but failed let's try again maybe?");
					return;
				}

				ChestType type = Optional.ofNullable(obj.grubChestType()).orElse(roll());
		...
		World.startEvent(event -> {
			...
			while (true) {
				if (eatCooldown > 0)
					...
					System.out.println("the corrupted scavs health is current " + npc.getHp());
					npc.animate(256);
					grubs--;
					if (grubs == 0)
						trough.restore();
