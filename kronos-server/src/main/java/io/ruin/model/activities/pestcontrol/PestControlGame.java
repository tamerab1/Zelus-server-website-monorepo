...
public class PestControlGame {
	...
	/**
	 * The collection of players involved with this {@link PestControlGame}
	 * instance.
	 */
	 ...
	 * @param participants
	 */
	public void start(ArrayList<Player> participants) throws DynamicMap.DynamicMapBuildException {
		initMap();
		addNpc(SQUIRE_ID, new Position(2660, 2608, 0));
		NPCType def = NPCType.get(settings.voidKnightId());
		...
		players.forEach(player -> {
			...
			player.openInterface(ToplevelComponent.PC, OVERLAY);
	 ...
	 *
	 * @param failed A flag indicating if the participants of this game failed to
	 *               complete the activity.
	 */
	private void end(boolean failed) {
		players.forEach(p -> {
			int rewardedPoints = settings.points() + pestPoints(p);
			p.lock();
	...
	private static int pestPoints(Player player) {
		if (player.isSecondaryGroup(SecondaryGroup.SUPREME_DONATOR)) {
			...
			return 3;
				...
				|| player.isSecondaryGroup(SecondaryGroup.ELITE_DONATOR)
				|| player.isSecondaryGroup(SecondaryGroup.SUPER_DONATOR)) {
			return 2;
	...
	private void pulse() {
		...
		if (cycles % 14 == 0) {
			...
			map.addNpc(new NPC(pestId).spawn(knight.getPosition().getX() + 3, knight.getPosition().getY() + 1,
					knight.getPosition().getZ()));
			map.addNpc(new NPC(pestId).spawn(knight.getPosition().getX() + 1, knight.getPosition().getY() + 2,
					knight.getPosition().getZ()));
			map.addNpc(new NPC(pestId).spawn(knight.getPosition().getX() + 2, knight.getPosition().getY() + 1,
					knight.getPosition().getZ()));
			map.addNpc(new NPC(pestId).spawn(knight.getPosition().getX() + 1, knight.getPosition().getY() + 3,
					knight.getPosition().getZ()));
		}
	...
	/**
	 * Updates the game interface with all information relevant to this Pest Control
	 * game.
	 */
	private void updateOverlay() {
		players.forEach(p -> {
			...
			p.getPacketSender().sendString(OVERLAY, 8,
					p.pestActivityScore > 50 ? "<col=00ff00>" + p.pestActivityScore : "<col=ff0000>" + p.pestActivityScore);
	...
	private void clearActivityOf(Player player) {
		player.closeInterface(ToplevelComponent.PC);
	...
	/**
	 * Randomly chooses a portal out of the collection of portals that are alive and
	 * drops its shield.
	 */
	private void dropRandomShield() {
		Collections.shuffle(portals);
		PestPortal portal = portals.stream().filter(PestPortal::alive).filter(PestPortal::shieldActive).findAny()
				.orElse(null);
		if (portal != null) {
			portal.dropShield();
