...
public class InstanceDialogue {
	...
	private void replaceInstance() {
		player.dialogue(new MessageDialogue(
				"You already have an active instance. It must be destroyed before you can create a new one.<br><br>Continue?"),
				new OptionsDialogue(
						new Option("Yes. Destroy my current instance.", () -> {
							...
							player.dialogue(new MessageDialogue("Instance destroyed. You may now create a new one."),
									new ActionDialogue(this::createMenu));
						}),
	...
	private void confirmCreation() {
		if (type == null) {
			player.dialogue(new MessageDialogue("Please select an instance type first."),
					new ActionDialogue(this::createMenu));
			return;
		}
		Item cost = new Item(COINS_995, type.getCost());
		player.dialogue(new YesNoDialogue("Create Instance?",
				"Creating this instance will cost " + NumberUtils.formatNumber(cost.getAmount()) + " " + cost.getDef().name
						+ ".<br>" +
	...
	private void create() {
		...
		if (!player.getInventory().contains(cost)) {
			if (!player.getBank().contains(cost)) {
				player.dialogue(new MessageDialogue("You cannot afford to create this instance.<br>You will need "
						+ NumberUtils.formatNumber(cost.getAmount()) + " " + cost.getDef().name +
	...
	private void changePrivacy() {
		if (player.getGameMode().isIronMan()) {
			player.dialogue(new MessageDialogue("Ironmen instances are locked to Private mode."),
					new ActionDialogue(this::createMenu));
			return;
		}
		player.dialogue(new OptionsDialogue(
	...
	private void changeType() {
		if (lockedType)
			player.dialogue(new MessageDialogue("Instance type cannot be changed at this location."),
					new ActionDialogue(this::createMenu));
		else {
			OptionScroll.open(player, "Choose an instance type", true,
					Arrays.stream(InstanceType.values()).map(this::toOption).collect(Collectors.toList()));
		}
	}
	...
	private void enterFriend() {
		if (player.getGameMode().isIronMan()) {
			player.dialogue(new MessageDialogue("Ironmen cannot enter other players' instances."),
					new ActionDialogue(this::start));
			return;
		}
		player.nameInput("Enter friend's name:", name -> {
			...
			if (instance == null) {
				...
				player.dialogue(new MessageDialogue("You cannot enter " + name + "'s from here."),
						new ActionDialogue(this::start));
			} else {
				if (instance.getPrivacy() == InstancePrivacy.PUBLIC)
					instance.enter(player);
				else if (instance.getPrivacy() == InstancePrivacy.PRIVATE)
					player.dialogue(new MessageDialogue(name + "'s instance is set to private mode. No one may enter."),
							new ActionDialogue(this::start));
	...
	private void enterOwn() {
		PVMInstance instance = PVMInstance.getByPlayer(player);
		if (instance == null) {
			player.dialogue(new MessageDialogue("You do not have an instance. Use the create option to create one."),
					new ActionDialogue(this::start));
		} else if (lockedType && instance.getType() != type) {
			player.dialogue(
					new MessageDialogue(
							"You cannot enter your " + instance.getType().getName() + " instance from this location."),
					new ActionDialogue(this::start));
		} else {
			instance.enter(player);
			if (instance.getType() == InstanceType.KRAKEN) {
				player.privateKrakenKills = 0;
			}
		}
	}
