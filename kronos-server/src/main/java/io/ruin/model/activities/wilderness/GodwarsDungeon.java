...
public class GodwarsDungeon {
	...
	public static void register() {
		...
		ObjectAction.register(26768, "pass", (player, obj) -> player.startEvent(event -> {
			...
			player.stepAbs(3066, player.getAbsY() > 10148 ? 10147 : 10149, StepType.WALK);
			event.delay(3);
		...
		NPCAction.register(6621, "move", (player, npc) -> player.startEvent(event -> {
			...
			player.stepAbs(player.getAbsX() >= 3054 ? player.getAbsX() - 3 : player.getAbsX() + 3, player.getAbsY(), StepType.WALK);
			npc.stepAbs(npc.getAbsX(), npc.getAbsY() + 1, StepType.WALK);
			event.delay(3);
			npc.stepAbs(npc.getAbsX(), npc.getAbsY() - 1, StepType.WALK);
			player.unlock();
		}));
		...
		ObjectAction.register(26769, 3062, 10131, 0, "use", (player, obj) -> player.startEvent(event -> {
			...
			if (!player.isAt(3062, 10130)) {
				player.stepAbs(3062, 10130, StepType.WALK);
				event.delay(1);
			}
			player.animate(2796);
			event.delay(2);
		...
		ObjectAction.register(26769, 3035, 10158, 0, "use", (player, obj) -> player.startEvent(event -> {
			player.lock();
			if (!player.isAt(3062, 10130)) {
				player.stepAbs(3034, 10158, StepType.WALK);
				event.delay(1);
			}
			player.animate(2796);
			event.delay(2);
