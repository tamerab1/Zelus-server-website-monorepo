	...
	public void init() {
		...
		npc.addEvent(event -> {
			...
			if (npc.getId() == 7541 || npc.getId() == 7542 || npc.getId() == 7543 || npc.getId() == 7544 || npc.getId() == 7545) {
				double factor = 1.1 + (0.20 * ChambersOfXeric.getPartySize(npc));
			...
			for (Player player : npc.localPlayers()) {
				...
				if ((side.deltaX != 0 && player.getAbsX() - src.getX() == side.deltaX) ||
					...
					if (player.getPrayer().isActive(Prayer.PROTECT_FROM_MELEE))
						maxDamage = 24;
					damagedPlayer = true;
					player.hit(new Hit(npc, AttackStyle.CRUSH).randDamage(15, maxDamage).ignorePrayer().ignoreDefence().delay(0));
				}
			}
		});
		return true;
