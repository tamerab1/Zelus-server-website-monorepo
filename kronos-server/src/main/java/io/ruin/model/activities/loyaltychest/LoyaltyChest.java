...
public class LoyaltyChest {
	...
	public static void register() {
		LoginListener.register(player -> {
			if (!player.newPlayer) {
				SecurityPin.onLogin(player);
			}
			if (player.loyaltyTimer < System.currentTimeMillis()) {
				player.sendFilteredMessage(Color.COOL_BLUE
					.wrap("Thank you for playing " + World.type.getWorldName() + "." + " We appreciate your Loyalty!"));
			}
		});
	}

	private static void openChest(Player player) {
		player.openInterface(ToplevelComponent.MAINMODAL, Interface.LOYALTY_CHEST);
		Item[] rewards = ECO_REWARDS;
		player.getPacketSender().sendLoyaltyRewards(player.loyaltyChestReward, player.loyaltyChestSpree,
			player.highestLoyaltyChestSpree, 1, rewards);
	}

	private static void claimReward(Player player, int requestedDayReward) {
		if (player.loyaltyTimer > System.currentTimeMillis()) {
			player.dialogue(new ItemDialogue().one(744, "You have to wait " + Color.COOL_BLUE.wrap(getRemainingTime(player))
				+ " until you're able to open the Loyalty Chest.").lineHeight(24));
			return;
		}
		if (recentPlayers.contains(player.getIpInt(), System.currentTimeMillis(), 720L)) {
			player.dialogue(
				new ItemDialogue().one(744, "You have already claimed your reward in the past 24 hours.").lineHeight(24));
			return;
		}
		if (!player.getInventory().hasFreeSlots(2)) {
			player.dialogue(
				new ItemDialogue().one(744, "You need at least 2 free inventory slot to claim your loyalty rewards."));
			return;
		}
