...
public class CrypticClue extends Clue {
	...
	public void open(Player player) {
		...
		player.openInterface(ToplevelComponent.MAINMODAL, 203);
	}
	...
	public static void register() {
		...
		registerSearch("Search a bookcase in the Wizards tower.", 12539, 3113, 3159, 0, ClueType.EASY);
		registerSearch("Search the bookcase in the monastery.", 380, 3054, 3484, 0, ClueType.EASY);
		...
		registerSearch("This village has a problem with cartloads of the undead. Try checking the bookcase to find an answer.", 394, 2833, 2992, 0, ClueType.HARD);
		...
		registerTalk(4626, "In the place Duke Horacio calls home, talk to a man with a hat dropped by goblins.", ClueType.BEGINNER);
		registerTalk(3063, "In a village of barbarians, I am the one who guards the village from up high.", ClueType.BEGINNER);
		registerTalk(4642, "Near the open desert I reside, to get past me you must abide. Go forward if you dare, for when you pass me, you'll be sweating by your hair.", ClueType.BEGINNER);
		registerTalk(2812, "A reck you say Let's pray there aren't any ghosts.", ClueType.ELITE);
		registerTalk(2577, "A bag belt only?', he asked his balding brothers.", ClueType.HARD);
		registerTalk(5315, "Dobson is my last name, and with gardening I seek fame.", ClueType.ELITE);
		registerTalk(669, "Generally speaking, his nose was very bent.", ClueType.HARD);
		registerTalk(2345, "Gold I see, yet gold I require. Give me 875 if death you desire.", ClueType.HARD);
		registerTalk(6524, "His bark is worse than his bite.", ClueType.ELITE);
		registerTalk(3199, "I burn between heroes and legends.", ClueType.ELITE);
		registerTalk(2891, "If a man carried my burden, he would break his back. I am not rich, but leave silver in my track. Speak to the keeper of my trail.", ClueType.HARD);
		registerTalk(2670, "I watch the sea. I watch you fish. I watch your tree.", ClueType.HARD);
		registerTalk(2461, "I am the one who watches the giants. The giants in turn watch me. I watch with two while they watch with one. Come seek where I may be.", ClueType.ELITE);
		registerTalk(3352, "Identify the back of this over-acting brother. (He's a long way from home.)", ClueType.HARD);
		registerTalk(3503, "My name is like a tree, yet it is spelt with a 'g'. Come see the fur which is right near me.", ClueType.HARD);
