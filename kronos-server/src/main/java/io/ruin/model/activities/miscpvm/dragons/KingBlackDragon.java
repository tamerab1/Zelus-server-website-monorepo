...
public class KingBlackDragon extends NPCCombat {
	...
	public boolean playerUsedOverheads = false;

	private void resetCombatAchievementVariables() {
		playerUsedOverheads = false;
	}
	...
	public void process() {
		if (target == null && npc.getHp() >= npc.getMaxHp())
			resetCombatAchievementVariables();

		if (target != null && target.player != null) {
			if (target.player.getPrayer().isActive(Prayer.PROTECT_FROM_MISSILES)
				|| target.player.getPrayer().isActive(Prayer.PROTECT_FROM_MAGIC) || target.player.getPrayer().isActive(Prayer.PROTECT_FROM_MELEE))
				playerUsedOverheads = true;
		}
	}
	...
	private void fire(Projectile proj, int minMaxDamage) {
		...
		if (target.isPlayer() && target.player.getEquipment().get(Equipment.SLOT_FEET) != null && target.player.getEquipment().get(Equipment.SLOT_FEET).getId() == 30505)
			minMaxDamage = 11;
