...
public class WintertodtActions {
	...
	public static void register() {
		...
		for (Brazier b : Wintertodt.BRAZIERS) {
			ObjectAction.register(b.getObject(), 1, (player, obj) -> {
				...
				if (obj.getId() == Wintertodt.BURNING_BRAZIER) {
					feed(player, b);
				} else if (obj.getId() == Wintertodt.EMPTY_BRAZIER) {
					light(player, b);
				} else {
					fix(player, b);
				}
	...
	private static void feed(Player player, Brazier b) {
		...
		player.startEvent(event -> {
			while (true) {
				if (b.getObject()
					.getId() != Wintertodt.BURNING_BRAZIER) {
	...
	private static void fix(Player player, Brazier b) {
		...
		if (hammer == null && imcandoHammer == null && !wieldingDragonWarhammer && !wieldingImcandoHammer) {
