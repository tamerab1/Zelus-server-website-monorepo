...
public class CorporealBeast extends NPCCombat {
	...
	public boolean hotOnYourFeet = true;
	...
	public void init() {
		...
		npc.deathStartListener = (entity, killer, killHit) -> {
			...
			for (Player player : npc.localPlayers())
				VarPlayerRepository.CORPOREAL_BEAST_DAMAGE.set(player, 0);
		};
		npc.addEvent(event -> {
			while (true) {
	...
	private void sendCore() {
		...
		core.deathEndListener = (entity, killer, killHit) -> {
			entity.npc.remove();
			hotOnYourFeet = false;
		};
		core.addEvent(event -> {
	...
	private boolean coreDrain() {
		...
		for (Player player : npc.localPlayers()) {
			if (canAttack(player) && Misc.getDistance(player.getPosition(), core.getPosition()) <= 1 && ProjectileRoute.allow(npc, core)) {
				...
				hotOnYourFeet = false;
				player.sendFilteredMessage("The dark energy core drains life from you and transfers it to its master!");
			}
		}
	...
	public List<String> hitters = new ArrayList<>();
	...
	private void postDefend(Hit hit) {
		if (hit.attacker != null && hit.attacker.player != null && hit.attackStyle != null) {
			...
			if (weapon == null || !usingSpear(hit.attacker.player)) {
				hit.damage *= 0.5;
				hit.attacker.player.sendMessage("The Corporeal Beast resists from your weapon.");
			}
		}
		if (hit.damage > 100 && !hit.isBlocked()) // dmg cap
			hit.damage = 100;
		if (hit.attacker != null && hit.attacker.player != null && hit.damage > 0) {
			if (!hitters.contains(hit.attacker.player.getName()))
				hitters.add(hit.attacker.player.getName());
			int damage = Math.min(2047, VarPlayerRepository.CORPOREAL_BEAST_DAMAGE.get(hit.attacker.player) + hit.damage);
			VarPlayerRepository.CORPOREAL_BEAST_DAMAGE.set(hit.attacker.player, damage);
		}
	}
	...
	private boolean usingSpear(Player player) {
		if (player.getEquipment().get(Equipment.SLOT_WEAPON) == null)
			return false;
		if (player.getEquipment().get(Equipment.SLOT_WEAPON).getId() == ItemID.SUNLIGHT_SPEAR)
			return true;
		if (player.getEquipment().get(Equipment.SLOT_WEAPON).getId() == ItemID.OSMUMTENS_FANG)
			return true;
		if (player.getEquipment().get(Equipment.SLOT_WEAPON).getId() == 27246) //Osmumten's fang (or)
			return true;
		if (player.getEquipment().get(Equipment.SLOT_WEAPON).getId() == 59527) //Osmumten's scythe
			return true;
