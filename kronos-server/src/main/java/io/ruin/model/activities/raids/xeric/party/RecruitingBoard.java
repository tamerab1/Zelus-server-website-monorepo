...
public class RecruitingBoard {
	...
	private static final Bounds BOUNDS2 = new Bounds(3640, 3201, 3680, 3235, 0);
	...
	public static void register() {
		...
		InterfaceHandler.register(Interface.RAIDING_RECRUITING_BOARD, h -> {
			h.actions[3] = (DefaultAction) (player, option, slot, itemId) -> {
				...
				if (slot == 1)
					if (VarPlayerRepository.RAIDS_PARTY.get(player) == -1)
		...
		InterfaceHandler.register(Interface.RAIDING_PARTY, h -> {
			h.actions[3] = (DefaultAction) (player, option, slot, itemId) -> {
				...
				if (slot == 4)
					displayRecruitingBoard(player);
				if (slot == 5)
					setPreferredSetting(player, "party size", VarPlayerRepository.RAIDS_PREFERRED_PARTY_SIZE, 1, 100);
				if (slot == 6)
					setPreferredSetting(player, "combat level", VarPlayerRepository.RAIDS_PREFERRED_COMBAT_LEVEL, 4, 126);
				if (slot == 7)
					setPreferredSetting(player, "skill total", VarPlayerRepository.RAIDS_PREFERRED_SKILL_TOTAL, 33, 2277);
				if (slot == 8)
		...
		MapListener.registerBounds(BOUNDS2)
			.onEnter(player -> player.setAction(1, PlayerAction.INVITE))
			.onExit((player, logout) -> {
				if (logout && player.raidsParty != null)
					leaveParty(player);
	...
	private static void displayRecruitingBoard(Player player) {
		player.openInterface(ToplevelComponent.MAINMODAL, Interface.RAIDING_RECRUITING_BOARD);
		for (int i = 0; i < 40; i++)
			if (i < ADVERTISED_PARTIES.size() && ADVERTISED_PARTIES.get(i) != null) {
				...
				String partyInformation = "<col=ffffff>" + partyHost.getName() + "</col>" + "|" +
					...
					VarPlayerRepository.RAIDS_PREFERRED_PARTY_SIZE.get(partyHost) + "|" +
					VarPlayerRepository.RAIDS_PREFERRED_COMBAT_LEVEL.get(partyHost) + "|" +
					VarPlayerRepository.RAIDS_PREFERRED_SKILL_TOTAL.get(partyHost) + "|" +
	...
	public static void createParty(Player player, boolean displayInterface) {
		VarPlayerRepository.RAIDS_PARTY.set(player, player.getIndex());
	...
	public static void joinParty(Player player, Party party) {
		...
		party.getLeader().dialogue(new OptionsDialogue(player.getName() + " (" + player.getCombat().getLevel() + " combat, " + NumberUtils.formatNumber(player.getStats().totalLevel) + " total level) wishes to join your party.",
			new Option("Accept", () -> {
				...
				displayParty(player, player.raidsParty);
				VarPlayerRepository.RAIDS_PARTY.set(player, leader.getIndex());
	...
	public static void invite(Player player, Player target) {
		...
		if (player.raidsParty.getMembers().size() >= 10) {
			target.sendMessage("Your party is full.");
			return;
		}
	...
	public static void disbandParty(Player player) {
		...
		if (player.raidsParty.getLeader() == player) {
			...
			player.raidsParty.getMembers().forEach(member -> {
				...
				VarPlayerRepository.RAIDS_PARTY.set(member, -1);
	...
	private static void displayParty(Player player, Party party) {
		...
		player.openInterface(ToplevelComponent.MAINMODAL, 507);
	...
	private static void copyPartyPreferences(Player player, Party party) {
		VarPlayerRepository.RAIDS_PREFERRED_PARTY_SIZE.set(player, VarPlayerRepository.RAIDS_PREFERRED_PARTY_SIZE.get(party.getLeader()));
		VarPlayerRepository.RAIDS_PREFERRED_COMBAT_LEVEL.set(player, VarPlayerRepository.RAIDS_PREFERRED_COMBAT_LEVEL.get(party.getLeader()));
		VarPlayerRepository.RAIDS_PREFERRED_SKILL_TOTAL.set(player, VarPlayerRepository.RAIDS_PREFERRED_SKILL_TOTAL.get(party.getLeader()));
	}

	public static void resetPartyPreferences(Player player) {
		VarPlayerRepository.RAIDS_PREFERRED_PARTY_SIZE.set(player, 0);
		VarPlayerRepository.RAIDS_PREFERRED_COMBAT_LEVEL.set(player, 0);
		VarPlayerRepository.RAIDS_PREFERRED_SKILL_TOTAL.set(player, 0);
	}

	private static void setPreferredSetting(Player player, String setting, VarPlayerRepository config, int minValue, int maxValue) {
		player.closeInterface(ToplevelComponent.MAINMODAL);
