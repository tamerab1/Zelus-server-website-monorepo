...
public class Skotizo extends NPCCombat { // Kourend catacombs boss
	...
	public boolean equippedDemonBaneWeapon = false;
	public boolean damagedPlayer = false;

	private NPC[] altars;
	private GameObject[] altarObjects;
	public int altarsKilled = 0;
	...
	public boolean allAltarsDorment() {
		for (NPC altar : altars) {
			if (altar.getId() == AWAKENED_ALTAR) {
				return false;
			}
		}
		return true;
	...
	public static void startFight(Player player, GameObject altar) {
		DynamicMap map;
		try {
			map = new DynamicMap().build(9048, 0);
		...
		player.startEvent(event -> {
			...
			event.delay(3);
			NPC skotizo = new NPC(7286).spawn(map.swRegion.baseX + SKOTIZO_SPAWN[0], map.swRegion.baseY + SKOTIZO_SPAWN[1], 0,
					Direction.SOUTH, 8);
			map.addNpc(skotizo);
			player.getMovement().teleport(map.convertX(2272), map.convertY(5658));
			player.currentDynamicMap = map;
			player.inDynamicMap = true;
			player.openInterface(ToplevelComponent.WILDERNESS_OVERLAY, 308);
			map.assignListener(player).onExit((p, logout) -> {
				if (logout) {
					...
					p.closeInterface(ToplevelComponent.WILDERNESS_OVERLAY);
				}
				map.destroy();
			});
	...
	public void init() {
		...
		for (int i = 0; i < altars.length; i++) {
			...
			altarObjects[i] = GameObject.spawn(OBJ_DORMANT_ALTAR, altars[i].getAbsX(), altars[i].getAbsY(),
					altars[i].getHeight(), 10, i);
		...
		npc.hitListener = new HitListener().preDefend(hit -> hit.boostDefence(0.30 * activeAltarCount()));
	...
	public void process() {
		...
		if (player.getEquipment().get(Equipment.SLOT_WEAPON) == null)
			return;
		if (player.getEquipment().get(Equipment.SLOT_WEAPON).getId() == ItemID.ARCLIGHT ||
				player.getEquipment().get(Equipment.SLOT_WEAPON).getId() == ItemID.DARKLIGHT)
			equippedDemonBaneWeapon = true;
	...
	private Position getAbsolute(int localX, int localY) {
		return new Position(npc.getPosition().getRegion().baseX + localX, npc.getPosition().getRegion().baseY + localY,
