...
public class GodwarsObstacle {
	...
	public static void register() {
		...
		ObjectAction.register(26461, 2851, 5333, 2, "open", (player, obj) -> {
			...
			boolean hammer = player.getInventory().hasId(Tool.HAMMER) || player.getInventory().hasId(Tool.IMCANDO_HAMMER) || player.getEquipment().hasId(25644) || player.getInventory().hasId(ItemID.DRAGON_WARHAMMER) || player.getEquipment().hasId(ItemID.DRAGON_WARHAMMER) || player.getEquipment().hasId(ItemID.CORRUPTED_WARHAMMER) || player.getInventory().hasId(ItemID.CORRUPTED_WARHAMMER);
			if (!hammer) {
			...
			if (player.getAbsX() >= 2851) {
				player.startEvent(event -> {
					...
					event.delay(2);
					obj.remove();
					player.stepAbs(2850, 5333, StepType.WALK);
					event.delay(2);
					obj.restore();
					player.unlock();
				});
				...
				player.startEvent(event -> {
					player.lock();
					obj.remove();
					player.stepAbs(2851, 5333, StepType.WALK);
					event.delay(2);
					obj.restore();
					player.unlock();
				});
		...
		ObjectAction.register(26561, 2913, 5300, 2, 1, (player, obj) -> {
			if (VarPlayerRepository.GODWARS_SARADOMIN_FIRST_ROPE.get(player) == 0) {
				...
				rope.remove();
				VarPlayerRepository.GODWARS_SARADOMIN_FIRST_ROPE.set(player, 1);
			}
		});
		...
		ObjectAction.register(26562, 2920, 5274, 1, 1, (player, obj) -> {
			if (VarPlayerRepository.GODWARS_SARADOMIN_SECOND_ROPE.get(player) == 0) {
				Item rope = player.getInventory().findItem(954);
				if (rope == null) {
					player.sendFilteredMessage("You aren't carrying a rope with you.");
					return;
				...
				rope.remove();
				VarPlayerRepository.GODWARS_SARADOMIN_SECOND_ROPE.set(player, 1);
			}
		});
