...
public class Zulrah extends NPCCombat { // this is a mess
	...
	public static void register() {
		...
		ObjectAction.register(10068, 2, (player, obj) -> enter(player));
	...
	public static void enter(Player player) {
		...
		player.startEvent(event -> {
			...
			event.delay(1);
			player.dialogue(new MessageDialogue("The priestess rows you to Zulrah's shrine,<br>then hurriedly paddles away.")
					.hideContinue());
			event.delay(2);
	...
	private static final Phase[][] ROTATIONS = {
			{ new Phase(Form.NORMAL, Location.CENTER).setSafe(8), new Phase(Form.MAGMA, Location.CENTER).setSafe(8),
					new Phase(Form.TANZANITE, Location.CENTER).setSafe(8), new Phase(Form.NORMAL, Location.SOUTH).westSafe(),
					new Phase(Form.MAGMA, Location.CENTER).setSafe(1), new Phase(Form.TANZANITE, Location.WEST).setSafe(1),
					new Phase(Form.NORMAL, Location.SOUTH).eastSafe(), new Phase(Form.TANZANITE, Location.SOUTH).westSafe(),
					new Phase(Form.JAD, Location.WEST).setSafe(0), new Phase(Form.MAGMA, Location.CENTER).setSafe(0).setSafe(1) },
			{ new Phase(Form.NORMAL, Location.CENTER).setSafe(8), new Phase(Form.MAGMA, Location.CENTER).setSafe(8),
					new Phase(Form.TANZANITE, Location.CENTER).setSafe(8), new Phase(Form.NORMAL, Location.WEST).westSafe(),
					new Phase(Form.TANZANITE, Location.SOUTH).setSafe(1), new Phase(Form.MAGMA, Location.CENTER).setSafe(1),
					new Phase(Form.NORMAL, Location.EAST).setSafe(4), new Phase(Form.TANZANITE, Location.SOUTH).westSafe(),
					new Phase(Form.JAD, Location.WEST).setSafe(0), new Phase(Form.MAGMA, Location.CENTER).setSafe(0).setSafe(1) },
			{ new Phase(Form.NORMAL, Location.CENTER).setSafe(8), new Phase(Form.NORMAL, Location.EAST).setSafe(8),
					new Phase(Form.MAGMA, Location.CENTER).eastSafe(), new Phase(Form.TANZANITE, Location.WEST).centerSafe(),
					new Phase(Form.NORMAL, Location.SOUTH).centerSafe(),
					new Phase(Form.TANZANITE, Location.EAST).setSafe(4).setSafe(5),
					new Phase(Form.NORMAL, Location.CENTER).westSafe(), new Phase(Form.NORMAL, Location.WEST).westSafe(),
					new Phase(Form.TANZANITE, Location.CENTER).eastSafe(), new Phase(Form.JAD, Location.EAST).eastSafe(),
					new Phase(Form.TANZANITE, Location.CENTER).eastSafe() },
			{ new Phase(Form.NORMAL, Location.CENTER).setSafe(8), new Phase(Form.TANZANITE, Location.EAST).setSafe(8),
					new Phase(Form.NORMAL, Location.SOUTH).setSafe(1), new Phase(Form.TANZANITE, Location.WEST).setSafe(1),
					new Phase(Form.MAGMA, Location.CENTER).eastSafe(), new Phase(Form.NORMAL, Location.EAST).eastSafe(),
					new Phase(Form.NORMAL, Location.SOUTH).centerSafe(), new Phase(Form.TANZANITE, Location.WEST).westSafe(),
					new Phase(Form.NORMAL, Location.CENTER).eastSafe(), new Phase(Form.TANZANITE, Location.CENTER).eastSafe(),
					new Phase(Form.JAD, Location.EAST).eastSafe(), new Phase(Form.TANZANITE, Location.CENTER).eastSafe() },
	};
	...
	private final int MAX_SNAKELINGS = 3;
	private final List<NPC> snaklingList = new ArrayList<>(MAX_SNAKELINGS);
	...
	private void nextPhase() {
		...
		if (target.player.debug)
			target.player.sendMessage("Rotation " + rotationId + ", Phase ["
					+ Color.COOL_BLUE.wrap(String.valueOf(currentPhase + 1 % rotation.length)) + "]: "
					+ Color.RED.wrap(next.form.toString()) + ", " + Color.RED.wrap(next.location.toString()));
	...
	private boolean fumes() {
		...
		if (possible.contains(spot + 1)) { // if the spot next to the one we just spawned clouds on is free, spawn a second
																				// one there
			spot = spot + 1;
	...
	private boolean hasEmptyFumesSpot() {
		for (int i = 0; i < getPhase().safeFumes.length; i++) {
			if (!getPhase().safeFumes[i] // our current form allows this spawn
					&& Tile.getObject(11700, npc.getSpawnPosition().getX() + fumeOffsets[i][0],
							npc.getSpawnPosition().getY() + fumeOffsets[i][1], npc.getHeight(), 10, -1) == null) { // and it's not
																																																			// already spawned
				return true;
			}
		}
	...
	private boolean checkSnakelings() {
		for (int i = 0; i < MAX_SNAKELINGS; i++) {
			if (snaklingList.size() < 3 && Random.rollPercent(20)) {
				if (spawnSnakeling())
					return true;
			}
		}
	...
	private boolean spawnSnakeling() {
		if (target == null)
			return false;
		...
		npc.addEvent(event -> {
			...
			if (isDead() || npc.isRemoved() || target == null)
				return;
			var snakeling = new NPC(2045).spawn(x, y, npc.getHeight(), 0);
			snakeling.targetPlayer(target.player, false);
			snakeling.attackTargetPlayer();
			snaklingList.add(snakeling);
		});
		return true;
	}
	...
	private void magicAttack() {
		...
		hit.postDamage(t -> {
			if ((getPhase().form == Form.TANZANITE || getPhase().form == Form.JAD) && target != null && target.isPlayer()
	...
	private void rangedAttack() {
		...
		hit.postDamage(t -> {
			if ((getPhase().form == Form.NORMAL || getPhase().form == Form.JAD) && target != null && target.isPlayer()
					&& !target.player.getPrayer().isActive(Prayer.PROTECT_FROM_MISSILES))
				damagedPlayer = true;
		});
		target.hit(hit);
	}
	...
	public void init() {
		...
		npc.deathStartListener = (entity, killer, killHit) -> {
			snaklingList.stream()
				...
				.forEach(snake -> snake.getCombat().startDeath(killHit));
		};
	...
	public boolean attack() {
		if (transitioning.isDelayed())
			return false;
		if (++attacks >= (getPhase().form == Form.JAD ? 12 : 8)
				&& !(getPhase().form == Form.MAGMA && meleeTarget != null)) {
			nextPhase();
			attacks = 0;
			return false;
		}
		if (getPhase().form == Form.NORMAL || getPhase().form == Form.TANZANITE
				|| (getPhase().form == Form.MAGMA && meleeTarget == null)) {
	...
	private void onDeath(Player player) {
		GameObject.spawn(11701, npc.getSpawnPosition().getX() + 2, npc.getSpawnPosition().getY() - 3,
				npc.getSpawnPosition().getZ(), 10, 0); // exit
		snaklingList.forEach(NPC::remove);
		npc.remove();
