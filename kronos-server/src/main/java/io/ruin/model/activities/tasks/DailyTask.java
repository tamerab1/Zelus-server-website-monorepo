...
public enum DailyTask {
	...
	public static void assignTasks(Player player) {
		...
		DailyTask selected;
		if (Math.random() < 0.5 && !player.getGameMode().isIronMan()) {
		...
		player.sendMessage(Color.RED.wrap("Click on a task in the Quest tab for more details."));
	}
...
//    }
	...
	public static void register() {
		LoginListener.register(p -> {
			for (int i = 0; i < p.currentDailyTasks.length; i++) {
				if (p.currentDailyTasks[i] == null) {
					DailyTasks.assignDailyTasks(p, true);
					break;
				}
			}
			Instant instant = Instant.ofEpochSecond(p.lastDailyTaskResetInEpoch);
			// Convert Instant to LocalDate
			LocalDate dateFromTimestamp = instant.atZone(ZoneId.systemDefault()).toLocalDate();
			// Get today's date
			LocalDate today = LocalDate.now();
			// Compare the dates
			boolean isBeforeToday = dateFromTimestamp.isBefore(today);
			if (isBeforeToday)
				DailyTasks.assignDailyTasks(p, false);
		});

	}

	public static class TaskEntry extends QuestTabEntry {

		public static final QuestTabEntry[] ENTRIES = {
			new TaskEntry(0),
			new TaskEntry(1),
			new TaskEntry(2),
		};
	...
	public static class PointsEntry extends QuestTabEntry {
