...
public class Scorpia extends NPCCombat {
	...
	public static void register() {
		NPCType.get(6615).ignoreOccupiedTiles = true; // so she doesnt get stuck on her babies
		NPCType.get(6617).ignoreOccupiedTiles = true;
	}

	private boolean spawnedGuardians = false;
	public boolean dealtDamage = false;
	public boolean killedAnyGuardians = false;
	...
	private void spawnGuardians(Hit hit) {
		if (!spawnedGuardians && npc.getHp() < 130) {
	...
	public boolean attack() {
		...
		if (Random.rollDie(1, 1))
			target.player.getPrayer().drain(Random.get(2, 2));
	...
	public void process() {
		for (NPC guardian : guardians) {
			if (guardian != null && guardian.getHp() < 1) {
				killedAnyGuardians = true;
				break;
			}
		}
	}
