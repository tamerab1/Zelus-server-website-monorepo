...
import io.ruin.model.activities.fightcaves.monsters.TzTokJad;
...
public class FightCaves {
	...
	public static void register() {
		...
		LoginListener.register(p -> {
			p.getFarming().patches.forEach((id, patch) -> {
				patch.data.getConfig().set(p, patch.getVarpbitValue());
			});
			if (p.toaInvocationFavourites.isEmpty()) {
				for (int i = 1; i < 6; i++) {
					p.toaInvocationFavourites.put(i, null);
				}
			}
			TombsOfAmascut.confiscateItems(p);
	...
	private static void join(Player player, int wave, boolean practice) {
		// probably will always be true, but just to be safe?
		if (player.fightCaves == null) {
			try {
	...
	private transient int waveRotationOffset, spawnRotationOffset;
	private transient ActivityTimer timer;
	private transient DynamicMap map;
	private transient boolean jadHealers, killedJad;
	...
	private void start(boolean login) throws DynamicMap.DynamicMapBuildException {
		...
		map.addEvent(e -> {
			if (wave == 63) {
				...
				if (!login)
					DumbRoute.route(player, map.convertX(C.getX()), map.convertY(C.getY()));
				player.dialogue(
						new NPCDialogue(HOST, "You're on your own now JalYt, prepare to fight for your life!").animate(615));
				e.delay(login ? 10 : 5);
	...
	private void stop(boolean logout) {
		...
		if (map != null && (map.getNpcs() != null || !map.getNpcs().isEmpty()))
			...
			if (lastWave == 0) {
				...
				if (!killedJad) {
					if (!player.isSapphire()) {
						player.dialogue(
								new NPCDialogue(HOST, "Well done in the cave, Tokkul has been added to your bank.").animate(588));
						player.getBank().add(6529, tokkul);
						player.sendMessage("Tokkul has been added to your bank.");
					} else {
						player.dialogue(new NPCDialogue(HOST, "Well I suppose you tried... better luck next time."));
					}
				} else {
					player.dialogue(new NPCDialogue(HOST,
							"You have defeated TzTok-Jad, I am most impressed! Please accept this gift. Give cape back to me if you not want it.")
									.animate(615));
					player.getInventory().addOrDrop(6570, 1);
					player.getInventory().addOrDrop(6529, tokkul + 4000);
					player.sendMessage("Tokkul has been added to your bank.");
					player.addToCollectionLog(new Item(6570, 1));
				}
			}
			player.fightCaves = null;
	...
	private void handleDeath(NPC npc) {
		...
		if (npc.getId() == TZ_TOK_JAD) {
			if (!finishPractice()) {
				TzTokJad jad = (TzTokJad) npc.getCombat();
				if (jad.offPrayerHit) {
					Objects.requireNonNull(player.combatAchievementsList
							.get(player.getCombatAchievementIndexByOrdinal(CombatAchievements.A_NEAR_MISS.ordinal()))
							.getCombatAchievement()).check(player);
				}
				Objects.requireNonNull(player.combatAchievementsList
						.get(player.getCombatAchievementIndexByOrdinal(CombatAchievements.TZTOK_JAD_ADEPT.ordinal()))
						.getCombatAchievement()).check(player);
				killedJad = true;
				player.currentDynamicMap = null;
				player.inDynamicMap = false;
