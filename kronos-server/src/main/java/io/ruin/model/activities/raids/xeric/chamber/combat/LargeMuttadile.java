...
public class LargeMuttadile extends NPCCombat {
	...
	private void meleeAttack() {
		...
		target.hit(new Hit(npc, AttackStyle.CRUSH).randDamage(maxDamage).ignorePrayer().ignoreDefence());
	}
	...
	private void stompAttack() {
		...
		if (target.player != null && target.player.getPrayer().isActive(Prayer.PROTECT_FROM_MELEE))
			maxDamage /= 2;
		target.hit(new Hit(npc, AttackStyle.CRUSH).randDamage(maxDamage / 2, maxDamage).ignorePrayer().ignoreDefence());
	}
	...
	private void rangedAttack() {
		...
		npc.startEvent(event -> event.delay(1));
		Hit hit = new Hit(npc, AttackStyle.RANGED).randDamage(maxDamage).clientDelay(delay).ignorePrayer().ignoreDefence();
		target.hit(hit);
	}
	...
	private void magicAttack() {
		...
		npc.startEvent(event -> event.delay(1));
		Hit hit = new Hit(npc, AttackStyle.MAGIC).randDamage(maxDamage).clientDelay(delay).ignorePrayer().ignoreDefence();
		target.hit(hit);
	}
