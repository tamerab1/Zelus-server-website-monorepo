...
public class Venenatis extends NPCCombat {
	...
	private final List<Player> damagingPlayers = new ArrayList<>();
	...
	public void startDeath(Hit killHit) {
		...
		npc.getPosition().getRegion().players.forEach(p -> {
			p.resetActions(false, false, true);
			if (p != killHit.attacker && damagingPlayers.contains(p)) {
				handleNewDrop(p, getNpc().getId(), p.getPosition());
			}
		});
	}
	...
	private void magicAttack() {
		...
		if (npc.inMulti()) {
			...
			projectileAttack(MAGIC_PROJECTILE, 9990, AttackStyle.MAGIC, 50);
		}
	}
