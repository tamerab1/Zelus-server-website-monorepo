...
public class Barrows {
	...
	private static void enteredMain(Player player) {
		player.openInterface(ToplevelComponent.WILDERNESS_OVERLAY, Interface.BARROWS);
	}
	...
	private static void register(BarrowsBrother brother, int digX, int digY, int cryptX, int cryptY, int stairsId,
			int sarcophagusId) {
	...
	private static void loot(Player player) {
		...
		if (!player.barrowsChestBrother.isKilled(player)) {
			player.barrowsChestBrother.spawn(player, true);
			return;
		}
		if (VarPlayerRepository.BARROWS_CHEST.get(player) == 0) {
		...
		player.startEvent(e -> {
			...
			player.openInterface(ToplevelComponent.MAINMODAL, Interface.BARROWS_LOOT);
			player.getPacketSender().sendIfEvents(Interface.BARROWS_LOOT, 3, 0, 8, 1024);
			loot.sendUpdates();
			...
			DailyTasks.handleTaskDecrement(player, "barrows");
			...
			VarPlayerRepository.BARROWS_CHEST.set(player, 0);
	...
	private static void toggleChest(Player player, boolean open) {
		player.animate(535);
		VarPlayerRepository.BARROWS_CHEST.set(player, open ? 1 : 0);
	}
