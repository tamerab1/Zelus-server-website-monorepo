...
public enum Fishing {
		...
		player.startEvent(event -> {
			...
			while (true) {
				...
				if (success(player, fish.levelReq)) {
					...
					PerkTaskHandler.handleGatherResource(player, fish.itemId, 1);
					DailyTasks.handleItemObtained(player, fish.itemId, StatType.Fishing);
					DailyTasks.handleTaskDecrementItemId(player, fish.itemId);
	...
	private static boolean success(Player player, int levelReq) {
		...
		if (player.getPlayerPerkHandler().getActivePerks(player).contains(Perks.SPEED_FISHER)) {
			...
			SpeedFisher c = (SpeedFisher) player.getPlayerPerkHandler().
				getActivePerks(player).get(perkIndex).getPerk(player);
			chance += c.getFishChance();
		}
