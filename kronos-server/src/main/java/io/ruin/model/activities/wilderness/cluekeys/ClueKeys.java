...
public class ClueKeys {
	...
	public static boolean DemonRegistered = false;
	...
	public static void register() {
		Arrays.stream(KeyData.values()).forEach(keyData -> Spade.registerDig(keyData.getPosition().getX(), keyData.getPosition().getY(), keyData.getPosition().getZ(), player -> {
			...
			if (!DemonRegistered) {
				...
				DemonRegistered = true;
			} else {
				player.sendMessage("You already have a demon to attack! kill that one first!");
	...
	public static void registerDemon(Player player) {
		NPC demon = new NPC(NPC_ID).spawn(player.getPosition().getX(), player.getPosition().getY(), player.getPosition().getZ()).targetPlayer(player, true);
