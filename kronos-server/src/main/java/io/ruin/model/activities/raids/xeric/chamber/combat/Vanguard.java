...
public class Vanguard extends NPCCombat {
	...
	private boolean magicAttack() {
		...
		for (Position pos : targets) {
			...
			npc.addEvent(event -> {
				event.delay(2);
				npc.localPlayers().stream().filter(p -> p.getPosition().distance(pos) < 1).forEach(p -> {
					int dmg = p.getPrayer().isActive(Prayer.PROTECT_FROM_MAGIC) ? 11 : 22;
					p.hit(new Hit(npc, AttackStyle.MAGIC).randDamage(dmg).ignoreDefence().ignorePrayer());
				});
			});
		}
		return true;
	...
	private boolean rangedAttack() {
		...
		for (int i = 0; i < 2; i++) {
			...
			npc.addEvent(event -> {
				...
				event.delay(1);
				npc.localPlayers().stream().filter(p -> p.getPosition().distance(position) < 1).forEach(p -> {
					int damage = p.getPrayer().isActive(Prayer.PROTECT_FROM_MISSILES) ? 19 : 8;
					p.hit(new Hit(npc, AttackStyle.RANGED).randDamage(damage).clientDelay(delay).ignoreDefence().ignorePrayer());
				});
			});
		}
		return true;
	...
	private boolean meleeAttack() {
		if (!withinDistance(1))
			return false;
		npc.animate(getInfo().attack_animation);
		int dmg = target.player.getPrayer().isActive(Prayer.PROTECT_FROM_MELEE) ? 7 : 18;
		for (int i = 0; i < 3; i++) {
			target.hit(new Hit(npc, AttackStyle.CRUSH).randDamage(dmg).ignoreDefence().ignorePrayer().delay(i));
		}
		return true;
	}
