...
public class PVMInstance {
	...
	private void build() throws DynamicMap.DynamicMapBuildException {
		// Create the map
		map = new DynamicMap();
			...
			map.getNpcs().forEach(n -> {
				if (n.getCombat() != null) {
					...
					n.deathEndListener = (entity, killer, killHit) -> {
						if (old != null)
							old.handle(entity, killer, killHit);
						if (timeLeft <= 0)
			...
			while (!destroyed) {
				if (timeLeft > 0) {
					...
					if (timeLeft == 3000) {
						...
						map.forPlayers(
								p -> p.sendMessage(Color.RED.wrap("The instance has expired. Monsters will no longer respawn.")));
					}
				}
				if (playersInside == 0) {
					idleTime++;
					if (idleTime >= 1000) {
						...
						if (owner != null)
							owner.sendMessage(Color.RED
									.wrap("Your " + type.getName() + " instance has expired, as it was left empty for over 10 minutes."));
					}
				} else {
					idleTime = 0;
				}
	...
	private void onExit(Player player, boolean logout) {
		...
		if (playersInside == 0 && timeLeft <= 0) {
			...
			Player owner = World.getPlayer(ownerId, true);
			if (owner != null) {
				player.sendMessage(Color.RED.wrap("Your " + type.getName()
						+ " instance is now empty. If it remains empty for 10 consecutive minutes, it will be destroyed."));
			}
		}
	}
	...
	private void destroy(boolean remove) {
		...
		// Loggers.logPvMInstance(ownerId, type.getName(), type.getCost(), timeCreated,
		// timeDestroyed);
	}

	private Position convertPosition(Position pos) {
		if (!pos.inBoundsWithoutPlane(type.getBounds())) {
