...
public class AbyssalSire extends NPCCombat {
	...
	public boolean damageWithTentacle = false;
	public boolean scionsMatured = false;
	public boolean damagedByMiasma = false;
	public boolean damagedByExplosion = false;
	public boolean damagedOffPrayer = false;
...
//        npc.hitsUpdate.hpBarType = 2;
		...
		AttackNpcListener attackListener = (player, npc1, message) -> {
			if (World.type == WorldType.ECO && !Slayer.isTask(player, npc) && !player.getName().equalsIgnoreCase("Kal")) { // slayer-only, single boss despite being in multi
				if (message)
		...
		npc.hitListener = new HitListener().preDamage(this::preDefend).postDefend(this::postDefend);
		npc.addEvent(event -> {
			event.delay(100);
			while (true) {
	...
	private void preDefend(Hit hit) {
		if (phase < 2) {
			...
			if (hit.attacker != null && hit.attacker.isPlayer())
				hit.attacker.player.sendMessage("The Abyssal Sires tentacles block all incoming damage.");
		}
	}
	...
	private void postDefend(Hit hit) {
		...
		} else if (phase == 1 && npc.getId() != 5888 && hit.attacker != null) { // look for stun
		...
		World.startEvent(event -> {
			...
			npc.localPlayers().forEach(p -> {
				if (Misc.getEffectiveDistance(npc, p) <= 1) {
					...
					if (p.vengeanceActive) {
						Objects.requireNonNull(p.combatAchievementsList.get(p.getCombatAchievementIndexByOrdinal(CombatAchievements.DEMONIC_REBOUND.ordinal())).
							getCombatAchievement()).check(p);
					}
					p.hit(new Hit(npc, null).randDamage(50, 72).ignoreDefence().ignorePrayer().delay(0));
				} else {
					p.sendMessage("Avoided explosion");
				}
			});
			int spawns = Random.get(3, 6);
			for (int i = 0; i < spawns; i++) {
				spawn(Random.get(npc.getPosition().getRegion().players), npc.getPosition().relative(Random.get(1, 4), Random.get(1, 4)));
			}
			npc.unlock();
			npc.face(Direction.SOUTH);
		});
	...
	private void teleportPlayer() {
		if (target == null) target = World.getPlayer(activatedBy);
		if(target != null) {
			final Player player = target.player;
			if (player.getPosition().distance(npc.getPosition()) > 15)
				return;
	...
	private void startPhase3() {
		...
		npc.addEvent(event -> {
			...
			while (!npc.isAt(dest)) {
				npc.step(dest.getX() - npc.getAbsX(), dest.getY() - npc.getAbsY(), StepType.WALK);
				event.delay(1);
			}
	...
	private void startFight(int uid) {
		...
		delayAttack(8);
		wakeTentacles();
		respiratorySystemNPCs.forEach(resp -> resp.hit(new Hit().fixedDamage(resp.getHp())));
	}
	...
	private void resetStates() {
		respiratorySystemNPCs.forEach(n -> {
			n.remove();
			n.getCombat().respawn();
		});
		respiratorySystemObjects.forEach(o -> o.setId(26953));
