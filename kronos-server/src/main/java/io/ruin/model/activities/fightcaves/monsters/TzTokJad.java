...
public class TzTokJad extends NPCCombat {
	...
	public boolean offPrayerHit = false;
		...
		if (Random.rollPercent(50)) {
			...
			npc.addEvent(e -> {
				...
				Hit hit = new Hit(npc, AttackStyle.MAGIC)
					...
					.clientDelay(delay);
				hit.preDamage(t -> {
					if (target != null && target.isPlayer() && !target.player.getPrayer().isActive(Prayer.PROTECT_FROM_MAGIC))
						offPrayerHit = true;
				});
				hit.postDamage(t -> {
					t.graphics(157);
			...
			npc.addEvent(e -> {
				...
				Hit hit = new Hit(npc, AttackStyle.RANGED)
					...
					.delay(2);
				hit.preDamage(t -> {
					if (target != null && target.isPlayer() && !target.player.getPrayer().isActive(Prayer.PROTECT_FROM_MISSILES))
						offPrayerHit = true;
				});
				hit.postDamage(t -> {
					t.graphics(157);
