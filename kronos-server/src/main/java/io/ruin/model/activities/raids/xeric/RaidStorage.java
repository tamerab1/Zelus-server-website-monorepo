...
public class RaidStorage extends ItemContainer {
	...
	public static void register() {
		...
		InterfaceHandler.register(Interface.RAID_STORAGE_INVENTORY, h -> {
			...
			h.actions[1] = (DefaultAction) (p, option, slot, id) -> {
				if (VarPlayerRepository.RAIDS_STORAGE_PRIVATE_INVENTORY.get(p) == 0) { // storing into shared storage
					if (!ChambersOfXeric.isRaiding(p))
						return;
					RaidStorage storage = p.raidsParty.getRaid().getStorage();
					switch (option) {
	...
	public static void openBuildMenu(Player player) {
		if (!ChambersOfXeric.isRaiding(player))
			return;
		player.openInterface(ToplevelComponent.MAINMODAL, Interface.CONSTRUCTION_FURNITURE_CREATION);
		int count = 1;
		...
		player.getPacketSender().sendIfEvents(458, 2, 1, count - 1, 1);
	...
	public static void selectStorageToBuild(Player p, int level) {
		...
		if (!p.getInventory().contains(Tool.HAMMER, 1) && !p.getInventory().contains(13576, 1) && !p.getInventory().contains(25644, 1) && !p.getEquipment().hasId(Tool.IMCANDO_HAMMER) && !p.getInventory().contains(23620, 1)) {
	...
	public static void openPrivateStorage(Player player) {
		player.getPrivateRaidStorage().clear();
		player.getPrivateRaidStorage().sendUpdates();
		player.getPrivateRaidStorage().sendAll = true;
		if (player.raidPrivateStorageItems != null) {
			for (Map.Entry<Integer, Integer> entry : player.raidPrivateStorageItems.entrySet()) {
				int key = entry.getKey();
				int value = entry.getValue();
				Item itemToMove = new Item(key, value);
				player.getPrivateRaidStorage().add(new Item(itemToMove.getId(), itemToMove.getAmount()));
				player.getPrivateRaidStorage().sendUpdates();
			}
		}
		player.openInterface(ToplevelComponent.MAINMODAL, Interface.RAID_PRIVATE_STORAGE);
		if (ChambersOfXeric.isRaiding(player)) {
			player.openInterface(ToplevelComponent.SIDEMODAL, Interface.RAID_STORAGE_INVENTORY);
			VarPlayerRepository.RAIDS_STORAGE_PRIVATE_INVENTORY.set(player, 1);
		}
	...
	private transient ChambersOfXeric raid;
	...
	public int add(int id, int amount, int uniqueValue) {
		...
		if (def == null || !def.coxItem)
			return 0;
		return super.add(id, amount);
	}


	public void open(Player player) {
		player.openInterface(ToplevelComponent.MAINMODAL, Interface.RAID_SHARED_STORAGE);
		player.openInterface(ToplevelComponent.SIDEMODAL, Interface.RAID_STORAGE_INVENTORY);
		VarPlayerRepository.RAIDS_STORAGE_PRIVATE_INVENTORY.set(player, 0);
