...
public class PlayerRestore {
	...
	private static final String IPS_PATH = ServerWrapper.dataFolder.getAbsolutePath() + "/runtime/player_restore/";
	...
	private static boolean reconstructStats(Player player) {
		Server.siteDb.exec((connection) -> {

			PreparedStatement ps = connection.prepareStatement("SELECT * FROM hiscores WHERE username='" + player.getName() + "'");
			...
			while (rs.next()) {
				...
				VarPlayerRepository.PVP_DEATHS.set(player, rs.getInt("deaths"));
				VarPlayerRepository.PVP_KILLS.set(player, rs.getInt("kills"));
