...
public class TimeUtils {
	...
	public static String fromMsShort(long ms) {
		long days = TimeUnit.MILLISECONDS.toDays(ms);
		long hours = TimeUnit.MILLISECONDS.toHours(ms);
		long minutes = TimeUnit.MILLISECONDS.toMinutes(ms);
		if (days > 0) {
	...
	public static void timeTask(Runnable r, Consumer<Long> timeMs) {
		var s = System.currentTimeMillis();
		r.run();
		timeMs.accept(System.currentTimeMillis() - s);
	}

	public interface RunnableExceptionally {
		void run() throws Exception;
	}

	public static void timeTaskExceptionally(RunnableExceptionally r, Consumer<Long> timeMs)
			throws Exception {
		var s = System.currentTimeMillis();
		r.run();
		timeMs.accept(System.currentTimeMillis() - s);
