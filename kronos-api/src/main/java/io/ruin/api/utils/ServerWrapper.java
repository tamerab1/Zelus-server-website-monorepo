...
public class ServerWrapper {
	...
	public static File dataFolder = parseDataFolder();

	private static File parseDataFolder() {
		...
		return FileUtils.get(properties.getProperty("data_path"));
	}

	public static void init(Class<?> c) {
		modulePath = new File(c.getProtectionDomain().getCodeSource().getLocation().getPath()).getParentFile()
			.getAbsolutePath()
			.replace("%20", " ")
			.replace("target", "");
	}
