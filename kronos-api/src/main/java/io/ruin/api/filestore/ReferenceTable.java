...
public class ReferenceTable {
	...
	public ReferenceTable(byte[] archiveData) {
		...
		boolean sized = (0x4 & hash) != 0;

		this.archiveCount = protocol >= 7 ? in.gSmart2or4() : in.readUnsignedShort();
		this.archiveIds = new int[archiveCount];

		int lastArchiveId = 0;
		int highestArchiveId = -1;
		for (int index = 0; index < archiveCount; index++) {
			int archiveId = lastArchiveId += protocol >= 7 ? in.gSmart2or4() : in.readUnsignedShort();
		...
		for (int index = 0; index < archiveCount; index++) {
			int archiveId = archiveIds[index];
			int fileCount = protocol >= 7 ? in.gSmart2or4() : in.readUnsignedShort();
		...
		for (int index = 0; index < archiveCount; index++) {
			...
			for (int i = 0; i < archive.fileCount; i++) {
				int fileId = lastFileId += protocol >= 7 ? in.gSmart2or4() : in.readUnsignedShort();
