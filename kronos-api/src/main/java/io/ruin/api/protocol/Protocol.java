...
public class Protocol {
	...
	public static OutBuffer messagePacket(String message, String sender, int type) {
		int senderLen = Protocol.strLen(sender);
		int estimatedSize = 4 + Protocol.strLen(message) + senderLen;
		OutBuffer out = new OutBuffer(estimatedSize).sendVarBytePacket(MESSAGE_GAME)
			.addSmart1or2(type);
		if (senderLen > 0) {
			out.addByte(1);
			out.addStringNullTerminated(sender);
		} else {
			out.addByte(0);
		}
		out.addStringNullTerminated(message);
		return out;
	}

	public static OutBuffer outgoingPm(String toUsername, String message) {
		OutBuffer out = new OutBuffer(255).sendVarShortPacket(MESSAGE_PRIVATE_ECHO)
			.addStringNullTerminated(toUsername);
