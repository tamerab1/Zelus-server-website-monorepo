...
public class Commands implements CommandHandler.Hook {
	...
	public static void register() {
		CommandHandler.hooks.register(Handle.class, Commands::handle);
	}

	public static HooksV2.Result handle(Handle ctx) {
		...
		switch (command) {
			case "donationdeals" -> {
				DonationDealsInterface.open(player);
